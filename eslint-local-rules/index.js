    ...
    create: function (context) {
      return {
        ImportDeclaration(node) {
          ...
          if (
            /^@clairview\/[^/]+\/.*$/.test(importPath) &&
            importPath !== "@clairview/backend-core/tests" &&
            importPath !== "@clairview/string-templates/test/utils"
          ) {
            context.report({
              node,
              message: `Importing from @clairview is not allowed, except for @clairview/backend-core/tests and @clairview/string-templates/test/utils.`,
            })
          }
        },
      }
