...
import { Api, BaseType, ProjectListType, ProjectTypes, UserType, WorkspaceType } from 'clairview-sdk';
...
const sqliteFilePath = (workerId: string) => {
  ...
  return `${rootDir}/../../packages/clairview/test_sakila_${workerId}.db`;
};
  ...
  try {
    ...
    if (isSuperUser && process.env.NC_CLOUD !== 'true') {
      ...
      response = await axios.post('http://localhost:8080/api/v1/auth/user/signin', {
        email: `user@clairview.com`,
        password: getDefaultPwd(),
      });
    } else {
      response = await axios.post('http://localhost:8080/api/v1/auth/user/signin', {
        email: `user-${parallelId}@clairview.com`,
        password: getDefaultPwd(),
      });
    }
      ...
      for (const w of ws.list) {
        ...
        if (w.title.startsWith(`ws_pgExtREST${process.env.TEST_PARALLEL_INDEX}`)) {
          try {
            ...
            console.log(`Error deleting workspace: ${w.id}`, `user-${parallelId}@clairview.com`, isSuperUser);
          }
        }
      }
    } else {
        ...
        for (const p of bases.list) {
          ...
          if (
            ...
            try {
              ...
              console.log(`Error deleting base: ${p.id}`, `user-${parallelId}@clairview.com`, isSuperUser);
            }
          }
        }
      }
    ...
    if (dbType === 'pg' && !isEmptyProject) {
      ...
      if (!isEmptyProject) {
        const testsDir = path.join(process.cwd(), '../../packages/clairview/tests');
  ...
  await page.addInitScript(
    async ({ token }) => {
      ...
      try {
        ...
        try {
          initialLocalStorage = JSON.parse(localStorage.getItem('clairview-gui-v2') || '{}');
        } catch (e) {
...
// Reference
// packages/clairview/src/lib/services/test/TestResetService/resetPgSakilaProject.ts
