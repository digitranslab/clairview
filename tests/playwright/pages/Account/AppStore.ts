...
export class AccountAppStorePage extends BasePage {
  ...
  get() {
    return this.accountPage.get().locator(`[data-testid="cv-settings-subtab-appStore"]`);
  }

  async install({ name }: { name: string }) {
    const card = this.accountPage.get().locator(`.cv-app-store-card-${name}`);
    await card.click();
    ...
    for (let i = 0; i < 5; i++) {
      if (await card.locator('.cv-app-store-card-install').isVisible()) {
  ...
  async configureSMTP({ email, host, port }: { email: string; host: string; port: string }) {
    const appStoreCard = this.rootPage.locator('.cv-modal-plugin-install');
  ...
  async uninstall(param: { name: string }) {
    const card = this.accountPage.get().locator(`.cv-app-store-card-${param.name}`);
