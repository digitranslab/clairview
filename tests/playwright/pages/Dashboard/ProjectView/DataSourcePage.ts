...
export class DataSourcePage extends BasePage {
  ...
  constructor(baseView: ProjectViewPage) {
    ...
    this.databaseType = this.get().locator('.cv-extdb-db-type');
  ...
  get() {
    return this.rootPage.locator('.cv-data-sources-view');
  }

  getDsDetailsModal() {
    return this.rootPage.locator('.cv-active-data-sources-view');
  }

  async closeDsDetailsModal() {
    await this.getDsDetailsModal().locator('.cv-close-btn').click();
    await this.getDsDetailsModal().waitFor({ state: 'hidden' });
  }

  async getDatabaseTypeList() {
    await this.databaseType.click();
    const nodes = this.rootPage.locator('.cv-dropdown-ext-db-type').locator('.ant-select-item');
    const list = [];
  ...
  async openMetaSync({ rowIndex }: { rowIndex: number }) {
    ...
    await this.getDsDetailsModal().getByTestId('cv-meta-sync-tab').click();
    // await row.getByTestId('cv-data-sources-view-meta-sync').click();
  }
  ...
  async openERD({ rowIndex }: { rowIndex: number }) {
    ...
    await this.getDsDetailsModal().getByTestId('cv-acl-tab').click();
    await this.getDsDetailsModal().getByTestId('cv-erd-tab').click();
  }
  ...
  async openAudit({ rowIndex }: { rowIndex: number }) {
    ...
    await this.getDsDetailsModal().waitFor({ state: 'visible' });
    await this.getDsDetailsModal().getByTestId('cv-audit-tab').click();

    // await row.getByTestId('cv-data-sources-view-audit').click();
  }
