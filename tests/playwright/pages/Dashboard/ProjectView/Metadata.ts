...
export class MetaDataPage extends BasePage {
  ...
  async verifyRow({ index, model, state }: { index: number; model: string; state: string }) {
    const fieldLocator = this.get().locator(`tr.cv-table-row`).nth(index).locator(`td.cv-table-cell`).nth(0);
    const fieldText = await getTextExcludeIconText(fieldLocator);
    expect(fieldText).toBe(model);

    await expect(this.get().locator(`tr.cv-table-row`).nth(index).locator(`td.cv-table-cell`).nth(1)).toHaveText(
      state,
      {
        ignoreCase: true,
      }
