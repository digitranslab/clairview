...
export class ToolbarCalendarRangePage extends BasePage {
  ...
  get() {
    return this.rootPage.getByTestId('cv-calendar-range-menu');
  }

  async click({ title }: { title: string }) {
    await this.get().getByTestId('cv-calendar-range-from-field-select').click();
  ...
  async newCalendarRange({ fromTitle, toTitle }: { fromTitle: string; toTitle?: string }) {
    ...
    if (toTitle) {
      await this.get().getByTestId(`cv-calendar-range-to-field-select`).click();
  ...
  async verifyCalendarRange({ fromTitle, toTitle }: { fromTitle: string; toTitle?: string }) {
    const from = await this.get().getByTestId('cv-calendar-range-from-field-select');

    const fromFieldText = await getTextExcludeIconText(from);
    expect(fromFieldText).toBe(fromTitle);

    if (toTitle) {
      const to = await this.get().getByTestId('cv-calendar-range-to-field-select');
