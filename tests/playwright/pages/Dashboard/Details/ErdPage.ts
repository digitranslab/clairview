...
export class ErdPage extends BasePage {
  ...
  constructor(details: DetailsPage) {
    ...
    this.btn_fullScreen = this.get().locator('.cv-erd-histogram > .cv-icon');
    this.btn_zoomIn = this.get().locator('.cv-erd-zoom-btn').nth(0);
    this.btn_zoomOut = this.get().locator('.cv-erd-zoom-btn').nth(1);

    this.contextMenuBase = this.get().locator('.cv-erd-context-menu');
  ...
  async verifyNode({
    ...
    await this.get().locator(`.cv-erd-table-node-${tableName}`).scrollIntoViewIfNeeded();
  ...
  async verifyJunctionTableLabel({ tableTitle, tableName }: { tableName: string; tableTitle: string }) {
    await this.get().locator(`.cv-erd-table-label-${tableTitle}-${tableName}`).waitFor({
      state: 'visible',
    });
  }
