...
export class OrgAdminPage extends BasePage {
  ...
  async goto() {
    ...
    if ((await this.rootPage.getByTestId('cv-sidebar-upgrade-workspace-to-org').count()) > 0) {
      await this.rootPage.getByTestId('cv-sidebar-upgrade-workspace-to-org').first().click();
    } else {
      await this.rootPage.getByTestId('cv-sidebar-org-admin-panel').click();
    }
