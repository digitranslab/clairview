...
test.describe('Command Shortcuts', () => {
  ...
  test('Verify Command K', async ({ page }) => {
    ...
    await page.waitForTimeout(1000);
    await dashboard.get().locator('.cv-active-view-title').waitFor({ state: 'visible' });
    await expect(dashboard.get().locator('.cv-active-view-title')).toContainText('Default View');
