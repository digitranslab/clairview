...
const fetchContributorStats = async (username: string) => {
  const apiUrl = `https://clairview.io/api/contributors/contributorStats/${username}`;
...
const fetchContributorImage = async (username: string) => {
  const apiUrl = `https://clairview.io/api/contributors/${username}/og.png`;
...
const runCongratulate = async () => {
  ...
  const { data: pullRequests } =
    await danger.github.api.rest.search.issuesAndPullRequests({
      q: `is:pr author:${userName} is:closed repo:digitranslab/clairview`,
      per_page: 2,
      page: 1,
    });
  ...
  const contributorUrl = `https://clairview.io/contributors/${userName}`;
  ...
  const message =
    ...
    `![Contributions](https://clairview.io/api/contributors/${userName}/og.png)`;
