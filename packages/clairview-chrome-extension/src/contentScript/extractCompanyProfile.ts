...
export const addCompany = async () => {
  ...
  // Convert URLs like https://www.linkedin.com/company/clairview/about/ to https://www.linkedin.com/company/clairview
  const companyURL = extractCompanyLinkedinLink(activeTab.url);
...
export const insertButtonForCompany = async () => {
  const companyButtonDiv = createDefaultButton('clairview-company-btn');
  ...
  const openCompanyOnSidePanel = (companyId: string) => {
    companyButtonSpan.textContent = 'View in Clairview';
  ...
  if (isDefined(company)) {
    ...
    companyButtonSpan.textContent = 'Add to Clairview';
