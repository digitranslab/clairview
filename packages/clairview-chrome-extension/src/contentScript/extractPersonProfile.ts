...
export const insertButtonForPerson = async () => {
  const personButtonDiv = createDefaultButton('clairview-person-btn');
  ...
  if (isDefined(personButtonDiv)) {
    ...
    const openPersonOnSidePanel = (personId: string) => {
      personButtonSpan.textContent = 'View in Clairview';
    ...
    if (isDefined(person)) {
      ...
      personButtonSpan.textContent = 'Add to Clairview';
      personButtonDiv.onClickHandler(async () => {
