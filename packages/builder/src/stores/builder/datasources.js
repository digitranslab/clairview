...
import { API } from "api"
import { DatasourceFeature } from "@clairview/types"
...
export function createDatasourcesStore() {
  ...
  const derivedStore = derived([store, tables], ([$store, $tables]) => {
    ...
    // internal datasource
    let internalDS = $store.list?.find(ds => ds._id === CLAIRVIEW_INTERNAL_DB_ID)
    let otherDS = $store.list?.filter(ds => ds._id !== CLAIRVIEW_INTERNAL_DB_ID)
    if (internalDS) {
      internalDS = {
        ...internalDS,
