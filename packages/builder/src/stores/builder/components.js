...
import { API } from "api"
import { Helpers } from "@clairview/bbui"
import analytics, { Events } from "analytics"
import { makePropSafe as safe } from "@clairview/string-templates"
import {
  findComponentPath,
...
import {
  CLAIRVIEW_INTERNAL_DB_ID,
  DEFAULT_BB_DATASOURCE_ID,
  DB_TYPE_INTERNAL,
  DB_TYPE_EXTERNAL,
} from "constants/backend"
import ClairStore from "../ClairStore"
import { Utils } from "@clairview/frontend-core"
import { FieldType } from "@clairview/types"
import { utils } from "@clairview/shared-core"
...
export class ComponentStore extends ClairStore {
  constructor() {
   ...
   * @example
   * '@clairview/standard-components/container'
   * @returns {object}
   */
  ...
  getDefaultDatasource() {
    ...
    let table = validTables.find(table => {
      return (
        table.sourceId === CLAIRVIEW_INTERNAL_DB_ID &&
  ...
  migrateSettings(enrichedComponent) {
    const componentPrefix = "@clairview/standard-components"
    let migrated = false
      ...
      if (value == null || value === "") {
        if (setting.type === "multifield" && setting.selectAllFields) {
          ...
          const form = findClosestMatchingComponent(
            screen.props,
            treeId,
            x => x._component === "@clairview/standard-components/form"
          )
      ...
      // Validate non-empty settings
        ...
        if (setting.type === "dataProvider") {
          ...
          const providers = findAllMatchingComponents(
            screen?.props,
            x => x._component === "@clairview/standard-components/dataprovider"
          )
  ...
  getComponentSettings(componentType) {
    ...
    if (
      ...
      !componentType.startsWith("@clairview")
    ) {
      componentType = `@clairview/standard-components/${componentType}`
    }
