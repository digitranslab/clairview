...
import { RoleUtils, Utils } from "@clairview/frontend-core"
...
export class ScreenStore extends ClairStore {
  constructor() {
    ...
    const findIllegalChild = (
      ...
      if (
        [
          ...
          "@clairview/standard-components/modal",
        ].includes(type)
      ) {
        illegalChildren = []
      }
      ...
      if (definition?.legalDirectChildren?.length) {
        legalDirectChildren = definition.legalDirectChildren.map(x => {
          return `@clairview/standard-components/${x}`
        })
      }
