...
const COMP_PREFIX = "@clairview/standard-components"
...
describe("Component store", () => {
  ...
  it("Retrieves component definitions by component type", ctx => {
    ...
    expect(def).toStrictEqual(
      componentDefs["@clairview/standard-components/container"]
    )

    const pluginDef = ctx.test.componentStore.getDefinition("plugin/clair-video")

    expect(pluginDef).toStrictEqual(pluginDefs["plugin/clair-video"])
  })
  ...
  it("Handle missing or invalid component definition keys", ctx => {
    ...
    const def = ctx.test.componentStore.getDefinition(
      "@clairview/standard-components/mystery"
    )
  ...
  it("enrichEmptySettings - initialise multifield type with schema keys", async ctx => {
    ...
    const componentDefs = componentDefinitionMap()
    const targetCompDef =
      componentDefs["@clairview/standard-components/rowexplorer"]
