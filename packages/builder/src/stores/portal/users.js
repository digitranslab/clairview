...
import { Constants } from "@clairview/frontend-core"
...
export function createUsersStore() {
  ...
  const getUserRole = user => {
    if (user && user.email === user.tenantOwnerEmail) {
      return Constants.ClairviewRoles.Owner
    } else if (sdk.users.isAdmin(user)) {
      return Constants.ClairviewRoles.Admin
    } else if (sdk.users.isBuilder(user)) {
      return Constants.ClairviewRoles.Developer
    } else if (sdk.users.hasCreatorPermissions(user)) {
      return Constants.ClairviewRoles.Creator
    } else {
      return Constants.ClairviewRoles.AppUser
    }
  }
