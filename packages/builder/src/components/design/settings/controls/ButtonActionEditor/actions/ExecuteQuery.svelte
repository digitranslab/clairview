<script>
  import { Select, Layout, Checkbox } from "@clairview/bbui"
  ...
  // Executequery must exclude clairview datasource
  $: executeQueryDatasources = $datasources.list.filter(
    x =>
      x._id !== CLAIRVIEW_INTERNAL_DB_ID && x.type !== CLAIRVIEW_DATASOURCE_TYPE
  )
