<script>
  ...
  let error
  let form
  let creationRoleType = Constants.ClairviewRoles.AppUser
  ...
  */
    ...
    const payload = [
      {
        email: newUserEmail,
        builder: {
          global: creationRoleType === Constants.ClairviewRoles.Admin,
          creator: creationRoleType === Constants.ClairviewRoles.Creator,
        },
        admin: { global: creationRoleType === Constants.ClairviewRoles.Admin },
      },
    ]

    const notCreatingAdmin = creationRoleType !== Constants.ClairviewRoles.Admin
  ...
  const checkAppAccess = e => {
    ...
    if (
      e.detail === Constants.ClairviewRoles.AppUser &&
      creationAccessType === Constants.Roles.CREATOR
    ) {
      creationAccessType = Constants.Roles.BASIC
    } else if (e.detail === Constants.ClairviewRoles.Admin) {
      creationAccessType = Constants.Roles.CREATOR
    }
  }
</script>
          ...
          <FancyForm bind:this={form}>
            ...
            <FancySelect
              ...
              options={sdk.users.isAdmin($auth.user)
                ? Constants.ClairviewRoleOptions
                : Constants.ClairviewRoleOptions.filter(
                    option => option.value !== Constants.ClairviewRoles.Admin
                  )}
              label="Role"
            ...
            <span class="role-wrap">
              <RoleSelect
                ...
                allowCreator={creationRoleType !==
                  Constants.ClairviewRoles.AppUser}
                quiet={true}
                autoWidth
                align="right"
                fancySelect
                allowedRoles={creationRoleType === Constants.ClairviewRoles.Admin
                  ? [Constants.Roles.CREATOR]
                  : null}
                footer={getRoleFooter({
                  isAdminOrGlobalBuilder:
                    creationRoleType === Constants.ClairviewRoles.Admin,
                })}
              />
            </span>
          </FancyForm>
