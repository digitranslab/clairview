<script>
  ...
  import { Layout, Heading, Body, Icon, notifications } from "@clairview/bbui"
  import ErrorSVG from "@clairview/frontend-core/assets/error.svg?raw"
  import { findComponent, findComponentPath } from "helpers/components"
  import { isActive, goto } from "@roxi/routify"
  import { ClientAppSkeleton } from "@clairview/frontend-core"
  import { getThemeClassNames, ThemeClassPrefix } from "@clairview/shared-core"
  ...
  // Clairview events are and initialisation events
  const MessageTypes = {
    READY: "ready",
    ERROR: "error",
    CLAIRVIEW: "type",
  ...
  const receiveMessage = async message => {
    ...
    try {
      await handleClairviewEvent(message)
    } catch (error) {
  ...
  const handleClairviewEvent = async event => {
