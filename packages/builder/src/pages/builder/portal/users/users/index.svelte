<script>
  ...
  import { Constants, Utils, fetchData } from "@clairview/frontend-core"
  import { API } from "api"
  ...
  const setEnrichedUsers = async (rows, owner) => {
    enrichedUsers = rows?.map(user => {
      ...
      const role = Constants.ExtendedClairviewRoleOptions.find(
      ...
      return {
        ...
        userGroups,
        __selectable:
          role.value === Constants.ClairviewRoles.Owner ||
  ...
  async function createUserFlow() {
    const payload = userData?.users?.map(user => ({
      email: user.email,
      builder: user.role === Constants.ClairviewRoles.Developer,
      creator: user.role === Constants.ClairviewRoles.Creator,
      admin: user.role === Constants.ClairviewRoles.Admin,
    ...
    <InlineAlert
      ...
      onConfirm={() => {
        if (isOwner) {
          ...
          window.open("https://clairview.com/pricing/", "_blank")
        }
      }}
