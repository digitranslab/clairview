<script>
  ...
  import { Constants, UserAvatar } from "@clairview/frontend-core"
  ...
  async function updateUserRole({ detail }) {
    if (detail === Constants.ClairviewRoles.Developer) {
      toggleFlags({ admin: { global: false }, builder: { global: true } })
    } else if (detail === Constants.ClairviewRoles.Admin) {
      toggleFlags({ admin: { global: true }, builder: { global: true } })
    } else if (detail === Constants.ClairviewRoles.AppUser) {
      toggleFlags({ admin: { global: false }, builder: { global: false } })
    } else if (detail === Constants.ClairviewRoles.Creator) {
      toggleFlags({
        admin: { global: false },
        builder: {
          global: false,
          ...
          <div class="field">
            ...
            <Select
              ...
              options={isTenantOwner
                ? Constants.ExtendedClairviewRoleOptions
