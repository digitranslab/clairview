<script>
  ...
  async function getVersion() {
    try {
      version = await API.getClairviewVersion()
    } catch (error) {
      notifications.error("Error getting Clairview version")
      version = null
    }
  }
  ...
  async function getLatestVersion() {
    try {
      ...
      const githubCheck = await fetch(
        "https://api.github.com/repos/Clairview/clairview/releases/latest"
      )
      ...
      notifications.error("Error getting the latest Clairview version")
      githubVersion = null
    }
  }
...
{#if sdk.users.isAdmin($auth.user)}
  <Layout noPadding>
    <Layout gap="XS" noPadding>
      ...
      <Body>
        Keep your clairview installation up to date to take advantage of the
    ...
    {#if loaded}
      ...
      <Divider />
      <Layout noPadding gap="XS">
        <Heading>Updating Clairview</Heading>
        <Body
          >To update your self-host installation, follow the docs found <Link
            size="L"
            href="https://docs.clairview.com/docs/updating-clairview">here.</Link
          ></Body
        >
      </Layout>
    {/if}
