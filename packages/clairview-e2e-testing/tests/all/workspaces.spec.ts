...
test.describe('', () => {
  test('Testing logging', async ({ page }) => {
    ...
    await expect(page.getByText('Welcome to Clairview')).not.toBeVisible();
  ...
  test('Syncing all workspaces', async () => {
    await sh('npx nx run clairview-server:command workspace:sync-metadata -f');
    await sh('npx nx run clairview-server:command workspace:sync-metadata -f');
  });

  test('Resetting database', async ({ page, browserName }) => {
    if (browserName === 'chromium') {
      await sh('yarn nx database:reset clairview-server'); // if this command fails for any reason, database must be restarted manually using the same command because database is in unstable state
      await page.goto('/');
      await page.getByRole('button', { name: 'Continue With Email' }).click();
      await page.getByPlaceholder('Email').fill('tim@apple.dev');
      await page.getByRole('button', { name: 'Continue' }).click();
