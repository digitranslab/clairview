...
class NewMgr {
  ...
  public static async getNewProjectInput(args): Promise<any> {
    ...
    console.log(
      `ClairView requires a database to store metadata of database-spreadsheets.\nPlease enter the database credentials (defaults to SQLite3)`
        .green.bold
    );
    ...
    if (answers.projectType === 'mvc' && !answers.isForExisting) {
      ...
      await promisify(download)(
        'direct:https://github.com/digitranslab/clairview-seed/archive/refs/heads/main.zip',
        args.folder
      );
        ...
        console.log(
          boxen(
            `# Please run the following docker commands\n\n${
              `docker run -p 8080:8080 ${linuxHost} -d ${env} digitranslab/clairview:latest`
                .green.bold
            }\n`,
            {
              borderColor: 'green',
...
\t${`npm install --save clairview`.green}
      
2. Add the following code in your existing express project

\t${
        `const {Noco} = require("clairview");
