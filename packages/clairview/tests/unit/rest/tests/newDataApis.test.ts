...
function userFieldBased() {
  ...
  it('Create record : using email', async function () {
    const newRecord = {
      userFieldSingle: 'a@clairview.com',
      userFieldMulti: 'a@clairview.com,b@clairview.com',
    };
    const rsp = await ncAxiosPost({ body: newRecord });
    expect(rsp.body).to.deep.equal({ Id: 401 });
    ...
    expect(record.body.userFieldSingle[0].email).to.equal('a@clairview.com');
    expect(record.body.userFieldMulti[0].email).to.equal('a@clairview.com');
    expect(record.body.userFieldMulti[1].email).to.equal('b@clairview.com');
  });
  ...
  it('Create record : using ID', async function () {
    ...
    const id1 = userList.find((u) => u.email === 'a@clairview.com').id;
    ...
    expect(record.body.userFieldMulti[1].email).to.equal('a@clairview.com');
  });
