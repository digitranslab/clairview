...
const up = async (knex) => {
  await knex.schema.createTable('cv_projects', (table) => {
  ...
  await knex.schema.createTable('cv_hooks', (table) => {
    table.increments();
    table.string('project_id');
    table.string('db_alias').defaultTo('db');
    table.string('title');
  ...
  await knex.schema.createTable('cv_acl', (table) => {
    table.increments();
    table.string('project_id');
    table.string('db_alias').defaultTo('db');
    table.string('tn');
  ...
  await knex.schema.createTable('cv_models', (table) => {
    table.increments();
    table.string('project_id');
    table.string('db_alias').defaultTo('db');
    table.string('title');
  ...
  await knex.schema.createTable('cv_projects_users', (table) => {
    table.string('project_id').index(); // .references('id').inTable('cv_projects')
    // todo: foreign key
  ...
  await knex.schema.createTable('cv_shared_views', (table) => {
    table.increments();
    table.string('project_id');
    table.string('db_alias');
  ...
  await knex.schema.createTable('cv_plugins', (table) => {
    table.increments();
    table.string('project_id');
    table.string('db_alias');
    table.string('title', 45);
  ...
  await knex.schema.createTable('cv_migrations', (table) => {
    table.increments();
    table.string('project_id');
    table.string('db_alias');
    table.text('up');
...
const down = async (knex) => {
  await knex.schema.dropTable('cv_plugins');
  await knex.schema.dropTable('nc_disabled_models_for_role');
  await knex.schema.dropTable('cv_shared_views');
  await knex.schema.dropTable('cv_projects_users');
  await knex.schema.dropTable('cv_projects');
  await knex.schema.dropTable('nc_roles');
  await knex.schema.dropTable('cv_hooks');
  await knex.schema.dropTable('nc_store');
  await knex.schema.dropTable('nc_cron');
  await knex.schema.dropTable('cv_acl');
  await knex.schema.dropTable('cv_models');
  ...
  await knex.schema.dropTable('cv_migrations');
