...
import type { ErrorReportReqType } from 'clairview-sdk';
...
export class UtilsService {
  ...
  async versionInfo() {
    if (
      ...
      const nonBetaTags = await axios
        .get('https://api.github.com/repos/digitranslab/clairview/tags', {
          timeout: 5000,
        })
        .then((response) => {
  ...
  async appInfo(param: { req: { ncSiteUrl: string } }) {
    ...
    if (instance.impacted >= 5) {
      giftUrl = `https://w21dqb1x.clairview.com/#/nc/form/4d2e0e4b-df97-4c5e-ad8e-f8b8cca90330?Users=${
        instance.impacted
  ...
  async feed(req: NcRequest) {
    ...
    try {
      response = await axios.post(
        'https://product-feed.clairview.com/api/v1/social/feed',
        payload,
        {
          params: {
            per_page: perPage,
