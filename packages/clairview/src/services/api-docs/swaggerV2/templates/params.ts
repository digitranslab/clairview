import { isLinksOrLTAR, RelationTypes, UITypes } from 'clairview-sdk';
...
export const whereParam = {
  ...
  name: 'where',
  description:
    'This can be used for filtering rows, which accepts complicated where conditions. For more info visit [here](https://docs.clairview.com/developer-resources/rest-apis#comparison-operators). Example : `where=(field1,eq,value)`',
};
...
export const nestedWhereParam = (colName) => ({
  ...
  description: `This can be used for filtering rows in nested column \`${colName}\`, which accepts complicated where conditions. For more info visit [here](https://docs.clairview.com/developer-resources/rest-apis#comparison-operators). Example : \`nested[${colName}][where]=(field1,eq,value)\``,
});
