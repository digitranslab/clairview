      ...
      if (!Tele.emitter) {
        ...
        Tele.emitter.on('evt_app_started', async (msg) => {
          try {
            ...
            await axios.post('https://telemetry.clairview.com/api/v1/telemetry', {
              ...teleData,
              evt_type: 'started',
              payload: {
                count: global.NC_COUNT,
        ...
        Tele.emitter.on('evt_api_created', async (data) => {
          try {
            ...
            if (isDisabled) return;
            await axios.post(
              'https://telemetry.clairview.com/api/v1/telemetry/apis_created',
              stats,
            );
          } catch (e) {}
        });
        ...
        Tele.emitter.on('evt_subscribe', async (email) => {
          try {
            if (isDisabled) return;
            await axios.post(
              'https://telemetry.clairview.com/api/v1/newsletter/sdhjh34u3yuy34bj343jhj4iwolaAdsdj3434uiut4nn',
              {
                email,
              },
            );
