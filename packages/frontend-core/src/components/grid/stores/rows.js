...
export const createActions = context => {
  ...
  const handleNewRows = (newRows, resetRows) => {
    ...
    let rowsToAppend = []
    let newRow
    const $hasClairviewIdentifiers = get(hasClairviewIdentifiers)
    for (let i = 0; i < newRows.length; i++) {
      ...
      if (!$hasClairviewIdentifiers && !newRow._id?.length) {
  ...
  const cleanRow = row => {
    ...
    if (!get(hasClairviewIdentifiers) && isGeneratedRowID(clone._id)) {
      delete clone._id
    }
    return clone
  }
