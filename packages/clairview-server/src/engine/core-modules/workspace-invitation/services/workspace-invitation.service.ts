...
import ms from 'ms';
import { SendInviteLinkEmail } from 'clairview-emails';
    ...
    for (const invitation of invitationsPr) {
      if (invitation.status === 'fulfilled') {
        ...
        await this.emailService.send({
          from: `${sender.firstName} ${sender.lastName} (via Clairview) <${this.environmentService.get('EMAIL_FROM_ADDRESS')}>`,
          to: invitation.value.email,
          subject: 'Join your team on Clairview',
          text,
          html,
        });
      }
