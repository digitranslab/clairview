...
export class GoogleAPIsService {
  ...
  async refreshGoogleRefreshToken(input: {
    ...
    const connectedAccountRepository =
      await this.clairviewORMGlobalManager.getRepositoryForWorkspace<ConnectedAccountWorkspaceEntity>(
        workspaceId,
        'connectedAccount',
      );
    ...
    const calendarChannelRepository =
      await this.clairviewORMGlobalManager.getRepositoryForWorkspace<CalendarChannelWorkspaceEntity>(
        workspaceId,
        'calendarChannel',
      );

    const messageChannelRepository =
      await this.clairviewORMGlobalManager.getRepositoryForWorkspace<MessageChannelWorkspaceEntity>(
        workspaceId,
        'messageChannel',
      );

    const workspaceDataSource =
      await this.clairviewORMGlobalManager.getDataSourceForWorkspace(workspaceId);
    ...
    await workspaceDataSource.transaction(async (manager: EntityManager) => {
      if (!existingAccountId) {
        ...
        const workspaceMemberRepository =
          await this.clairviewORMGlobalManager.getRepositoryForWorkspace<WorkspaceMemberWorkspaceEntity>(
            workspaceId,
            'workspaceMember',
          );
