...
import { WorkspaceDataSource } from 'src/engine/clairview-orm/datasource/workspace.datasource';
import {
  ClairviewORMException,
  ClairviewORMExceptionCode,
} from 'src/engine/clairview-orm/exceptions/clairview-orm.exception';
import { EntitySchemaFactory } from 'src/engine/clairview-orm/factories/entity-schema.factory';
import { CacheManager } from 'src/engine/clairview-orm/storage/cache-manager.storage';
...
export class WorkspaceDatasourceFactory {
  ...
  public async create(
    ...
    if (
      ...
      throw new ClairviewORMException(
        ...
        ClairviewORMExceptionCode.METADATA_VERSION_MISMATCH,
      );
    }
          ...
          async () => {
            ...
            if (!dataSourceMetadata) {
              throw new ClairviewORMException(
                ...
                ClairviewORMExceptionCode.WORKSPACE_SCHEMA_NOT_FOUND,
              );
            }
            ...
            if (!cachedObjectMetadataMaps) {
              throw new ClairviewORMException(
                `Workspace Schema not found for workspace ${workspaceId}`,
                ClairviewORMExceptionCode.METADATA_COLLECTION_NOT_FOUND,
              );
            }
    ...
    if (latestWorkspaceMetadataVersion === undefined) {
      ...
      if (failOnMetadataCacheMiss) {
        throw new ClairviewORMException(
          ...
          ClairviewORMExceptionCode.METADATA_VERSION_NOT_FOUND,
        );
      } else {
