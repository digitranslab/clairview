...
The root folder for unit tests is `packages/clairview/tests/unit`
...
We will configure `beforeEach` which is called before each test is executed. We will use `init` function from `clairview/packages/clairview/tests/unit/init/index.ts`, which is a helper function which configures the test environment(i.e resetting state, etc.).
...
We create a new file `table.test.ts` in `packages/clairview/tests/unit/rest/tests` directory.
...
We can then import the `Table` test suite to `Rest` test suite in `packages/clairview/tests/unit/rest/index.test.ts` file(`Rest` test suite is imported in the root test suite file which is `packages/clairview/tests/unit/index.test.ts`).
