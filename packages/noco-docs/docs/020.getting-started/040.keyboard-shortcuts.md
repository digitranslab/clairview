...
:::note
This feature is available only in ClairView Cloud hosted version.
:::
...
This shortcut is often referred to as "Command-K". It's a great way to save time when you're navigating around ClairView.
...
Quickly search through docs from within ClairView UI using `⌘` + `J` (or `Ctrl` + `J` on Windows). Search results will be displayed in a modal window; click on the result to open the page in a new tab.
