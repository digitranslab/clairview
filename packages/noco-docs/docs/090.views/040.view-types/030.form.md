---
title: 'Form'
description: 'Learn how to create, import, and manage forms in ClairView.'
tags: ['Views', 'Form view']
keywords: ['ClairView form', 'form view', 'form builder', 'form view builder', 'form view actions', 'form view operations']
---
...
#### Hide Branding
To hide ClairView branding from the form, toggle the `Hide ClairView Branding` switch in the **Appearance Settings** panel.
...
ClairView allows you to configure the form view to perform various actions after a form is submitted. These actions can be configured in the **Post Form Submission Settings** panel.
...
You can use **`{record_id}`** in the URL as a query parameter to display the newly created record as expanded form (provided user submitting form has access to the base). For example, `https://app.clairview.com/#/<workspaceId>/<baseId>/<tableId>?rowId={record_id}`
...
:::info
Field validations are only available in the cloud version of ClairView.
:::

ClairView allows you to configure input data validations for fields in the form. To configure field validation, click on the required field in the **Form Area**, and in the right configuration panel, configure the desired validations. Supported validations are type-specific and are listed below. 
...
ClairView suffixes an eye icon next to the field name in the form builder to indicate that the field is conditionally visible.

## Pre-Filling Form Fields
ClairView offers a convenient feature that allows pre-filling form fields with specific values by setting URL parameters. This functionality enables the creation of custom URLs with desired field values, streamlining data entry and enhancing user experience.
...
ClairView provides an intuitive alternative approach to generate pre-filled URLs through the form builder. 
...
## Survey Form View
ClairView supports a special type of form view called `Survey Form View`. This view is ideal for creating surveys & questionnaires. This view can be enabled by toggling the `Survey Mode` switch when creating [shared form view](/views/share-view#share-form-view-options). 
