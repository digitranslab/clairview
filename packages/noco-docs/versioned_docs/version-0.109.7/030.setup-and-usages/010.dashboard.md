...
Once you have started ClairView, you can visit the dashboard via `example.com`. You will be redirected to `example.com/#/signup`. 
...
Once you have logged into ClairView, you should see `My Projects`.
...
If you are running ClairView on Docker and your local DB is running on your host machine, your Host Address would be host.docker.internal instead of localhost.
...
Click `Test Database Connection` to see if the connection can be established or not. ClairView creates a new **empty database** with specified parameters if the database doesn't exist.
