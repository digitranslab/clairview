...
import { env, features } from "@clairview/backend-core"
 ...
 * of custom configurations and Clairview AI
...
export async function run({
  ...
  if (inputs.prompt == null) {
    return {
      success: false,
      response: "Clairview OpenAI Automation Failed: No prompt supplied",
    }
  }
  ...
  try {
    ...
    const clairviewAIEnabled =
      (await features.flags.isEnabled(FeatureFlag.CLAIRVIEW_AI)) &&
      (await pro.features.isClairviewAIEnabled())

    let llm
    if (clairviewAIEnabled || customConfigsEnabled) {
