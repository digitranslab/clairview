...
import { clairviewTempDir } from "../clairviewDir"
 ...
 */
export const init = () => {
  const tempDir = clairviewTempDir()
...
export const checkDevelopmentEnvironment = () => {
  ...
  if (!fs.existsSync(clairviewTempDir())) {
    fs.mkdirSync(clairviewTempDir())
  }
  let error
...
export const apiFileReturn = (contents: any) => {
  const path = join(clairviewTempDir(), uuid())
...
export const createTempFolder = (item: any) => {
  const path = join(clairviewTempDir(), item)
  try {
