...
import fs from "fs"
import { context, objectStore } from "@clairview/backend-core"
...
export function devClientLibPath() {
  return require.resolve("@clairview/client")
}
 ...
 * {appId}/manifest.json
 * {appId}/clairview-client.js
 *
 * The paths for the backups are:
 * {appId}/manifest.json.bak
 * {appId}/clairview-client.js.bak
 *
...
export async function backupClientLibrary(appId: string) {
  ...
  const tmpClientPath = await objectStore.retrieveToTmp(
    ObjectStoreBuckets.APPS,
    join(appId, "clairview-client.js")
  )
  ...
  const clientUpload = objectStore.upload({
    bucket: ObjectStoreBuckets.APPS,
    filename: join(appId, "clairview-client.js.bak"),
    path: tmpClientPath,
...
export async function updateClientLibrary(appId: string) {
  ...
  if (env.isDev()) {
    ...
    client = resolve(TOP_LEVEL_PATH, "client", "clairview-client.js")
  }
  ...
  const clientUpload = objectStore.streamUpload({
    bucket: ObjectStoreBuckets.APPS,
    filename: join(appId, "clairview-client.js"),
...
export async function revertClientLibrary(appId: string) {
  ...
  if (env.isDev()) {
    ...
    clientPath = await objectStore.retrieveToTmp(
      ObjectStoreBuckets.APPS,
      join(appId, "clairview-client.js.bak")
    )
  }
