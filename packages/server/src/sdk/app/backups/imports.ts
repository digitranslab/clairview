import { db as dbCore, encryption, objectStore } from "@clairview/backend-core"
import {
  Database,
  Row,
  Automation,
...
import { clairviewTempDir } from "../../../utilities/clairviewDir"
import {
  DB_EXPORT_FILE,
...
function rewriteAttachmentUrl(appId: string, attachment: RowAttachment) {
  // URL looks like: /prod-clair-app-assets/appId/attachments/file.csv
...
export async function untarFile(file: { path: string }) {
  const tmpPath = join(clairviewTempDir(), uuid())
