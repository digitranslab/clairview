...
import { sql } from "@clairview/backend-core"
...
class OracleIntegration extends Sql implements DatasourcePlus {
      ...
      const options: ExecuteOptions = {
        ...
        fetchTypeHandler: function (metaData) {
          if (metaData.dbType === oracledb.CLOB) {
            ...
            // When we create a new table in OracleDB from Clairview, bigints get
            // created as NUMBER(20,0). Clairview expects bigints to be returned
  ...
  private getConnection = async (): Promise<Connection> => {
    ...
    // Clairview server, this is because several column types (e.g. time-only
