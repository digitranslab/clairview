import { Datasource, SourceName } from "@clairview/types"
import { GenericContainer, Wait } from "testcontainers"
import { generator, testContainerUtils } from "@clairview/backend-core/tests"
...
export async function getDatasource(): Promise<Datasource> {
  ...
  if (!ports) {
    ...
    let image = "clairview/oracle-database:23.2-slim-faststart"
    if (process.arch.startsWith("arm")) {
      // there isn't an ARM compatible 23.2 build
      image = "clairview/oracle-database:19.3.0-ee-slim-faststart"
    }
