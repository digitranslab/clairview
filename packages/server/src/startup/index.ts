...
export async function startup(
  ...
  printFeatures()
  if (env.CLAIRVIEW_ENVIRONMENT) {
    console.log(`service running environment: "${env.CLAIRVIEW_ENVIRONMENT}"`)
  }
  STARTUP_RAN = true
  if (app && server && !env.CLUSTER_MODE) {
    console.log(`Clairview running on ${JSON.stringify(server.address())}`)
