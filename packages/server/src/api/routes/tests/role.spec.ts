import { roles, events, db as dbCore } from "@clairview/backend-core"
import * as setup from "./utilities"
import { PermissionLevel, BuiltinPermissionID } from "@clairview/types"
  ...
  describe("accessible", () => {
    ...
    it("should not fetch higher level accessible roles when a custom role header is provided", async () => {
      ...
      await config.withHeaders(
        { "x-clairview-role": customRoleName },
        async () => {
          const res = await config.api.roles.accessible({
            status: 200,
          })
    ...
    it("should fetch preview role correctly even without basic specified", async () => {
      ...
      await config.withHeaders({ "x-clairview-role": role.name }, async () => {
        const res = await config.api.roles.accessible({
          status: 200,
        })
