jest.mock("@clairview/backend-core", () => {
  const core = jest.requireActual("@clairview/backend-core")
  return {
    ...core,
    objectStore: {
      ...core.objectStore,
...
import { events, objectStore } from "@clairview/backend-core"
import * as setup from "./utilities"
import nock from "nock"
import { PluginSource } from "@clairview/types"
  ...
  describe("github", () => {
    beforeEach(async () => {
      nock("https://api.github.com")
        .get("/repos/my-repo/clairview-comment-box")
        .reply(200, {
          name: "clairview-comment-box",
          releases_url:
            "https://api.github.com/repos/my-repo/clairview-comment-box{/id}",
        })
        .get("/repos/my-repo/clairview-comment-box/latest")
        .reply(200, {
          assets: [
            {
              content_type: "application/gzip",
              browser_download_url:
                "https://github.com/my-repo/clairview-comment-box/releases/download/v1.0.2/comment-box-1.0.2.tar.gz",
            },
          ],
        })
      ...
      nock("https://github.com")
        .get(
          "/my-repo/clairview-comment-box/releases/download/v1.0.2/comment-box-1.0.2.tar.gz"
        )
        .replyWithFile(
          200,
    ...
    it("should be able to create a plugin from github", async () => {
      const { plugin } = await config.api.plugin.create({
        ...
        url: "https://github.com/my-repo/clairview-comment-box.git",
        githubToken: "token",
      })
    ...
    it("should fail if the url is not from github", async () => {
      await config.api.plugin.create(
        {
          source: PluginSource.GITHUB,
          url: "https://notgithub.com/my-repo/clairview-comment-box",
          githubToken: "token",
        },
        {
          status: 400,
  ...
  describe("npm", () => {
    it("should be able to create a plugin from npm", async () => {
      nock("https://registry.npmjs.org")
        .get("/clairview-component")
        .reply(200, {
          name: "clairview-component",
          "dist-tags": {
            latest: "1.0.0",
          },
          versions: {
            "1.0.0": {
              dist: {
                tarball:
                  "https://registry.npmjs.org/clairview-component/-/clairview-component-1.0.1.tgz",
              },
            },
          },
        })
        .get("/clairview-component/-/clairview-component-1.0.1.tgz")
        .replyWithFile(
          200,
          "src/api/routes/tests/data/clairview-component-1.0.1.tgz"
        )

      const { plugin } = await config.api.plugin.create({
        source: PluginSource.NPM,
        url: "https://www.npmjs.com/package/clairview-component",
      })
      expect(plugin._id).toEqual("plg_clairview-component")
