...
import { App, Plugin, UserCtx } from "@clairview/types"
import { db as dbCore, context, tenancy } from "@clairview/backend-core"
  ...
  try {
    ...
    for (let { manifest, library } of componentManifests) {
      for (let key of Object.keys(manifest)) {
        // These keys are not components, and should not be preprended with the `@clairview/` prefix
