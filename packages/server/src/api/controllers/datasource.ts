...
import { context, db as dbCore, events } from "@clairview/backend-core"
import {
...
export async function update(
  ...
  const isClairviewSource =
    baseDatasource.type === dbCore.CLAIRVIEW_DATASOURCE_TYPE

  const dataSourceBody: Datasource = isClairviewSource
    ? {
        name: ctx.request.body?.name,
        type: dbCore.CLAIRVIEW_DATASOURCE_TYPE,
        source: SourceName.CLAIRVIEW,
      }
    : ctx.request.body
  ...
  if (datasource.source && !isClairviewSource) {
...
export async function destroy(ctx: UserCtx) {
  ...
  if (datasource.type === dbCore.CLAIRVIEW_DATASOURCE_TYPE) {
