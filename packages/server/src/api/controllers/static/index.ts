import { InvalidFileExtensions } from "@clairview/shared-core"
import AppComponent from "./templates/ClairviewApp.svelte"
import { join } from "../../../utilities/centralPath"
import * as uuid from "uuid"
import { ObjectStoreBuckets } from "../../../constants"
import { processString } from "@clairview/string-templates"
import {
  loadHandlebarsFile,
  NODE_MODULES_PATH,
  shouldServeLocally,
...
export const toggleBetaUiFeature = async function (ctx: Ctx) {
  ...
  await objectStore.downloadTarballDirect(
    "https://cdn.clair.live/beta:design_ui/new_ui.tar.gz",
    builderPath
  )
...
export const serveApp = async function (ctx: UserCtx) {
  ...
  const bbHeaderEmbed =
    ctx.request.get("x-clairview-embed")?.toLowerCase() === "true"
  ...
  try {
    ...
    if (!env.isJest()) {
      ...
      const { head, html, css } = AppComponent.render({
        ...
        metaTitle:
          branding?.metaTitle || `${appInfo.name} - built with Clairview`,
...
export const serveClientLibrary = async function (ctx: Ctx) {
  ...
  let rootPath = join(NODE_MODULES_PATH, "@clairview", "client", "dist")
  if (!appId) {
  ...
  if (!serveLocally) {
    ...
    const tsPath = join(require.resolve("@clairview/client"), "..")
    return send(ctx, "clairview-client.js", {
