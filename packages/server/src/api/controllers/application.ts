...
import { DefaultAppTheme, sdk as sharedCoreSDK } from "@clairview/shared-core"
...
async function createInstance(appId: string, template: AppTemplate) {
  ...
  if (template && template.useTemplate) {
    const opts = {
      ...
      updateAttachmentColumns: !template.key, // preserve attachments when using Clairview templates
    }
...
export async function fetchAppDefinition(
  ...
  ctx.body = {
    layouts,
    screens,
    libraries: ["@clairview/standard-components"],
  }
}
