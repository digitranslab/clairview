...
export async function generateMetadata({
  ...
  const basePath = '/src/content/clairview-ui';
  const mainPost = await fetchArticleFromSlug(params.folder, basePath);
  return {
    title: 'Clairview - ' + formattedSlug,
    description: mainPost?.itemInfo?.info,
  };
}

export default async function ClairviewUISlug({
  params,
}: {
  params: { folder: string };
}) {
  const filePath = `src/content/clairview-ui/${params.folder}/`;
