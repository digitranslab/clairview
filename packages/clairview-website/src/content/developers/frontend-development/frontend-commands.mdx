...
```bash
npx nx run clairview-front:graphql:generate --configuration=metadata
```
OR
```bash
npx nx run clairview-front:graphql:generate
```

### Lint

```bash
npx nx run clairview-front:lint # pass --fix to fix lint errors
```

### Test

```bash
npx nx run clairview-front:test # run jest tests
npx nx run clairview-front:storybook:serve:dev # run storybook
npx nx run clairview-front:storybook:test # run tests # (needs yarn storybook:serve:dev to be running)
npx nx run clairview-front:storybook:coverage # (needs yarn storybook:serve:dev to be running)
```
