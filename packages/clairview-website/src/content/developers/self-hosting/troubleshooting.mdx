...
During Clairview installation, you need to provision your postgres database with the right schemas, extensions, and users.
...
#### Cannot find module 'clairview-emails' or its corresponding type declarations.

You have to build the package `clairview-emails` before running the initialization of the database with `npx nx run clairview-emails:build`

#### Missing clairview-x package

Make sure to run yarn in the root directory and then run `npx nx server:dev clairview-server`. If this still doesn't work try building the missing package manually.
...
#### While running `npx nx start` or `npx nx start clairview-front`, Out of memory error is thrown

In `packages/clairview-front/.env` uncomment `VITE_DISABLE_TYPESCRIPT_CHECKER=true` and `VITE_DISABLE_ESLINT_CHECKER=true` to disable background checks thus reducing amount of needed RAM.

**If it does not work:**  
Run only the services you need, instead of `npx nx start`. For instance, if you work on the server, run only `npx nx worker clairview-server`

**If it does not work:**  
Investigate which processes are taking you most of your machine RAM. At Clairview, we noticed that some VScode extensions were taking a lot of RAM so we temporarily disable them.
...
```
npx nx worker clairview-server
```
...
1. Run the following commands:
    ```bash
    docker exec -it clairview-server-1 yarn
    docker exec -it clairview-server-1 npx nx database:reset  --configuration=no-seed
    ```
...
If you're running Clairview behind a reverse proxy and experiencing connection issues:
...
2. **Check Reverse Proxy Settings:**
    ...
    - Confirm that your reverse proxy is correctly forwarding requests to the Clairview server.
    - Ensure headers like `X-Forwarded-For` and `X-Forwarded-Proto` are properly set.

3. **Restart Services:**

    After making changes, restart both the reverse proxy and Clairview containers.
...
- Community Support:
    ...
    Reach out to the [Clairview community](https://github.com/digitranslab/clairview/issues) or [support channels](https://discord.gg/cx5n4Jzs57) for assistance.
