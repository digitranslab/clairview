...
info: The guide for contributors (or curious developers) who want to run Clairview locally (on laptop, PC...)
---
...
Before you can install and use Clairview, make sure you install the following on your computer:
...
```bash
git clone git@github.com:digitranslab/clairview.git
```
</ArticleTab>
<ArticleTab>
...
You can now access the database at [localhost:5432](localhost:5432), with user `clairview` and password `clairview` .

## Step 4: Set up a Redis Database (cache)
Clairview requires a redis cache to provide the best performance
...
Use environment variables or `.env` files to configure your project. More info [here](https://clairview.io/developers/section/self-hosting/setup)

Copy the `.env.example` files in `/front` and `/server`:
```bash
cp ./packages/clairview-front/.env.example ./packages/clairview-front/.env
cp ./packages/clairview-server/.env.example ./packages/clairview-server/.env
```

## Step 6: Installing dependencies
To build Clairview server and seed some data into your database, run the following command:
```bash
yarn
```
...
```bash
npx nx database:reset clairview-server
```

Start the server, the worker and the frontend services:
```bash
npx nx start clairview-server
npx nx worker clairview-server
npx nx start clairview-front
```
...
## Step 8: Use Clairview

**Frontend**  

Clairview's frontend will be running at [http://localhost:3001](http://localhost:3001).
You can log in using the default demo account: `tim@apple.dev` (password: `Applecar2025`)

**Backend**  

- Clairview's server will be up and running at [http://localhost:3000](http://localhost:3000)
