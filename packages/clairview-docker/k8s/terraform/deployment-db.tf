resource "kubernetes_deployment" "clairviewcrm_db" {
  metadata {
  ...
  spec {
    replicas = var.clairviewcrm_db_replicas
    selector {
      match_labels = {
        app = "${var.clairviewcrm_app_name}-db"
      }
    ...
    template {
      ...
      spec {
        container {
          image = var.clairviewcrm_db_image
          ...
          env {
            ...
            value = var.clairviewcrm_pgdb_admin_password
          }
          env {
            name  = "BITNAMI_DEBUG"
            value = true
