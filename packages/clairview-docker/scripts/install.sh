...
version=${VERSION:-$(curl -s https://api.github.com/repos/digitranslab/clairview/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
# Create a directory named clairview
echo "📁 Creating directory '$dir_name'"
mkdir -p "$dir_name" && cd "$dir_name" || { echo "❌ Failed to create/access directory '$dir_name'"; exit 1; }

# Copy clairview/packages/clairview-docker/docker-compose.yml in it
echo -e "\t• Copying docker-compose.yml"
curl -sLo docker-compose.yml https://raw.githubusercontent.com/digitranslab/clairview/$branch/packages/clairview-docker/docker-compose.yml

# Copy clairview/packages/clairview-docker/.env.example to .env
echo -e "\t• Setting up .env file"
curl -sLo .env https://raw.githubusercontent.com/digitranslab/clairview/$branch/packages/clairview-docker/.env.example
...
if [ "$answer" = "n" ]; then
  ...
  pid=$!
  while [ ! $(docker inspect --format='{{.State.Health.Status}}' clairview-server-1) = "healthy" ]; do
    sleep 1
  done
  kill $pid
  echo ""
