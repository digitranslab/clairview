...
import { bucketTTLConfig, clairviewTempDir } from "./utils"
import { v4 } from "uuid"
...
export async function retrieveToTmp(bucketName: string, filepath: string) {
  ...
  const outputPath = join(clairviewTempDir(), v4())
  fs.writeFileSync(outputPath, data)
  return outputPath
}

export async function retrieveDirectory(bucketName: string, path: string) {
  let writePath = join(clairviewTempDir(), v4())
...
export async function downloadTarball(
  ...
  const tmpPath = join(clairviewTempDir(), path)
