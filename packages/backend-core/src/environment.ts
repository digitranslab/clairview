...
import { ServiceType } from "@clairview/types"
...
function getPackageJsonFields(): {
  ...
  try {
    ...
    return {
      VERSION:
        localVersion || process.env.CLAIRVIEW_VERSION || parsedContent.version,
      SERVICE_NAME: parsedContent.name,
    }
  } catch {
    // throwing an error here is confusing/causes backend-core to be hard to import
    return { VERSION: process.env.CLAIRVIEW_VERSION || "", SERVICE_NAME: "" }
  }
}
...
function isQA() {
  return environment.CLAIRVIEW_ENVIRONMENT === "QA"
}
...
const environment = {
  ...
  CLAIRVIEW_ENVIRONMENT: process.env.CLAIRVIEW_ENVIRONMENT,
  ...
  ACCOUNT_PORTAL_URL:
    process.env.ACCOUNT_PORTAL_URL || "https://account.clairview.app",
  ...
  SERVICE: process.env.SERVICE || "clairview",
