...
import { clairviewTempDir } from "../objectStore"

const logsFileName = `clairview.log`
const clairviewLogsHistoryFileName = "clairview-logs-history.txt"

const logsPath = path.join(clairviewTempDir(), "systemlogs")
...
export function localFileDestination() {
  ...
  const outFile = rfs.createStream(logsFileName, {
    ...
    immutable: true,
    history: clairviewLogsHistoryFileName,
...
export function getLogReadStream() {
  const streams = []
  const historyFile = getFullPath(clairviewLogsHistoryFileName)
