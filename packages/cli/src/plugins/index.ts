...
import fs from "fs"
import { PluginType, PLUGIN_TYPE_ARR } from "@clairview/types"
import { plugins } from "@clairview/backend-core"
...
async function init(opts: PluginOpts | PluginType) {
  ...
  const name = await questions.string("Name", `clairview-${type}`)
...
export default new Command(`${CommandWord.PLUGIN}`)
  .addHelp(
    "Custom plugins for Clairview, init, build and verify your components and datasources with this tool."
  )
  .addSubOption(
    "--init [type]",
