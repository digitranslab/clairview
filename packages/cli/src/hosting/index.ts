...
export default new Command(`${CommandWord.HOSTING}`)
  .addHelp("Controls self hosting on the Clairview platform.")
  .addSubOption(
    "--init [type]",
  ...
  .addSubOption(
    "--update",
    "Update the Clairview images to the latest version.",
    update
  )
  .addSubOption(
    "--watch-plugin-dir [directory]",
    "Add plugin directory watching to a Clairview install.",
    watchPlugins
  )
  .addSubOption(
    "--gen-user",
