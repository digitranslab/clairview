...
const COMPOSE_URL =
  "https://raw.githubusercontent.com/Clairview/clairview/master/hosting/docker-compose.yaml"
...
export async function checkDockerConfigured() {
  const error =
    "docker has not been installed, please follow instructions at: https://docs.clairview.com/docs/docker-compose"
...
export function updateDockerComposeService(
  ...
  if (!service) {
    console.log(
      error(
        "Unable to locate service within compose file, is it a valid Clairview configuration?"
      )
    )
    return
  }
