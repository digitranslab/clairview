...
import loadSpectrumIcons from "@clairview/bbui/spectrum-icons-rollup.js"
...
const loadClairview = async () => {
  // Update builder store with any builder flags
  builderStore.set({
    ...get(builderStore),
    inBuilder: !!window["##CLAIRVIEW_IN_BUILDER##"],
    layout: window["##CLAIRVIEW_PREVIEW_LAYOUT##"],
    screen: window["##CLAIRVIEW_PREVIEW_SCREEN##"],
    selectedComponentId: window["##CLAIRVIEW_SELECTED_COMPONENT_ID##"],
    previewId: window["##CLAIRVIEW_PREVIEW_ID##"],
    theme: window["##CLAIRVIEW_PREVIEW_THEME##"],
    customTheme: window["##CLAIRVIEW_PREVIEW_CUSTOM_THEME##"],
    previewDevice: window["##CLAIRVIEW_PREVIEW_DEVICE##"],
    navigation: window["##CLAIRVIEW_PREVIEW_NAVIGATION##"],
    hiddenComponentIds: window["##CLAIRVIEW_HIDDEN_COMPONENT_IDS##"],
    usedPlugins: window["##CLAIRVIEW_USED_PLUGINS##"],
    location: window["##CLAIRVIEW_LOCATION##"],
    snippets: window["##CLAIRVIEW_SNIPPETS##"],
  })

  // Set app ID - this window flag is set by both the preview and the real
  // server rendered app HTML
  appStore.actions.setAppId(window["##CLAIRVIEW_APP_ID##"])

  // Set the flag used to determine if the app is being loaded via an iframe
  appStore.actions.setAppEmbedded(
    window["##CLAIRVIEW_APP_EMBEDDED##"] === "true"
  )
  ...
  window.handleBuilderRuntimeEvent = (type, data) => {
    if (!window["##CLAIRVIEW_IN_BUILDER##"]) {
      return
    }
  ...
  if (window["##CLAIRVIEW_CUSTOM_COMPONENTS##"]) {
    window["##CLAIRVIEW_CUSTOM_COMPONENTS##"].forEach(component => {
...
window.loadClairview = loadClairview
