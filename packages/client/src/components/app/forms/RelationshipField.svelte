<script>
  import { CoreSelect, CoreMultiselect } from "@clairview/bbui"
  import { FieldType } from "@clairview/types"
  import { fetchData, Utils } from "@clairview/frontend-core"
  ...
  $: {
    if (primaryDisplay && fieldState && !optionsObj) {
      ...
      optionsObj = valueAsSafeArray.reduce((accumulator, value) => {
        ...
        // https://linear.app/clairview/issue/CLAIR-7577/refactor-the-relationshipfield-component-to-have-better-support-for
        if (!value._id) {
          return accumulator
        }
