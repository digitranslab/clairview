<!--
  ...
  required maintenance to be done on the Clairview installation. We only use this
  if we detect that the Clairview installation is in a state where the vast
...
<script>
  ...
  import { Heading, Body, Layout } from "@clairview/bbui"
...
<div class="content">
  {#each maintenanceList as maintenance}
    {#if maintenance.type === MaintenanceType.SQS_MISSING}
      <Layout>
        <Heading>Clairview installation requires maintenance</Heading>
        <Body>
          The administrator of this Clairview installation needs to take actions
