...
export const ClairviewPrefix = "@clairview/standard-components/"
...
const createComponentStore = () => {
  ...
  const getComponentDefinition = type => {
    ...
    if (type === ScreenslotType) {
      type = `${ClairviewPrefix}${type}`
    }

    // Handle built-in components
    if (type.startsWith(ClairviewPrefix)) {
      type = type.replace(ClairviewPrefix, "")
