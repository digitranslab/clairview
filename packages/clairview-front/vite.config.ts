...
export default defineConfig(({ command, mode }) => {
  ...
  if (VITE_DISABLE_ESLINT_CHECKER !== 'true') {
    checkers['eslint'] = {
      lintCommand:
        'cd ../.. && eslint packages/clairview-front --report-unused-disable-directives --max-warnings 0 --config .eslintrc.cjs',
    };
  }

  return {
    root: __dirname,
    cacheDir: '../../node_modules/.vite/packages/clairview-front',
    ...
    plugins: [
      ...
      tsconfigPaths({
        projects: ['tsconfig.json', '../clairview-ui/tsconfig.json'],
      }),
      svgr(),
      checker(checkers),
