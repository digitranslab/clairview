...
function focusInput() {
  if (document && typeof document !== 'undefined') {
    const inputEl =
      (document.querySelector('.cv-cell input') as HTMLInputElement) ||
      (document.querySelector('.cv-cell textarea') as HTMLTextAreaElement) ||
      (document.querySelector('.cv-cell [tabindex="0"]') as HTMLElement)
...
const showSubmitConfirmModal = async () => {
  ...
  setTimeout(() => {
    ...
    document.querySelector('.cv-survery-form__confirmation_modal div[tabindex="0"]')?.focus()
    document.querySelector('.cv-survey-form-btn-submit.cv-button')?.focus()
  }, 50)
}
...
onKeyStroke(['Enter'], async (e) => {
  ...
  if (!isStarted.value && !submitted.value) {
    ...
    if (isLast.value) {
      ...
      if (activeElement?.classList && activeElement.classList.contains('cv-survey-form__btn-next')) return
  ...
  <div class="h-full">
    <div class="survey md:p-0 w-full h-full flex flex-col max-w-[max(33%,688px)] mx-auto mb-4rem lg:mb-10rem">
      <div v-if="sharedFormView" class="my-auto z-2">
        <template v-if="!isStarted || submitted">
          ...
          <div class="rounded-3xl border-1 border-gray-200 p-6 lg:p-12 bg-white">
            <h1 class="text-2xl font-bold text-gray-900 mb-4" data-testid="cv-survey-form__heading">
              {{ sharedFormView.heading }}
            </h1>

            <div v-if="submitted" class="flex flex-col justify-center items-center text-center">
              <a-alert
                class="cv-survey-form__success-msg !p-4 !rounded-lg text-left w-full !bg-white !border-gray-200 !items-start"
                type="success"
                data-testid="cv-survey-form__success-msg"
                outlined
                show-icon
              >
                <template #message>
              ...
              >
                ...
                <div class="flex-1 self-end flex justify-end">
                  <NcButton
                    ...
                    data-testid="cv-survey-form__btn-submit-another-form"
                    @click="resetForm"
                  >
            ...
            <template v-else-if="!isStarted">
              <div v-if="sharedFormView.subheading?.trim()">
                <LazyCellRichText
                  ...
                  read-only
                  sync-value-change
                  data-testid="cv-survey-form__sub-heading"
                />
              </div>
              ...
              <div class="flex justify-end mt-12">
                <div class="flex items-center gap-3">
                  ...
                  <NcButton
                    :size="isMobileMode ? 'medium' : 'small'"
                    data-testid="cv-survey-form__fill-form-btn"
                    @click="onStart()"
                  >
                    Fill Form
                  </NcButton>
        ...
        <template v-if="isStarted && !submitted">
          <Transition :name="`slide-${transitionName}`" :duration="transitionDuration" mode="out-in">
            <a-form :model="formState">
              ...
              >
                <div class="select-none text-gray-500 mb-4 md:mb-2" data-testid="cv-survey-form__footer">
                  {{ index + 1 }} / {{ formColumns?.length }}
                </div>

                <div v-if="field" class="flex flex-col gap-2">
                  <div class="cv-form-column-label text-sm font-semibold text-gray-800" data-testid="cv-form-column-label">
                    <span>
                  ...
                  <div
                    ...
                    class="cv-form-column-description text-gray-500 text-sm"
                    data-testid="cv-survey-form__field-description"
                  >
                    <LazyCellRichText
                  ...
                  <NcTooltip :disabled="!field?.read_only">
                    ...
                    >
                      <SmartsheetDivDataCell class="relative cv-form-data-cell" @click.stop="handleFocus">
                        <LazySmartsheetVirtualCell
                          v-if="isVirtualCol(field)"
                          v-model="formState[field.title]"
                          class="mt-0 cv-input h-auto"
                          :class="{
                            readonly: field?.read_only,
                          }"
                          :row="{ row: {}, oldRow: {}, rowMeta: {} }"
                          :data-testid="`cv-survey-form__input-${field.title.replaceAll(' ', '')}`"
                          :column="field"
                  ...
                  <div class="flex-1 flex justify-end">
                    <div v-if="isLast">
                      <NcButton
                        ...
                        data-testid="cv-survey-form__btn-submit-confirm"
                    ...
                    <div v-else class="flex items-center gap-3">
                      ...
                      <NcButton
                        :size="isMobileMode ? 'medium' : 'small'"
                        data-testid="cv-survey-form__btn-next"
                        class="cv-survey-form__btn-next"
                        :class="[
      ...
      <div class="lg:(absolute bottom-0 left-0 right-0 px-4 pb-4) lg:px-10 lg:pb-10 pointer-events-none">
        <div class="flex justify-end items-center gap-4 cv-survey-form-branding">
          ...
          <div v-if="isStarted && !submitted" class="flex items-center gap-3">
            <NcButton
              type="secondary"
              :size="isMobileMode ? 'medium' : 'small'"
              data-testid="cv-survey-form__icon-prev"
              :disabled="isFirst"
              @click="goPrevious()"
            >
            ...
            <NcButton
              :size="isMobileMode ? 'medium' : 'small'"
              type="secondary"
              data-testid="cv-survey-form__icon-next"
    ...
    <NcModal v-model:visible="dialogShow" size="small" class="cv-survery-form__confirmation_modal">
      <div>
        ...
        <div class="flex justify-end mt-7 gap-x-2">
          ...
          <NcButton
            type="primary"
            :size="isMobileMode ? 'medium' : 'small'"
            data-testid="cv-survey-form__btn-submit"
            class="cv-survey-form-btn-submit"
            @click="submit"
          >
            {{ $t('general.submit') }}
          </NcButton>
...
.survey {
  ...
  .cv-form-column-label__checkbox {
    @apply flex items-center justify-center gap-2 text-left;
  }

  .cv-form-data-cell.cv-data-cell {
