...
import { SupportedExportCharset, type ViewType, ViewTypes, charsetOptions, csvColumnSeparatorOptions } from 'clairview-sdk'
...
<template>
  <ExtensionsExtensionWrapper>
    ...
    <div
      ...
      class="data-exporter"
      :class="{
        'bg-cv-bg-gray-extralight': fullscreen,
      }"
    >
      <div
        v-if="!fullscreen"
      ...
      >
        <div class="flex items-center justify-between gap-2.5 flex-wrap">
          <div
            class="cv-data-exporter-select-wrapper flex-1 flex items-center border-1 border-cv-border-gray-medium rounded-lg relative shadow-default"
            :class="{
          ...
          >
            ...
            >
              <NcSelect
                ...
                class="cv-data-exporter-table-select cv-select-shadow"
            ...
            >
              <NcSelect
                ...
                :disabled="isExporting"
                class="cv-data-exporter-view-select cv-select-shadow"
                dropdown-class-name="w-[250px]"
                :filter-option="filterOption"
                show-search
      ...
      >
        <div
          v-if="fullscreen"
          class="w-[320px] border-r-1 border-r-cv-border-gray-medium bg-white p-4 pt-t flex flex-col gap-5 cv-scrollbar-thin"
        >
          <div class="text-base font-bold text-cv-content-gray-extreme">Settings</div>
          <div class="flex flex-col gap-2">
            <div class="text-cv-content-gray font-medium">Table</div>
            <a-form-item class="!my-0">
              <NcSelect
                v-model:value="exportPayload.tableId"
                placeholder="-select table-"
                :disabled="isExporting"
                class="cv-data-exporter-table-select-sidebar cv-select-shadow"
                :filter-option="filterOption"
                dropdown-class-name="w-[250px]"
                show-search
                @change="onTableSelect"
          ...
          <div class="flex flex-col gap-2">
            <div class="text-cv-content-gray font-medium">View</div>
            <a-form-item class="!my-0 min-w-1/2">
              <NcSelect
                v-model:value="exportPayload.viewId"
                placeholder="-select view-"
                :disabled="isExporting"
                class="cv-data-exporter-view-select-sidebar cv-select-shadow"
                dropdown-class-name="w-[250px]"
                :filter-option="filterOption"
                show-search
                placement="bottomRight"
          ...
          <div class="flex flex-col gap-2">
            ...
            <a-form-item class="!my-0 flex-1">
              <NcSelect
                ...
                :disabled="isExporting"
                class="cv-data-exporter-separator cv-select-shadow"
          ...
          <div class="flex flex-col gap-2">
            ...
            <a-form-item class="!my-0 flex-1">
              <NcSelect
                ...
                class="cv-data-exporter-encoding cv-select-shadow"
        ...
        <div class="flex flex-col flex-1 cv-scrollbar-thin">
            ...
            <template v-for="exp of exportedFiles">
              <div
                ...
                :class="{
                  ...
                  'bg-cv-bg-red-light': exp.status !== JobStatus.COMPLETED,
                }"
              >
                <div
                ...
                >
                  ...
                  <div class="flex-1 max-w-[calc(100%_-_28px)] flex flex-col gap-1">
                    ...
                    <div v-if="exp.result.timestamp" name="error" class="text-small leading-[18px] text-cv-content-gray-muted">
                ...
                <div v-if="exp.result.isNew" class="flex h-7 flex items-center">
                  <NcBadge color="green" :border="false" class="!bg-cv-bg-green-light !text-cv-content-green-dark">{{
                    $t('general.new')
                  }}</NcBadge>
                </div>
...
.data-exporter {
  ...
  .cv-data-exporter-select-wrapper {
    &:not(:focus-within) {
      &::after {
        @apply absolute left-1/2 h-full content-[''] border-r-1 border-cv-border-gray-medium;
      }
    }
  }

  :deep(.cv-data-exporter-table-select.ant-select) {
    &.ant-select-focused {
      .ant-select-selector {
  ...
  :deep(.cv-data-exporter-view-select.ant-select) {
    &.ant-select-focused {
      .ant-select-selector {
  ...
  :deep(.cv-data-exporter-separator.ant-select),
  :deep(.cv-data-exporter-encoding.ant-select) {
    .ant-select-selector {
...
<style lang="scss">
.cv-cv-data-exporter .extension-content {
  @apply !p-0;
}
</style>
