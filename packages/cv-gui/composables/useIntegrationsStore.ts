...
import type { FormDefinition, IntegrationType, PaginatedType } from 'clairview-sdk'
import { ClientType, IntegrationsType, SyncDataType } from 'clairview-sdk'
...
const [useProvideIntegrationViewStore, _useIntegrationStore] = useInjectionState(() => {
  ...
  const deleteIntegration = async (integration: IntegrationType, force = false) => {
    ...
    try {
      ...
      if (error.error === CvErrorType.INTEGRATION_NOT_FOUND) {
