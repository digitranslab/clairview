...
import type { RoleLabels } from 'clairview-sdk'
import { RoleDescriptions } from 'clairview-sdk'
...
<template>
  <div
    ref="dropdownRef"
    size="lg"
    class="cv-roles-selector relative flex items-center gap-3"
  ...
  >
    ...
    <a-select
      ...
      dropdown-class-name="!rounded-lg !h-fit max-w-[350px] cv-role-selector-dropdown"
    ...
    >
      <a-select-option v-for="rl in props.disabledRoles || []" :key="rl" :value="rl" disabled>
        <div
          ...
          class="flex flex-col cv-role-select-dropdown gap-1"
        >
      ...
      <a-select-option v-for="rl in props.roles" :key="rl" v-e="['c:workspace:settings:user-role-change']" :value="rl">
        ...
        >
          <div class="flex items-center justify-between">
            <RolesBadge :border="false" :class="`cv-role-select-${rl}`" :inherit="inheritRef === rl" :role="rl" />
            <GeneralIcon v-if="rl === roleRef" icon="check" class="text-primary" />
          </div>
          <div v-if="descriptionRef" class="text-gray-500 text-xs">{{ RoleDescriptions[rl] }}</div>
        </div>
