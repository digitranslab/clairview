...
import { RelationTypes, isLinksOrLTAR, isSystemColumn } from 'clairview-sdk'
...
<template>
  <div class="cv-modal-child-list h-full w-full" :class="{ active: vModel }" @keydown.enter.stop>
    <div class="flex flex-col h-full">
      <div class="cv-dropdown-link-record-header bg-gray-100 py-2 rounded-t-xl flex justify-between pl-3 pr-2 gap-2">
        <div v-if="!isForm" class="flex-1 cv-dropdown-link-record-search-wrapper flex items-center py-0.5 rounded-md">
          <a-input
            ref="filterQueryRef"
          ...
          >
            <template #prefix>
              <GeneralIcon icon="search" class="cv-search-icon mr-2 h-4 w-4 text-gray-500" />
            </template>
          </a-input>
        </div>
        <div v-else>&nbsp;</div>
        <LazyVirtualCellComponentsHeader
          data-testid="cv-link-count-info"
      ...
      <div ref="childrenListRef" class="flex-1 overflow-auto cv-scrollbar-thin">
          ...
          <div class="cursor-pointer">
            ...
            <template v-else>
              <LazyVirtualCellComponentsListItem
                ...
                :row="refRow"
                data-testid="cv-child-list-item"
        ...
        <div v-else class="h-full flex flex-col gap-2 my-auto items-center justify-center text-gray-500 text-center">
          ...
          <NcButton
            ...
            v-e="['c:links:link']"
            data-testid="cv-child-list-button-link-to"
            size="small"
      ...
      <div class="cv-dropdown-link-record-footer bg-gray-100 p-2 rounded-b-xl flex items-center justify-between gap-3 min-h-11">
...
:deep(.cv-nested-list-item .ant-card-body) {
  @apply !px-1 !py-0;
}
...
<style lang="scss">
.cv-dropdown-link-record-search-wrapper {
  .cv-search-icon {
