...
<template>
  <LazyVirtualCellComponentsLinkRecordDropdown v-model:is-open="isOpen">
    <div class="cv-cell-field flex items-center gap-1 w-full chips-wrapper min-h-4">
      ...
      <div v-if="!isUnderLookup && !isSystemColumn(column)" class="flex justify-end gap-1 min-h-4 items-center">
        <GeneralIcon
          icon="maximize"
          class="select-none transform text-sm cv-action-icon text-gray-500/50 hover:text-gray-500 cv-arrow-expand h-3 w-3"
        ...
        <GeneralIcon
          ...
          icon="plus"
          class="select-none text-sm cv-action-icon text-gray-500/50 hover:text-gray-500 cv-plus"
...
.chips-wrapper:hover .cv-action-icon {
  @apply flex;
}
</style>
