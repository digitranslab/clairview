...
import type { ColumnType, LinkToAnotherRecordType, LookupType } from 'clairview-sdk'
...
<template>
  <div
    class="cv-cell-field h-full w-full cv-lookup-cell"
    tabindex="-1"
    :style="{
      height:
  ...
  >
    <div
      ...
      :class="{
        '!overflow-x-auto cv-cell-lookup-scroll cv-scrollbar-x-md !overflow-y-hidden': rowHeight === 1,
      }"
    >
      <template v-if="lookupColumn">
        ...
        <template v-else>
          ...
          <template v-else>
            <div
              class="max-h-full max-w-full w-full cv-cell-lookup-scroll"
              :class="{
                'cv-scrollbar-md ': rowHeight !== 1 && !isAttachment(lookupColumn),
              }"
            >
              <div
                ...
                :class="{
                  ...
                  '!overflow-x-auto cv-cell-lookup-scroll cv-scrollbar-x-md !overflow-y-hidden':
...
.cv-cell-lookup-scroll:hover {
  &::-webkit-scrollbar-thumb {
    @apply bg-gray-200;
  }
}

.cv-lookup-cell .cv-text-area-clamped-text {
  @apply !mr-1;
}

.cv-lookup-cell {
  &:has(.cv-cell-attachment) {
