...
import { ButtonActionsType, type ButtonType, type ColumnType } from 'clairview-sdk'
  ...
  >
    <NcTooltip :disabled="isFieldAiIntegrationAvailable || isPublic || !isUIAllowed('dataEdit')" class="flex">
      ...
      <component
        ...
        data-testid="cv-button-cell"
        :class="[
          `${column.colOptions.color ?? 'brand'} ${column.colOptions.theme ?? 'solid'}`,
          { '!w-6': !column.colOptions.label },
        ]"
        class="cv-cell-button cv-button-cell-link btn-cell-colors truncate flex items-center h-6"
        @click="triggerAction"
      >
        <GeneralLoader
