...
<template>
  <a-select
    ...
    :dropdown-class-name="`cv-select-dropdown ${dropdownClassOverride}`"
  ...
  >
    <template #suffixIcon>
      ...
      <GeneralIcon v-else class="text-gray-800 cv-select-expand-btn" :icon="suffixIcon" />
    </template>
...
.cv-select.ant-select-focused:not(.ant-select-disabled).ant-select:not(.ant-select-customize-input) .ant-select-selector {
  box-shadow: none;
  @apply border-brand-500;
}
.cv-select.ant-select.ant-select-disabled .cv-select-expand-btn {
  @apply text-gray-300;
}
.cv-select-dropdown {
