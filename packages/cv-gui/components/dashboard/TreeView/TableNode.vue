...
import { type BaseType, type TableType, ViewTypes } from 'clairview-sdk'
...
<template>
  <div
    class="cv-tree-item cv-table-node-wrapper text-sm select-none w-full"
    :data-order="table.order"
    :data-id="table.id"
    :data-table-id="table.id"
    :class="[`cv-base-tree-tbl cv-base-tree-tbl-${table.title?.replaceAll(' ', '')}`]"
    :data-active="openedTableId === table.id"
  >
    <div class="flex items-center py-0.5">
      <div
        v-e="['a:table:open']"
        class="flex-none flex-1 table-context flex items-center gap-1 h-full cv-tree-item-inner cv-sidebar-node pr-0.75 mb-0.25 rounded-md h-7 w-full group cursor-pointer hover:bg-gray-200"
        :class="{
        ...
        :data-testid="`cv-tbl-side-node-${table.title}`"
      ...
      >
        <div class="flex flex-row h-full items-center">
          <div class="flex w-auto" :data-testid="`tree-view-table-draggable-handle-${table.title}`">
            ...
            <div
              ...
              class="flex items-center cv-table-icon"
              :class="{
        ...
        <NcTooltip
          class="cv-tbl-title cv-sidebar-node-title text-ellipsis overflow-hidden select-none !flex-1"
          show-on-truncate-only
        >
          <template #title>{{ table.title }}</template>
          <span
            :class="isTableOpened ? 'text-brand-600 !font-medium' : 'text-gray-600'"
            :data-testid="`cv-tbl-title-${table.title}`"
        ...
        <div class="flex items-center">
          <NcTooltip v-if="table.description?.length" placement="bottom">
            ...
            <NcButton type="text" class="!hover:bg-transparent" size="xsmall">
              <GeneralIcon icon="info" class="!w-3.5 !h-3.5 cv-info-icon group-hover:opacity-100 text-gray-600 opacity-0" />
            </NcButton>
          </NcTooltip>
          ...
          <NcDropdown v-model:visible="isOptionsOpen" :trigger="['click']" @click.stop>
            <NcButton
              v-e="['c:table:option']"
              class="cv-sidebar-node-btn cv-tbl-context-menu text-gray-700 hover:text-gray-800"
              :class="{
                '!opacity-100 !inline-block': isOptionsOpen,
              }"
              data-testid="cv-sidebar-table-context-menu"
              type="text"
              size="xxsmall"
              @click.stop
            >
              ...
              <NcMenu class="!min-w-62.5" :data-testid="`sidebar-table-context-menu-list-${table.title}`">
                ...
                >
                  ...
                  <NcMenuItem class="!text-gray-700" @click="onDuplicate">
                    ...
                    <GeneralIcon v-else class="cv-view-copy-icon" icon="duplicate" />
                    {{
                  ...
                  <NcMenuItem
                    ...
                    class="!text-red-500 !hover:bg-red-50 cv-table-delete"
                    @click="deleteTable"
                  >
          ...
          <NcButton
            ...
            type="text"
            size="xxsmall"
            class="cv-sidebar-node-btn cv-sidebar-expand text-gray-700 hover:text-gray-800"
            :class="{
          ...
          >
            <GeneralIcon
              icon="chevronRight"
              class="cv-sidebar-source-node-btns cursor-pointer transform transition-transform duration-200 !text-current text-[20px]"
