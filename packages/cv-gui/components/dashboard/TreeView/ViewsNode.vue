...
import { type TableType, ViewLockType, type ViewType, type ViewTypes } from 'clairview-sdk'
...
<template>
  <a-menu-item
    class="cv-sidebar-node !min-h-7 !max-h-7 !my-0.5 select-none group text-gray-700 !flex !items-center hover:(!bg-gray-200 !text-gray-700) cursor-pointer"
    :class="{
    ...
    >
      ...
      <div v-e="['a:view:open', { view: vModel.type }]" class="text-sm flex items-center w-full gap-1" data-testid="view-item">
        ...
        >
          ...
          >
            <template #default>
              <GeneralViewIcon :meta="props.view" class="cv-view-icon w-4 !text-[16px]"></GeneralViewIcon>
            </template>
          </LazyGeneralEmojiPicker>
        </div>
        ...
        <NcTooltip
          v-else
          class="cv-sidebar-node-title text-ellipsis overflow-hidden select-none max-w-full"
          :class="{
        ...
        <template v-if="!isEditing && !isLocked">
          ...
          <NcDropdown v-model:visible="isDropdownOpen" overlay-class-name="!rounded-lg">
            <NcButton
              ...
              type="text"
              size="xxsmall"
              class="cv-sidebar-node-btn invisible !group-hover:(visible opacity-100) cv-sidebar-view-node-context-btn"
              :class="{
