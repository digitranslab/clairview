...
import type { TableType, ViewType } from 'clairview-sdk'
...
const scrollTableNode = () => {
  const activeTableDom = document.querySelector(`.cv-treeview [data-table-id="${_activeTable.value?.id}"]`)
...
<template>
  <div class="cv-treeview-container flex flex-col justify-between select-none">
    <div v-if="!isSharedBase" class="text-gray-500 font-medium pl-3.5 mb-1">{{ $t('objects.projects') }}</div>
    <div mode="inline" class="cv-treeview pb-0.5 flex-grow min-h-50 overflow-x-hidden">
