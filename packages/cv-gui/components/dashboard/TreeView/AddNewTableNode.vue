...
import type { BaseType } from 'clairview-sdk'
...
<template>
  ...
  >
    ...
    <span class="text-gray-500 group-hover:(text-primary/100) flex-1 cv-add-new-table">{{ $t('tooltip.addTable') }}</span>

    <a-dropdown v-if="!isSharedBase" :trigger="['click']" overlay-class-name="cv-dropdown-import-menu" @click.stop>
      <GeneralIcon
        icon="threeDotVertical"
        class="transition-opacity opacity-0 group-hover:opacity-100 cv-import-menu outline-0"
      />
        ...
        <a-menu class="!py-0 rounded text-sm">
          <a-menu-item-group class="!px-0 !mx-0">
            ...
            <a-menu-item key="table-magic" @click="openTableCreateMagicDialog(base.sources[sourceIndex].id)">
              <div class="color-transition cv-base-menu-item group">
          ...
          <a-menu-item v-if="isUIAllowed('importRequest', { roles: baseRole })" key="add-new-table" class="py-1 rounded-b">
            <a
              ...
              href="https://github.com/digitranslab/clairview/issues/2052"
              target="_blank"
              class="prose-sm hover:(!text-primary !opacity-100) color-transition cv-base-menu-item group after:(!rounded-b)"
