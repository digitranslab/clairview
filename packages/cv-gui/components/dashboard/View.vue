...
<template>
  <Splitpanes
    class="cv-sidebar-content-resizable-wrapper !w-screen h-full"
    :class="{
  ...
  >
    <Pane
      ...
      max-size="60%"
      class="cv-sidebar-splitpane !sm:max-w-140 relative !overflow-visible flex"
      :style="{
    ...
    >
      <div
        ref="wrapperRef"
        class="cv-sidebar-wrapper relative flex flex-col h-full justify-center !sm:(max-w-140) absolute overflow-visible"
        :class="{
          'mobile': isMobileMode,
...
<style lang="scss">
.cv-sidebar-wrapper.minimized-height > * {
  @apply h-4/5 pb-2 !(rounded-r-lg border-1 border-gray-200 shadow-lg);
  width: calc(100% + 4px);
}

.mobile.cv-sidebar-wrapper.minimized-height > * {
  @apply !h-full;
}
...
.cv-sidebar-wrapper.hide-sidebar {
  @apply !min-w-0;
...
.cv-sidebar-content-resizable-wrapper > {
...
.cv-sidebar-content-resizable-wrapper.hide-resize-bar > {
  .splitpanes__splitter {
