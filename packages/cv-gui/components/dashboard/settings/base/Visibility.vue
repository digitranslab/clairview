...
<template>
  <div data-testid="cv-settings-subtab-visibility" class="item-card flex flex-col w-full">
    <div class="text-cv-content-gray-emphasis font-semibold text-lg">
      {{ $t('labels.visibilityAndDataHandling') }}
    </div>

    <div class="text-cv-content-gray-subtle2 mt-2 leading-5">
      {{ $t('labels.visibilityConfigLabel') }}
    </div>

    <div class="flex flex-col border-1 rounded-lg mt-6 border-cv-border-gray-medium">
      <div class="flex w-full px-3 py-2 gap-2 flex-col">
        <div class="flex w-full gap-1 items-center">
          <NcSwitch v-model:checked="includeM2M" v-e="['c:themes:show-m2m-tables']" class="cv-settings-meta-misc-m2m">
            <span class="text-cv-content-gray font-semibold flex-1">
      ...
      <div class="flex w-full px-3 border-t-1 border-cv-border-gray-medium py-2 gap-2 flex-col">
        <div class="flex w-full gap-1 items-center">
          <NcSwitch v-model:checked="showNull" v-e="['c:settings:show-null']" class="cv-settings-show-null">
            <span class="text-cv-content-gray font-semibold flex-1">
      ...
      <div class="flex w-full px-3 py-2 border-t-1 border-cv-border-gray-medium gap-2 flex-col">
        <div class="flex w-full gap-1 items-center">
          <NcSwitch
            v-model:checked="showNullAndEmptyInFilter"
            v-e="['c:settings:show-null-and-empty-in-filter']"
            class="cv-settings-show-null-and-empty-in-filter"
