...
<template>
  <a-modal
    ...
    wrap-class-name="cv-modal-settings"
    @cancel="emits('update:modelValue', false)"
  >
    <div class="cv-modal-settings-content">
      <!--    Settings -->
      <div class="flex flex-row justify-between w-full items-center p-4 border-b-1 border-gray-200">
        ...
        <NcButton type="text" size="small" data-testid="settings-modal-close-button" @click="vModel = false">
          <component :is="iconMap.close" class="cursor-pointer cv-modal-close w-4" />
        </NcButton>
      </div>
        ...
        <a-layout-content class="h-full scrollbar-thumb-gray-500">
          ...
          >
            <component
              ...
              class="h-full"
              :data-testid="`cv-settings-subtab-${selectedSubTab.key}`"
              :base-id="baseId"
            />
            <component
              :is="selectedSubTab?.body"
