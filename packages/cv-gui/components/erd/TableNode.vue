...
import type { LinkToAnotherRecordType } from 'clairview-sdk'
import { isLinksOrLTAR, isVirtualCol } from 'clairview-sdk'
...
<template>
  ...
  >
    ...
    <div
      class="relative h-full max-w-76 flex flex-col justify-center bg-white min-w-16 min-h-8 rounded-lg cv-erd-table-node"
      :class="[
        `cv-erd-table-node-${table.table_name}`,
    ...
    >
      ...
      <div v-else-if="hasColumns" class="py-1 pr-0.5">
        <div
          ...
          :class="`cv-erd-table-node-${table.table_name}-column-${col.column_name}`"
        >
          <LazySmartsheetHeaderCell v-if="col" class="cv-erd-table-node-column" :column="col" :hide-menu="true" />
        </div>
        ...
        <div v-for="(col, index) in nonPkColumns" :key="col.title">
          ...
          >
            <div
              ...
              class="flex w-full"
              :class="`cv-erd-table-node-${table.table_name}-column-${col.title?.toLowerCase().replace(' ', '_')}`"
            >
              <Handle
              ...
              <LazySmartsheetHeaderVirtualCell class="cv-erd-table-node-column" :column="col" :hide-menu="true" />
            </div>
            ...
            <LazySmartsheetHeaderVirtualCell
              ...
              :column="col"
              :hide-menu="true"
              :class="`cv-erd-table-node-column cv-erd-table-node-${table.table_name}-column-${col.column_name}`"
            />
