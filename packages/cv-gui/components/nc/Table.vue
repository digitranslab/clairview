...
const headerRowClassName = computed(() => `cv-table-header-row ${props.headerRowClassName}`)

const bodyRowClassName = computed(() => `cv-table-row ${props.headerRowClassName}`)
...
<template>
  <div
    class="cv-table-container relative"
    :class="{
      bordered,
  ...
  >
    <div
      ref="tableWrapper"
      class="cv-table-wrapper max-h-full relative cv-scrollbar-thin !overflow-auto"
      :class="{
        ...
        <thead>
          ...
          >
            <th
              ...
              :key="index"
              class="cv-table-header-cell"
              :class="[
                `${headerCellClassName}`,
                `cv-table-header-cell-${index}`,
                {
              ...
              :data-test-id="`cv-table-header-cell-${col.name || col.key}`"
        ...
        >
          <tbody>
            <tr
              ...
              :class="[`${bodyRowClassName}`, `cv-table-row-${recordIndex}`]"
            ...
            >
              <td
                ...
                :data-test-id="`cv-table-cell-${col.name || col.key}`"
              >
                <div
    ...
    <div
      ...
      class="flex-none cv-table-empty flex items-center justify-center py-8 px-6 h-full"
      :style="{
