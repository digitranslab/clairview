...
import type { FormBuilderElement } from 'clairview-sdk'
...
<template>
  <div class="cv-form-builder cv-scrollbar-thin relative">
    <a-form ref="form" :model="formState" hide-required-mark layout="vertical" class="flex flex-col gap-4">
      <template v-for="category in Object.keys(formElementsCategorized)" :key="category">
        <div class="cv-form-section">
          <div v-if="category !== FORM_BUILDER_NON_CATEGORIZED" class="cv-form-section-title">{{ category }}</div>
            ...
            <div class="flex flex-wrap">
              <template v-for="field in formElementsCategorized[category]" :key="field.model">
                ...
                <a-form-item
                  ...
                  :style="`width:${+field.width || 100}%`"
                  :required="false"
                  :data-testid="`cv-form-input-${field.model}`"
                >
