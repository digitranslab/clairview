...
const newWrapClassName = computed(() => {
  let className = 'cv-modal-wrapper'
  if (_wrapClassName) {
...
if (stopEventPropogation.value) {
  watch(ncModalRef, () => {
    ...
    const modal = document.querySelector('.cv-modal-wrapper') as HTMLElement
...
<template>
  ...
  >
    <div
      ref="ncModalRef"
      class="flex flex-col cv-modal p-6 h-full"
    ...
    >
      <div
        ...
        class="flex pb-2 mb-2 cv-modal-header text-lg font-medium"
      >
        <slot name="header" />
      </div>
