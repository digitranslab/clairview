...
<template>
  <div class="cv-pagination flex flex-row items-center gap-x-0.25">
    <component :is="props.firstPageTooltip && mode === 'full' ? NcTooltip : 'div'" v-if="mode === 'full'">
      ...
      <NcButton
        ...
        @click="goToFirstPage"
      >
        <GeneralIcon icon="doubleLeftArrow" class="cv-pagination-icon" />
      </NcButton>
    </component>
    ...
    <component :is="props.prevPageTooltip && mode === 'full' ? NcTooltip : 'div'">
      ...
      >
        <GeneralIcon icon="arrowLeft" class="cv-pagination-icon" />
      </NcButton>
    </component>

    <div v-if="!isMobileMode" class="text-gray-500">
      <NcDropdown placement="top" overlay-class-name="!shadow-none">
        <NcButton class="!border-0 cv-select-page" type="secondary" size="xsmall">
          <div class="flex gap-1 items-center px-2">
            <span class="cv-current-page">
              {{ current }}
            </span>
            <GeneralIcon icon="arrowDown" class="text-gray-800 mt-0.5 cv-select-expand-btn" />
          </div>
        </NcButton>

        <template #overlay>
          <NcMenu class="cv-pagination-menu overflow-hidden">
    ...
    <component :is="props.nextPageTooltip && mode === 'full' ? NcTooltip : 'div'">
      ...
      >
        <GeneralIcon icon="arrowRight" class="cv-pagination-icon" />
      </NcButton>
    </component>
    ...
    <component :is="props.lastPageTooltip && mode === 'full' ? NcTooltip : 'div'" v-if="mode === 'full'">
      ...
      <NcButton
        ...
        @click="goToLastPage"
      >
        <GeneralIcon icon="doubleRightArrow" class="cv-pagination-icon" />
      </NcButton>
    </component>
