...
<template>
  <div class="flex flex-col">
    <div
      class="flex justify-between border-b-1 cv-date-week-header items-center box-border"
      :class="{
    ...
    >
      ...
      <div v-if="isCellInputField" class="text-gray-700 text-sm font-semibold">
        <span class="cv-month-picker-btn cursor-pointer hover:text-brand-500" @click="pickerType = 'month'">{{
          currentMonth
        }}</span>
        {{ ' ' }}
        <span class="cv-year-picker-btn cursor-pointer hover:text-brand-500" @click="pickerType = 'year'">{{ currentYear }}</span>
      </div>
      <span v-else class="text-gray-700 text-sm font-semibold">{{ currentMonthYear }}</span>

      <NcTooltip hide-on-click>
        <NcButton class="!border-0" data-testid="cv-calendar-next-btn" size="small" type="text" @click="paginate('next')">
    ...
    <div v-if="!hideCalendar" class="max-w-[320px] rounded-y-xl">
      ...
      <div
        class="grid gap-1 py-1 cv-date-week-grid-wrapper grid-cols-7"
        :class="{
      ...
      >
        <span
          ...
          :class="{
            ...
            'cv-selected-week !font-semibold z-1': isDateInSelectedWeek(date) && isWeekPicker,
            'border-none': isWeekPicker,
            'border-transparent': !isWeekPicker,
            'text-gray-400': !isDateInCurrentMonth(date),
            'cv-selected-week-start': isSameDate(date, selectedWeek?.start),
            'cv-selected-week-end': isSameDate(date, selectedWeek?.end),
            'rounded-md text-brand-500 !font-semibold cv-calendar-today': isSameDate(date, dayjs()) && isDateInCurrentMonth(date),
            'text-gray-500': date.get('day') === 0 || date.get('day') === 6,
            'cv-date-item font-weight-400': isCellInputField,
        ...
        >
          ...
          ></span>
          <span class="cv-date-item-inner z-2">
            {{ date.get('date') }}
          </span>
        </span>
      </div>
      <div v-if="isCellInputField" class="flex items-center justify-center px-2 pb-2 pt-1 gap-2">
        <NcButton class="cv-date-picker-now-btn !h-7" size="small" type="secondary" @click="handleSelectDate(dayjs())">
        ...
        <NcTooltip v-if="showCurrentDateOption" :disabled="showCurrentDateOption !== 'disabled'">
          ...
          <NcButton
            class="cv-date-picker-current-date-btn !h-7"
            size="small"
            type="secondary"
...
.cv-selected-week-start:before {
  @apply !border-l-1 !rounded-l-lg;
}

.cv-selected-week-end:before {
  width: 100%;
