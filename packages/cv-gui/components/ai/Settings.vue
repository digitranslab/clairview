    ...
    <template #overlay>
      <div class="flex flex-col w-[320px] overflow-hidden">
        <div class="flex items-center justify-between w-full p-3 bg-purple-50">
          <span class="text-sm font-bold text-cv-content-gray">Settings</span>
          <!-- Todo: add docs link  -->
          <a
            target="_blank"
            rel="noopener noreferrer"
            class="!no-underline !hover:(underline text-cv-content-purple-dark) text-cv-content-purple-dark"
            >{{ $t('title.docs') }}</a
          >
        </div>
        <div class="flex flex-col p-3 text-sm gap-3">
          ...
          <div class="flex items-center gap-2">
            <span class="text-cv-content-gray w-2/6">Integration</span>
            ...
            <a-form-item class="flex-1 !my-0">
              <NcSelect
                ...
                class="w-full cv-select-shadow cv-ai-input"
                size="middle"
                ...
                <a-select-option v-for="integration in aiIntegrations" :key="integration.id" :value="integration.id">
                  <div class="w-full flex gap-2 items-center">
                    ...
                    <component
                      ...
                      class="text-cv-content-purple-medium w-4 h-4"
                    />
                  </div>
                </a-select-option>
              </NcSelect>
          ...
          <!-- Model Select -->
          <div class="flex items-center gap-2">
            <span class="text-cv-content-gray w-2/6">Model</span>
            <div v-if="showTooltip" class="w-1/6 flex justify-end">
              <NcTooltip placement="top">
                <template #title>
          ...
          <!-- Randomness 
          <div class="flex items-center gap-2">
            <span class="text-cv-content-gray w-2/6">Randomness</span>
            <div v-if="showTooltip" class="w-1/6 flex justify-end">
              <NcTooltip placement="top">
                <template #title>
