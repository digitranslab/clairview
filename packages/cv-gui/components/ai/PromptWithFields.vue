...
function loadMentionFieldTagTooltip() {
  document.querySelectorAll('.cv-ai-prompt-with-fields .prompt-field-tag').forEach((el) => {
...
<style lang="scss">
.cv-ai-prompt-with-fields {
  @apply relative;

  .cv-prompt-with-field-suggestion-btn {
  ...
  .ProseMirror {
    @apply px-3 pb-3 pt-2 h-[120px] min-h-[120px] overflow-y-auto cv-scrollbar-thin outline-none border-1 border-gray-200 bg-white text-cv-content-gray rounded-lg !rounded-b-none transition-shadow ease-linear -mx-[1px] -mt-[1px];
    resize: vertical;
    min-width: 100%;
  ...
  .ProseMirror-focused {
    @apply !rounded-b-lg outline-none border-cv-fill-purple-medium shadow-selected-ai;
  }
