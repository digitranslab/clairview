...
import { stringToViewTypeMap } from 'clairview-sdk'
...
<template>
  <div class="h-full">
    <div class="flex items-center gap-2.5 px-4 py-2 border-b-1 border-purple-100">
      <div class="flex-1 flex items-center gap-3 text-cv-content-purple-dark">
    ...
    <div class="h-[calc(100%_-_49px)] flex">
      <div
        ...
        class="w-[480px] h-full relative flex flex-col cv-scrollbar-thin border-r-1 border-purple-100"
      >
        <!-- create base config panel -->
        <div class="flex-1 p-6 flex flex-col gap-6">
          <div class="text-sm font-bold text-cv-content-purple-dark">Tell us more about your usecase</div>
          <div class="flex flex-wrap gap-3 max-h-[188px] cv-scrollbar-thin overflow-visible">
            <!-- Predefined tags -->

            <template v-for="prompt of predefinedBasePrompts" :key="prompt.tag">
              <a-tag
                class="cv-ai-base-schema-tag cv-ai-suggested-tag relative"
                :class="{
                  'cv-selected': prompt.description === aiFormState.prompt.trim(),
                  'cv-disabled': !aiIntegrationAvailable || (aiLoading && callFunction === 'onPredictSchema'),
                }"
              ...
              >
                ...
                <div
                  ...
                  class="bg-cv-fill-purple-dark text-white rounded-full absolute -right-[3px] -top-[4px] h-3 w-3 grid place-items-center"
                >
            ...
            <NcButton size="xs" type="text" icon-position="right" class="cv-show-more-tags-btn" @click="onToggleShowMore">
          ...
          <div>
            <a-textarea
              ...
              class="!w-full !min-h-[120px] !rounded-lg mt-2 overflow-y-auto cv-scrollbar-thin cv-input-shadow cv-ai-input"
              size="middle"
              :disabled="!aiIntegrationAvailable || (aiLoading && callFunction === 'onPredictSchema')"
              ...
              <div class="flex flex-col gap-6 pt-6">
                <div v-for="field of additionalDetails" :key="field.title" class="flex items-center gap-2">
                  ...
                  <a-input
                    ...
                    class="cv-input-sm cv-input-shadow cv-ai-input"
                    hide-details
        ...
        <div
          ...
          :class="{
            'border-cv-border-gray-medium': showBtnTopBorder,
            'border-transparent': !showBtnTopBorder,
          }"
        >
          <div v-if="aiError" class="w-full flex items-start gap-3 bg-cv-bg-red-light rounded-lg p-4">
            <GeneralIcon icon="ncInfoSolid" class="flex-none !text-cv-content-red-dark w-6 h-6" />

            <div class="w-[calc(100%_-_36px)] flex flex-col gap-1">
              <div class="font-bold text-base text-cv-content-gray">Something went wrong</div>
              <NcTooltip class="truncate text-sm text-cv-content-gray-subtle" show-on-truncate-only>
                <template #title>
                  {{ aiError }}
                </template>
                {{ aiError }}
          ...
          <AiIntegrationNotFound v-else class="justify-between" @on-navigate="dialogShow = false">
            <template #icon>
              <GeneralIcon icon="alertTriangleSolid" class="flex-none !text-cv-content-orange-medium w-6 h-6" />
            </template>
            <template #title>
              <div class="text-base font-bold text-cv-content-gray">{{ $t('title.aiIntegrationMissing') }}</div>
            </template>
            <template #description>
              <div class="text-sm text-cv-content-gray-subtle">{{ $t('title.noAiIntegrationsHaveBeenAdded') }}</div>
            </template>
          </AiIntegrationNotFound>
        </div>
      </div>
      <div class="w-[calc(100%_-_480px)] h-full p-6 cv-scrollbar-thin flex flex-col gap-6">
        <!-- create base preview panel -->

        <template v-if="aiStep === AI_STEP.LOADING || aiStep === AI_STEP.PROMPT">
          <div v-if="aiStep === AI_STEP.LOADING" class="text-sm font-bold text-cv-content-purple-dark">
            {{ $t('title.generatingBaseTailoredToYourRequirement') }}
          </div>
          <div v-else class="text-sm font-bold text-cv-content-purple-dark">{{ $t('labels.preview') }}</div>

          <template v-if="aiStep === AI_STEP.LOADING">
            <div
              v-for="(loadingText, idx) of activeLoadingText"
              :key="idx"
              class="text-sm text-cv-content-purple-light flex items-center"
            >
              {{ loadingText }}
              <div v-if="loadingText.length === loadingMessages[idx]?.length" class="cv-animate-dots"></div>
            </div>
              ...
              >
                <div class="flex-1 flex items-center gap-2">
                  <a-skeleton-input
                    ...
                    class="!w-4 !h-4 !rounded overflow-hidden !bg-cv-bg-purple-light"
                  />
                  <a-skeleton-input
                    :active="aiStep === AI_STEP.LOADING"
                    class="!h-4 !rounded overflow-hidden !bg-cv-bg-purple-light"
                    :class="{
                      '!w-[133px]': idx % 2 === 0,
                      '!w-[90px]': idx % 2 !== 0,
                    }"
                ...
                <div class="grid place-items-center h-6 w-6">
                  <GeneralIcon icon="arrowDown" class="text-cv-content-purple-light" />
                </div>
              </div>
            </div>
          </template>
        ...
        <template v-if="aiStep === AI_STEP.MODIFY">
          <div class="text-sm font-bold text-cv-content-purple-dark">{{ $t('title.hereYourCrmBase') }}</div>
            ...
            >
              <template #tabContent>
                <a-collapse
                  ...
                  class="cv-schema-preview-table flex flex-col"
                >
                  <template #expandIcon> </template>
...
:deep(.cv-schema-preview-table .ant-collapse-header) {
...
:deep(.ant-collapse.cv-schema-preview-table) {
...
:deep(.cv-schema-preview-table .cv-checkbox > .ant-checkbox) {
  @apply !mr-0;
}

:deep(.ant-tag.cv-ai-base-schema-tag) {
  &.cv-selected {
    @apply !bg-cv-bg-purple-dark;
  }
}

.cv-show-more-tags-btn {
