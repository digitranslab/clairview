...
<template>
  <div v-if="currentWorkspace" class="flex w-full flex-col cv-workspace-settings">
    <div
      v-if="!props.workspaceId"
      class="min-w-0 p-2 h-[var(--topbar-height)] border-b-1 border-gray-200 flex items-center gap-3"
    >
      <div class="flex-1 cv-breadcrumb cv-no-negative-margin pl-1 cv-workspace-title">
        <div class="cv-breadcrumb-item capitalize">
          {{ currentWorkspace?.title }}
        </div>
        <GeneralIcon icon="ncSlash1" class="cv-breadcrumb-divider" />

        <h1 class="cv-breadcrumb-item active">
    ...
    <NcTabs v-model:activeKey="tab">
      ...
      <template v-if="isUIAllowed('workspaceManage')">
        <a-tab-pane key="settings" class="w-full">
          <template #tab>
            <div class="tab-title" data-testid="cv-workspace-settings-tab-settings">
