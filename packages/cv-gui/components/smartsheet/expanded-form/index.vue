...
import type { ColumnType, TableType, ViewType } from 'clairview-sdk'
import { ViewTypes, isSystemColumn } from 'clairview-sdk'
...
<template>
  ...
  >
    <div class="h-[85vh] xs:(max-h-full h-full) max-h-215 flex flex-col">
      ...
      <div
        class="flex gap-2 min-h-7 flex-shrink-0 w-full items-center cv-expanded-form-header p-4 xs:(px-2 py-0 min-h-[48px]) border-b-1 border-gray-200"
      >
        <div class="flex gap-2">
          <div class="flex gap-2">
            <NcTooltip v-if="props.showNextPrevIcons">
              ...
              <NcButton
                ...
                class="cv-prev-arrow !w-7 !h-7 !text-gray-500 !disabled:text-gray-300"
                type="text"
                size="xsmall"
                @click="onPrev"
              >
            ...
            <NcTooltip v-if="props.showNextPrevIcons">
              ...
              <NcButton
                ...
                class="cv-next-arrow !w-7 !h-7 !text-gray-500 !disabled:text-gray-300"
                type="text"
                size="xsmall"
                @click="onNext"
              >
          ...
          <div v-else class="flex-1 flex items-center gap-2 xs:(flex-row-reverse justify-end)">
            <div class="hidden md:flex items-center rounded-lg bg-gray-100 px-2 py-1 gap-2">
              ...
              <span class="cv-expanded-form-table-name">
                {{ tableTitle }}
              </span>
            </div>
            <div
        ...
        <div class="ml-auto md:mx-auto">
          <NcSelectTab
            ...
            class="cv-expanded-form-mode-switch"
        ...
        <div class="flex gap-2">
          <NcTooltip v-if="!isMobileMode && isUIAllowed('dataEdit')">
            ...
            <NcButton
              ...
              :loading="isSaving"
              class="cv-expand-form-save-btn !xs:(text-base) !h-7 !px-2"
          ...
          <NcTooltip>
            ...
            <NcButton
              ...
              @click="copyRecordUrl()"
            >
              <div v-e="['c:row-expand:copy-url']" data-testid="cv-expanded-form-copy-url" class="flex items-center">
                <component :is="iconMap.check" v-if="isRecordLinkCopied" class="cursor-pointer cv-duplicate-row h-4 w-4" />
                <component :is="iconMap.copy" v-else class="cursor-pointer cv-duplicate-row h-4 w-4" />
              </div>
            </NcButton>
          </NcTooltip>
          <NcDropdown v-if="!isNew && rowId && !isMobileMode" placement="bottomRight">
            <NcButton type="text" size="xsmall" class="cv-expand-form-more-actions !w-7 !h-7" :disabled="isLoading">
            ...
            <template #overlay>
              <NcMenu>
                ...
                <NcMenuItem
                  ...
                  @click="copyRecordUrl()"
                >
                  <div v-e="['c:row-expand:copy-url']" data-testid="cv-expanded-form-copy-url" class="flex gap-2 items-center">
                ...
                <NcMenuItem v-if="isUIAllowed('dataEdit')" class="text-gray-700" @click="!isNew ? onDuplicateRow() : () => {}">
                  <div v-e="['c:row-expand:duplicate']" class="flex gap-2 items-center" data-testid="cv-expanded-form-duplicate">
                    <component :is="iconMap.duplicate" class="cursor-pointer cv-duplicate-row" />
                    <span class="-ml-0.25">
                ...
                >
                  <div v-e="['c:row-expand:delete']" class="flex gap-2 items-center" data-testid="cv-expanded-form-delete">
          ...
          <NcButton
            class="cv-expand-form-close-btn !w-7 !h-7"
            data-testid="cv-expanded-form-close"
            type="text"
            size="xsmall"
            @click="onClose"
          >
...
.cv-data-cell {
  ...
  &:not(.cv-readonly-div-data-cell):not(.cv-system-field):not(.cv-attachment-cell):not(.cv-virtual-cell-button):not(
      :has(.cv-cell-ai-button)
    ) {
    box-shadow: 0px 0px 4px 0px rgba(0, 0, 0, 0.08);
  }
  &:not(:focus-within):hover:not(.cv-readonly-div-data-cell):not(.cv-system-field):not(.cv-virtual-cell-button):not(
      :has(.cv-cell-ai-button)
    ) {
    @apply !border-1;
    &:not(.cv-attachment-cell):not(.cv-virtual-cell-button):not(:has(.cv-cell-ai-button)) {
