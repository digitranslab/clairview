...
const editor = useEditor({
  ...
  onBlur: (e) => {
    ...
    if (
      !targetEl?.closest(
        '.comment-bubble-menu, .cv-rich-text-comment, .tippy-box, .cv-comment-save-btn, .rich-text-bottom-bar, .mention, .cv-mention-list, .tippy-content, .cv-comment-rich-editor',
      )
    ) {
  ...
  (e: FocusEvent) => {
    ...
    if (
      ...
      (e.target as HTMLElement)?.closest(
        '.comment-bubble-menu, .cv-comment-save-btn, .cv-mention-list, .mention, .rich-text-bottom-bar, .tippy-content, .cv-comment-rich-editor',
      )
    )
      return
    ...
    if (
      !targetEl?.closest(
        '.comment-bubble-menu, .cv-comment-save-btn, .rich-text-bottom-bar, .mention, .tippy-content, .cv-mention-list, .cv-comment-rich-editor',
      )
    ) {
      isFocused.value = false
...
onClickOutside(editorDom, (e) => {
  ...
  if (
    !targetEl?.closest(
      '.tippy-content, .cv-rich-text-comment, .cv-comment-save-btn, .cv-mention-list, .rich-text-bottom-bar, .mention, .comment-bubble-menu, .cv-comment-rich-editor',
    )
  ) {
    isFocused.value = false
    emits('blur')
...
onMounted(() => {
  ...
  setTimeout(() => {
    document.querySelectorAll('.cv-rich-link-tooltip').forEach((el) => {
...
<template>
  <div
    :class="{
      'readonly': readOnly,
      'cv-rich-text-grid': isGrid,
    }"
    :tabindex="1"
    class="cv-rich-text-comment flex flex-col w-full h-full"
  ...
  >
    ...
    <template v-else>
      ...
      <EditorContent
        ...
        class="flex flex-col cv-comment-rich-editor w-full scrollbar-thin scrollbar-thumb-gray-200 cv-truncate scrollbar-track-transparent"
      ...
      <div v-if="!hideOptions" class="flex justify-between pt-1 rich-text-bottom-bar items-center">
        ...
        <NcButton
          ...
          class="!disabled:bg-gray-100 cv-comment-save-btn !h-7 !w-7 !shadow-none"
          size="xsmall"
          @click="saveComment"
        >
...
.cv-rich-text-comment {
  .readonly {
    .cv-comment-rich-editor {
      .ProseMirror {
        resize: none;
  ...
  .cv-comment-rich-editor {
    &.cv-truncate {
      ...
      &.cv-line-clamp-1 .tiptap.ProseMirror {
        -webkit-line-clamp: 1;
      }
      &.cv-line-clamp-2 .tiptap.ProseMirror {
        -webkit-line-clamp: 2;
      }
      &.cv-line-clamp-3 .tiptap.ProseMirror {
        -webkit-line-clamp: 3;
      }
      &.cv-line-clamp-4 .tiptap.ProseMirror {
        -webkit-line-clamp: 4;
      }
    }
