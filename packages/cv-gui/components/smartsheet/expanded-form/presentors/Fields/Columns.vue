...
<template>
  <div
    ...
    class="flex flex-col flex-grow gap-6 h-full max-h-full cv-scrollbar-thin items-center w-full p-4 xs:(px-4 pt-4 pb-2 gap-6) children:max-w-[588px] <lg:(children:max-w-[450px])"
  >
    <div
      v-for="(col, i) of fields"
      v-show="!isVirtualCol(col) || !isNew || isLinksOrLTAR(col)"
      :key="col.title"
      :class="`cv-expand-col-${col.title}`"
      :col-id="col.id"
      :data-testid="`cv-expand-col-${col.title}`"
      class="cv-expanded-form-row w-full"
    >
      <div
        class="flex items-start cv-expanded-cell min-h-[37px]"
        :class="{
      ...
      >
        ...
        >
          <LazySmartsheetHeaderVirtualCell
            ...
            :column="col"
            class="cv-expanded-cell-header h-full flex-none"
          />
          <LazySmartsheetHeaderCell v-else :column="col" class="cv-expanded-cell-header flex-none" />
        </div>
        ...
        <template v-else>
          <SmartsheetDivDataCell
            ...
            :class="{
              ...
              '!select-text cv-system-field': isReadOnlyVirtualCell(col),
              '!select-text cv-readonly-div-data-cell': readOnly,
              'cv-mentioned-cell': col.id === mentionedCell,
            }"
          >
    ...
    <template v-if="hiddenFields.length > 0 && showHiddenFields">
      <div
        ...
        class="cv-expanded-form-row w-full"
      >
        <div class="flex items-start flex-row sm:(gap-x-2) <lg:(flex-col w-full) cv-expanded-cell min-h-[37px]">
          <div class="w-45 <lg:(w-full px-0) h-[37px] xs:(h-auto) flex items-center rounded-lg overflow-hidden">
            <LazySmartsheetHeaderVirtualCell
              v-if="isVirtualCol(col)"
              :column="col"
              is-hidden-col
              class="cv-expanded-cell-header flex-none"
            />

            <LazySmartsheetHeaderCell v-else :column="col" is-hidden-col class="cv-expanded-cell-header flex-none" />
          </div>
          ...
          <template v-else>
            <LazySmartsheetDivDataCell
              ...
              :class="{
                '!select-text cv-system-field': isReadOnlyVirtualCell(col),
                '!bg-gray-50 !select-text cv-readonly-div-data-cell': readOnly,
                'cv-mentioned-cell': col.id === mentionedCell,
              }"
            >
              <LazySmartsheetVirtualCell
