...
import { type CommentType, ProjectRoles } from 'clairview-sdk'
...
<template>
  ...
  >
    ...
    <div v-else class="flex flex-col h-full">
      <div v-if="comments.length === 0" class="flex flex-col my-1 text-center justify-center h-full cv-scrollbar-thin">
      ...
      <div v-else ref="commentsWrapperEl" class="flex flex-col h-full py-1 cv-scrollbar-thin">
        <div
          v-for="commentItem of comments"
          :key="commentItem.id"
          :class="`${commentItem.id}`"
          class="cv-comment-item"
          @mouseover="handleResetHoverEffect"
        >
          <div
            :class="{
                  'hover:bg-gray-100': editCommentValue?.id !== commentItem!.id,
                  'cv-hovered-comment bg-gray-100': hoveredCommentId === commentItem!.id
                }"
            ...
            <div class="flex items-start justify-between">
              ...
              <div class="flex items-center">
                <NcDropdown
                  ...
                  class="cv-comment-more-actions !hidden !group-hover:block"
                  overlay-class-name="!min-w-[160px]"
                  placement="bottomRight"
                >
                  <NcButton
                    class="cv-expand-form-more-actions !hover:bg-gray-200 !w-7 !h-7 !bg-transparent"
                    size="xsmall"
                    type="text"
                  >
                ...
                <div v-if="appInfo.ee">
                  <NcTooltip v-if="!commentItem.resolved_by && hasEditPermission">
                    <NcButton
                      class="cv-resolve-comment-btn !w-7 !h-7 !bg-transparent !hover:bg-gray-200 !hidden !group-hover:block"
                      size="xsmall"
                      type="text"
      ...
      <div v-if="hasEditPermission" class="px-3 pb-3 cv-comment-input !rounded-br-2xl gap-2 flex">
        <SmartsheetExpandedFormRichComment
          ref="commentInputRef"
...
:deep(.expanded-form-comment-edit-input .cv-comment-rich-editor) {
  @apply bg-white;
}

.cv-hovered-comment {
  .cv-expand-form-more-actions,
  .cv-resolve-comment-btn {
    @apply !block;
  }
}
</style>
