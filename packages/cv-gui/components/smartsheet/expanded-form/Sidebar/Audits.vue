...
import { type AuditType, ProjectRoles } from 'clairview-sdk'
...
function scrollToAudit(auditId?: string) {
  ...
  const auditEl = auditsWrapperEl.value?.querySelector(`.cv-audit-item.${auditId}`)
  if (auditEl) {
...
<template>
  <div class="h-full pb-1">
    ...
    <div v-else ref="auditsWrapperEl" class="flex flex-col h-full py-1 cv-scrollbar-thin">
      ...
      <div v-for="audit of audits" :key="audit.id" :class="`${audit.id}`" class="cv-audit-item">
