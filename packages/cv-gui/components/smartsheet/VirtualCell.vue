...
import { isCreatedOrLastModifiedByCol, isCreatedOrLastModifiedTimeCol } from 'clairview-sdk'
...
<template>
  <div
    class="cv-virtual-cell w-full flex items-center"
    :class="[
      `cv-virtual-cell-${(column.uidt || 'default').toLowerCase()}`,
      {
        'text-right justify-end': isGrid && !isForm && isRollup(column) && !isExpandedForm,
        'cv-display-value-cell': isPrimary(column) && !isForm,
      },
    ]"
