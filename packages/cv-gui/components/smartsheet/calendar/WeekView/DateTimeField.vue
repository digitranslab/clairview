...
<template>
  <div
    ref="scrollContainer"
    class="prevent-select h-[calc(100vh-5.4rem)] overflow-y-auto cv-scrollbar-md relative flex flex-col w-full"
    data-testid="cv-calendar-week-view"
    @drop="dropEvent"
  >
    <div
    ...
    >
      <div
        ...
        :key="index"
        class="h-13 first:mt-0 pt-7.1 cv-calendar-day-hour text-right pr-2 font-semibold text-xs text-gray-500 py-1"
      >
        {{ hour.format('hh a') }}
      </div>
    </div>
    ...
    >
      <div
        ...
        data-testid="cv-calendar-week-day"
      >
        <div
          ...
          data-testid="cv-calendar-week-hour"
      ...
      <div
        ...
        data-testid="cv-calendar-week-record-container"
      >
        <template v-for="record in recordsAcrossAllRange.records" :key="record.rowMeta.id">
          <div
            v-if="record.rowMeta.style?.display !== 'none'"
            :data-testid="`cv-calendar-week-record-${record.row[displayField!.title!]}`"
