...
import { type ColumnType, UITypes } from 'clairview-sdk'
...
watch(
  ...
  () => {
    setTimeout(() => {
      ...
      else document.querySelectorAll('.cv-calendar-day-hour').item(9)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 100)
  },
  { immediate: true },
)
...
<template>
  <div class="h-[calc(100vh-5.3rem)] overflow-y-auto cv-scrollbar-md">
    ...
    />
    <div ref="container" class="w-full flex relative no-selection" data-testid="cv-calendar-day-view" @drop="dropEvent">
      <div
      ...
      <div>
        <div
          ...
          data-testid="cv-calendar-day-hour"
      ...
      <div class="w-full">
        <div
          ...
          class="flex w-full border-l-gray-100 h-13 transition cv-calendar-day-hour relative border-1 group hover:bg-gray-50 border-white border-b-gray-100"
          data-testid="cv-calendar-day-hour"
          @click="selectHour(hour)"
          @dblclick="newRecord(hour)"
        >
      ...
      <div class="absolute inset-0 pointer-events-none">
        <div
          class="relative !ml-[68px] !mr-1 z-2 cv-calendar-day-record-container"
          data-testid="cv-calendar-day-record-container"
        >
          <template v-for="record in recordsAcrossAllRange.record" :key="record.rowMeta.id">
            <div
              v-if="record.rowMeta.style?.display !== 'none'"
              :data-testid="`cv-calendar-day-record-${record.row[displayField!.title!]}`"
