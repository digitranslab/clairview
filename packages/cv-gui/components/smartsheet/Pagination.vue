...
<template>
  <div
    class="flex items-center bg-white border-gray-200 cv-grid-pagination-wrapper"
  ...
  >
    ...
    >
      <div v-if="isViewDataLoading" class="cv-pagination-skeleton flex flex-row justify-center item-center min-h-10 min-w-42">
    ...
    <div v-if="!isMobileMode" class="flex-1 flex justify-end items-center">
      ...
      <div class="text-right">
        <span
          ...
          class="caption cv-grid-row-count mr-2.5 text-gray-500 text-xs"
          data-testid="grid-pagination"
        >
          {{ count }} {{ customLabel ? customLabel : count !== 1 ? $t('objects.records') : $t('objects.record') }}
        </span>
...
:deep(.cv-pagination .ant-pagination-item) {
