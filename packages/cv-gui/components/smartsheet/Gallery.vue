...
<template>
  <div
    ref="scrollContainer"
    data-testid="cv-gallery-wrapper"
    class="flex flex-col w-full cv-gallery select-none relative cv-scrollbar-md bg-gray-50 h-[calc(100svh-93px)]"
  >
    <NcDropdown
    ...
    >
      ...
      <div class="flex-1">
        <div :key="containerHeight" class="relative" :style="{ height: `${containerHeight}px` }">
          ...
          <div class="cv-gallery-container grid gap-3 p-3">
            <div
            ...
            >
              <LazySmartsheetRow :row="record">
                <a-card
                  ...
                  :data-testid="`cv-gallery-card-${record.rowMeta.rowIndex}`"
            ...
            <template v-if="visibleRows.length <= 4">
              <div v-for="index of Array(8 - visibleRows.length)" :key="index" class="cv-empty-card"></div>
            </template>
          </div>
        </div>
      </div>
