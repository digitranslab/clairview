...
const enum NcForm {
  heading = 'cv-form-heading',
  subheading = 'cv-form-sub-heading',
}
...
const handleAutoScrollFormField = (title: string, isSidebar: boolean) => {
  const field = document.querySelector(
    `${isSidebar ? '.cv-form-field-item-' : '.cv-form-drag-'}${CSS.escape(title?.replaceAll(' ', ''))}`,
  )
  ...
  (e: MouseEvent) => {
    if (
      ...
      (e.target as HTMLElement)?.closest(
        '.cv-form-right-panel, [class*="dropdown"], .cv-form-rich-text-field, .ant-modal, .ant-modal-wrap, .cv-share-base-button, .cv-form-right-sidebar-content-resizable-wrapper .splitpanes__splitter',
      )
    ) {
      return
    }
...
<template>
  <div class="h-full relative">
    ...
    <template v-else>
      <div
        v-if="submitted"
        class="h-full p-6 overflow-auto cv-form-scrollbar"
        :style="{ background: parseProp(formViewData?.meta)?.background_color || '#F9F9FA' }"
        data-testid="cv-form-wrapper-submit"
      >
          ...
          >
            <div v-if="formViewData" class="items-center justify-center text-left mt-2">
              ...
              <div class="flex justify-center">
                <div class="w-full">
                  <a-alert class="cv-form-success-msg !my-4 !py-4 text-left !rounded-lg" type="success" outlined>
                    <template #message>
                      <LazyCellRichText
      ...
      <div v-else class="cv-form-wrapper h-full w-full flex relative" data-testid="cv-form-wrapper">
        ...
        <template v-else-if="formViewData">
          <SmartsheetFormLayout>
            <template #preview>
              <div
                class="w-full h-full overflow-auto cv-form-scrollbar p-6"
                :style="{background:(formViewData?.meta as Record<string,any>).background_color || '#F9F9FA'}"
              >
                <div class="min-w-[616px] overflow-x-auto cv-form-scrollbar">
                  <GeneralImageCropper
                    v-if="isEditable"
                      ...
                      <div class="flex items-center space-x-1 m-2">
                        <NcTooltip :disabled="isEeUI || isLocked">
                          ...
                          <NcButton
                            type="secondary"
                            size="small"
                            class="cv-form-upload-banner-btn"
                            data-testid="cv-form-upload-banner-btn"
                        ...
                        <NcTooltip v-if="isEeUI && formViewData.banner_image_url" :disabled="isLocked">
                          ...
                          <NcButton
                            type="secondary"
                            size="small"
                            class="cv-form-delete-banner-btn"
                            data-testid="cv-form-delete-banner-btn"
                            :disabled="isLocked"
                            @click="
                              () => {
                                if (isEditable) {
                  ...
                  >
                    <a-form :model="formState" class="cv-form" no-style>
                      <!-- form header -->
                      <div class="flex flex-col px-4 lg:px-6">
                        <!-- Form logo  -->
                        <div class="mb-4">
                          <div
                            class="cv-form-logo-wrapper mx-6 group relative inline-block h-56px overflow-hidden flex items-center"
                            :class="
                          ...
                          >
                            <LazyCellAttachmentPreviewImage
                              ...
                              :srcs="getFormLogoSrc"
                              class="flex-none cv-form-logo !object-contain object-left max-h-full max-w-full !m-0"
                            />
                            <div
                            ...
                            >
                              <NcTooltip :disabled="isEeUI || isLocked">
                                ...
                                <NcButton
                                  ...
                                  type="secondary"
                                  size="small"
                                  class="cv-form-upload-logo-btn"
                                  data-testid="cv-form-upload-log-btn"
                                  :disabled="!isEeUI || isLocked"
                              ...
                              <NcTooltip v-if="isEeUI && formViewData.logo_url" :disabled="isLocked">
                                ...
                                <NcButton
                                  type="secondary"
                                  size="small"
                                  class="cv-form-delete-logo-btn"
                                  data-testid="cv-form-delete-logo-btn"
                                  :disabled="isLocked"
                                  @click="
                              () => {
                                if (isEditable) {
                        ...
                        >
                          <a-form-item v-if="isEditable" class="!my-0">
                            <a-textarea
                              ...
                              class="cv-form-focus-element !p-0 !m-0 w-full !font-bold !text-2xl !border-0 !rounded-none !text-gray-900"
                              :style="{
                        ...
                        >
                          <LazyCellRichText
                            ...
                            class="cv-form-description cv-form-focus-element font-medium text-base !text-gray-500 -ml-1"
                            is-form-field
                      ...
                      <Draggable
                        ...
                        draggable=".item"
                        handle=".cv-form-field-drag-handler"
                        group="form-inputs"
                        ghost-class="cv-form-field-ghost"
                      ...
                      >
                        <template #item="{ element }">
                          <div
                            ...
                            :key="element.id"
                            class="cv-editable cv-form-focus-element item relative bg-white p-4 lg:p-6"
                            :class="[
                              `cv-form-drag-${element.title.replaceAll(' ', '')}`,
                              {
                              ...
                              {
                                'cv-form-field-drag-handler border-transparent hover:(bg-gray-50) cursor-pointer':
                          ...
                          >
                            <div v-if="activeRow === element.id" class="absolute -left-3 top-6">
                              <NcButton
                                type="primary"
                                size="small"
                                class="cv-form-field-drag-handler !cursor-move !p-1 !min-w-6 !h-auto !rounded"
                              >
                                <component
                                  :is="iconMap.drag"
                                  class="cv-form-field-drag-handler flex-none !h-4 !w-4 text-white font-bold"
                                />
                              </NcButton>
                            </div>
                              ...
                              >
                                ...
                                <Transition name="icon-fade">
                                  <GeneralIcon
                                    ...
                                    icon="eye"
                                    class="cv-field-visibility-icon cv-field-visible w-3.5 h-3.5 flex-none text-cv-content-gray-muted"
                                  />
                                  <GeneralIcon
                                    v-else
                                    icon="eyeSlash"
                                    class="cv-field-visibility-icon w-3.5 h-3.5 flex-none text-cv-content-gray-muted"
                                  />
                                </Transition>
                              </NcTooltip>
                              <div class="text-sm font-semibold text-gray-800">
                                <span data-testid="cv-form-input-label">
                            ...
                            <LazyCellRichText
                              ...
                              read-only
                              sync-value-change
                              class="cv-form-help-text text-gray-500 text-sm mt-2 -ml-1"
                              data-testid="cv-form-help-text"
                              @update:value="updateColMeta(element)"
                            />

                            <!-- Field Body  -->

                            <div class="cv-form-field-body">
                              <div class="mt-2">
                                <a-form-item
                                  v-if="fieldMappings[element.title]"
                                  :name="fieldMappings[element.title]"
                                  class="!my-0 cv-input-required-error cv-form-input-item"
                                ...
                                >
                                  <LazySmartsheetDivDataCell class="relative" @click.stop>
                                    <LazySmartsheetVirtualCell
                                      ...
                                      :row="row"
                                      class="cv-input"
                                      :class="`cv-form-input-${element.title.replaceAll(' ', '')}`"
                                      :data-testid="`cv-form-input-${element.title.replaceAll(' ', '')}`"
                                      :column="element"
                                    />
                                    <LazySmartsheetCell
                                      v-else
                                      ...
                                      :class="[
                                        `cv-form-input-${element.title.replaceAll(' ', '')}`,
                      ...
                      <div class="flex justify-between items-center mt-6 !px-8 !lg:px-12">
                        <NcButton
                          ...
                          class="cv-form-clear cv-form-focus-element"
                          data-testid="cv-form-clear"
                          data-title="cv-form-clear"
                          @click="clearForm"
                        >
                        ...
                        <NcButton
                          ...
                          size="small"
                          :disabled="!isUIAllowed('dataInsert') || !visibleColumns.length"
                          class="cv-form-submit cv-form-focus-element"
                          data-testid="cv-form-submit"
                          data-title="cv-form-submit"
                          @click="submitForm"
                        >
                          {{ $t('general.submit') }}
                        </NcButton>
            ...
            <template #sidebar>
              <!-- Right Panel -->
              <div
                class="cv-form-right-panel h-full flex-grow max-w-full"
                :class="{
                  'overflow-y-auto cv-form-scrollbar': activeField,
                  'relative': isLocked,
                }"
              >
                <!-- Form Field settings -->
                <div v-if="activeField && activeColumn" :key="activeField?.id" class="cv-form-field-right-panel">
                  <!-- Field header -->
                  <div class="px-3 pt-4 pb-2 flex items-center justify-between border-b border-gray-200 font-medium">
                    ...
                    <div class="flex items-center space-x-2">
                      <a-dropdown
                        ...
                        :trigger="['click']"
                        overlay-class-name="cv-dropdown-form-edit-column"
                        :disabled="!isUIAllowed('fieldEdit') || isLocked"
                        @visible-change="onVisibilityChange('showEditColumn')"
                      >
                        <NcButton type="secondary" size="small" class="cv-form-add-field" data-testid="cv-form-add-field">
                  ...
                  <!-- Field text -->
                  <div class="cv-form-field-text p-4 flex flex-col gap-4 border-b border-gray-200">
                    ...
                    <a-textarea
                      ...
                      auto-size
                      hide-details
                      class="form-meta-input cv-form-input-label !max-h-7.5rem cv-form-scrollbar"
                      data-testid="cv-form-input-label"
                    ...
                    <LazyCellRichText
                      ...
                      class="form-meta-input cv-form-input-help-text"
                      is-form-field
                      :hidden-bubble-menu-options="hiddenBubbleMenuOptions"
                      hide-mention
                      data-testid="cv-form-input-help-text"
                ...
                <!-- Form Settings -->
                <template v-else>
                  <Splitpanes v-if="formViewData" horizontal class="cv-form-settings w-full cv-form-right-splitpane">
                    <Pane min-size="30" size="50" class="cv-form-right-splitpane-item p-4 flex flex-col space-y-4 !min-h-200px">
                      <div class="flex flex-wrap justify-between items-center gap-2">
                        ...
                        <a-dropdown
                          ...
                          :trigger="['click']"
                          :disabled="isLocked"
                          overlay-class-name="cv-dropdown-form-add-column"
                      ...
                      <form autocomplete="off">
                        <a-input
                          key="cv-form-field-search-input"
                          v-model:value="searchQuery"
                          type="text"
                          autocomplete="off"
                          class="!h-9 !px-3 !py-1 !rounded-lg"
                          :placeholder="`${$t('placeholder.searchFields')}...`"
                          name="cv-form-field-search-input"
                          data-testid="cv-form-field-search-input"
                        >
                          <template #prefix>
                          ...
                          <template #suffix>
                            <GeneralIcon
                              ...
                              data-testid="cv-form-field-clear-search"
                              @click="searchQuery = ''"
                            />
                          </template>
                        </a-input>
                      ...
                      <div class="cv-form-fields-list border-1 border-gray-200 rounded-lg overflow-y-auto cv-form-scrollbar">
                          ...
                          >
                            <template #item="{ element: field }">
                              <div
                                ...
                                :class="[
                                  `cv-form-field-item-${field.title.replaceAll(' ', '')}`,
                                  `${activeRow === field.id ? 'bg-brand-50 font-medium' : 'hover:bg-gray-50'}`,
                                ]"
                                :data-testid="`cv-form-field-item-${field.title}`"
                              >
                                      ...
                                      <div class="w-full flex items-center">
                                        <div class="ml-1 inline-flex" :class="field.label?.trim() ? 'max-w-1/2' : 'max-w-[95%]'">
                                          <NcTooltip class="truncate text-sm" :disabled="drag" show-on-truncate-only>
                                            ...
                                            <span data-testid="cv-field-title"> {{ field.title }} </span>
                                          </NcTooltip>
                                        </div>
                                        <div
                                        ...
                                        >
                                          ...
                                          <NcTooltip class="truncate" :disabled="drag" show-on-truncate-only>
                                            ...
                                            <span data-testid="cv-field-title ">{{ field.label?.trim() }}</span>
                                          </NcTooltip>
                                          <span>)</span>
                                        </div>
                    ...
                    <Pane min-size="20" size="50" class="cv-form-right-splitpane-item !overflow-y-auto cv-form-scrollbar">
                      <div class="p-4 flex flex-col space-y-4 border-b border-gray-200">
                        ...
                        <div class="flex flex-col space-y-3">
                          <div :class="isLocked || !isEditable ? 'pointer-events-none' : ''">
                            ...
                            <div class="flex justify-start">
                              <LazyGeneralColorPicker
                                ...
                                color-box-border
                                is-new-design
                                class="cv-form-theme-color-picker !pb-0 !pl-0 -ml-1"
                          ...
                          <div class="flex items-center justify-between gap-3">
                            <!-- Hide ClairView Branding -->

                            <span>{{ $t('labels.hideClairviewBranding') }}</span>
                            ...
                            <a-switch
                              ...
                              size="small"
                              class="cv-form-hide-branding"
                              data-testid="cv-form-hide-branding"
                              :disabled="isLocked || !isEditable"
                              @change="(value) => {
                          ...
                          <div class="flex items-center justify-between gap-3">
                            ...
                            <a-switch
                              ...
                              size="small"
                              class="cv-form-hide-banner"
                              data-testid="cv-form-hide-banner"
                              :disabled="isLocked || !isEditable"
                              @change="(value) => {
                                  if (isLocked || !isEditable) return
                        ...
                        <div class="flex flex-col gap-3">
                          <div class="flex flex-col gap-3">
                            <div class="flex items-center justify-between gap-3">
                              ...
                              <a-switch
                                ...
                                size="small"
                                class="cv-form-checkbox-redirect-url"
                                data-testid="cv-form-checkbox-redirect-url"
                                :disabled="isLocked || !isEditable"
                                @change="updateView"
                              />
                            </div>
                            <div v-if="isOpenRedirectUrl" class="flex flex-col gap-2 max-w-[calc(100%_-_40px)]">
                              <a-form-item class="!my-0" v-bind="redirectLinkValidation">
                                <a-input
                                  ...
                                  data-testid="cv-form-redirect-url-input"
                              ...
                              <div class="text-small leading-[18px] text-gray-400 pl-3">
                                ...
                                <a
                                  href="https://docs.clairview.com/views/view-types/form/#redirect-url"
                                  target="_blank"
                          ...
                          <template v-if="!isOpenRedirectUrl">
                            <div class="flex items-center justify-between gap-3">
                              ...
                              <a-switch
                                ...
                                size="small"
                                class="cv-form-checkbox-submit-another-form"
                                data-testid="cv-form-checkbox-submit-another-form"
                                :disabled="isLocked || !isEditable"
                                @change="updateView"
                              />
                            </div>
                            ...
                            <div class="flex items-center justify-between gap-3">
                              ...
                              <a-switch
                                ...
                                size="small"
                                class="cv-form-checkbox-show-blank-form"
                                data-testid="cv-form-checkbox-show-blank-form"
                                :disabled="isLocked || !isEditable"
                                @change="updateView"
                              />
                            </div>
                          ...
                          <div class="flex items-center justify-between gap-3">
                            ...
                            <a-switch
                              ...
                              size="small"
                              class="cv-form-checkbox-send-email"
                              data-testid="cv-form-checkbox-send-email"
                              :disabled="isLocked || !isEditable"
                        ...
                        <div v-if="!isOpenRedirectUrl" class="pb-10">
                          ...
                          <a-form-item class="!my-0">
                            <LazyCellRichText
                              ...
                              class="cv-form-after-submit-msg editable"
                              is-form-field
                              :hidden-bubble-menu-options="hiddenBubbleMenuOptions"
                              hide-mention
                              data-testid="cv-form-after-submit-msg"
                              @update:value="updateView" />
                            <LazyCellRichText
                              v-else
                              :value="formViewData.success_msg"
                              class="cv-form-after-submit-msg"
                              is-form-field
                              read-only
                              data-testid="cv-form-after-submit-msg"
                          /></a-form-item>
...
:deep(.cv-cell-attachment) {
  ...
  .cv-attachment-cell {
    ...
    .cv-attachment-cell-dropzone {
      @apply rounded bg-gray-400/75;
    }
  }
}

.cv-form-input-item .cv-data-cell {
...
.cv-form-input-enable-scanner-form-item {
  :deep(.ant-form-item-control-input) {
    @apply min-h-max;
  }
}
:deep(.cv-form-right-splitpane .splitpanes__splitter) {
...
:deep(.cv-form-theme-color-picker .color-selector) {
  @apply !text-white;
}

:deep(.cv-form-field-body .cv-cell) {
  @apply my-0;
}
.cv-form-field-ghost {
  @apply bg-gray-50;
}
:deep(.cv-form-input-required + button):focus-visible {
  box-shadow: 0 0 0 2px #fff, 0 0 0 4px #3366ff;
}
:deep(.cv-form-switch-focus):focus-visible {
  box-shadow: 0 0 0 2px #fff, 0 0 0 4px #3366ff;
}
.cv-form-field-layout {
...
.form-meta-input {
  ...
  &.cv-form-input-label .cv-textarea-rich-editor {
    @apply pt-2 pb-1 font-semibold text-gray-800;
  }
  &.cv-form-input-help-text .cv-textarea-rich-editor {
...
.cv-form-description {
  .cv-form-field-bubble-menu-wrapper {
    @apply -bottom-12;
  }
}
</style>
