...
import { type LinkToAnotherRecordType, ModelTypes, MssqlUi, RelationTypes, SqliteUi, UITypes, ViewTypes } from 'clairview-sdk'
...
<template>
  <div class="w-full flex flex-col gap-4">
    <div class="flex flex-col gap-4">
      <a-form-item :label="$t('labels.relationType')" v-bind="validateInfos.type" class="cv-ltar-relation-type">
    ...
    <template v-else>
      <a-form-item class="flex w-full pb-2 cv-ltar-child-table" v-bind="validateInfos.childId">
        <a-select
          ...
          dropdown-class-name="cv-dropdown-ltar-child-table"
    ...
    <div class="flex flex-col gap-2">
      <div class="flex gap-2 items-center">
        ...
        <span
          v-e="['c:link:limit-record-by-view', { status: limitRecToView }]"
          class="text-s"
          data-testid="cv-limit-record-view"
          @click="onViewLabelClick"
          >Limit record selection to a view</span
        >
      </div>
      <a-form-item v-if="limitRecToView" class="!pl-8 flex w-full pb-2 mt-4 space-y-2 cv-ltar-child-view">
        <NcSelect
          v-model:value="vModel.childViewId"
          :placeholder="$t('labels.selectView')"
          show-search
          :filter-option="filterOption"
          dropdown-class-name="cv-dropdown-ltar-child-view"
        >
    ...
    <template v-if="isEeUI">
      <div class="flex flex-col gap-2">
        <div class="flex gap-2 items-center">
          ...
          <span
            v-e="['c:link:limit-record-by-filter', { status: limitRecToCond }]"
            data-testid="cv-limit-record-filters"
          ...
          <div class="flex flex-row space-x-2">
            <a-form-item class="flex w-1/2" :label="$t('labels.onUpdate')">
              <a-select
                ...
                name="onUpdate"
                dropdown-class-name="cv-dropdown-on-update"
            ...
            <a-form-item class="flex w-1/2" :label="$t('labels.onDelete')">
              <a-select
                ...
                :disabled="vModel.virtual"
                name="onDelete"
                dropdown-class-name="cv-dropdown-on-delete"
                @change="onDataTypeChange"
              >
                <template #suffixIcon>
                  <GeneralIcon icon="arrowDown" class="text-gray-700" />
...
:deep(.cv-ltar-relation-type .ant-radio-group) {
  ...
  .ant-radio-wrapper {
    ...
    span:not(.ant-radio):not(.cv-ltar-icon) {
...
:deep(.cv-ltar-relation-type .ant-col.ant-form-item-control) {
  @apply h-8.5;
}
</style>
...
<!-- todo: remove later
<style lang="scss" scoped>
.cv-ltar-relation-type-radio-group {
  .cv-ltar-icon {
