...
import { UITypes, isHiddenCol, jsepCurlyHook } from 'clairview-sdk'
...
<template>
  ...
  >
    ...
    <div class="flex flex-col max-h-120 cv-scrollbar-thin pr-2">
  ...
  <template v-if="isFeatureEnabled(FEATURE_FLAG.AI_FEATURES)">
    <div v-if="aiMode === AI_MODE.NONE" class="w-full flex justify-end mt-2">
      <NcButton size="small" type="text" :loading="aiLoading" @click="enableAI">
        <template #icon>
          <GeneralIcon icon="ncAutoAwesome" class="text-cv-content-purple-medium h-4 w-4" />
        </template>
        <template #loadingIcon>
          <GeneralLoader class="!text-cv-content-purple-medium" size="regular" />
        </template>
    ...
    <template v-else-if="aiMode === AI_MODE.PROMPT">
      ...
      <div v-else class="prompt-wrapper">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="9" viewBox="0 0 18 9" fill="none" class="cv-polygon-2">
          <path d="M1.51476 8.5L9 0.721111L16.4852 8.5H1.51476Z" fill="white" stroke="#e5d4f5" />
        </svg>
        <div class="prompt-input-wrapper w-full flex">
          <div class="cv-triangle-bottom-bar"></div>

          <div class="flex items-center gap-2 pl-3 pr-1 py-1">
            <div class="flex-1 text-small leading-[18px] font-bold text-cv-content-gray-subtle2">Prompt</div>
            <div class="flex items-center gap-2">
              <NcButton
                ...
                theme="ai"
                :bordered="false"
                class="cv-formula-helper-ai-btn !px-2"
          ...
          <a-textarea
            ...
            class="cv-ai-formula-helper-input cv-input-shadow cv-ai-input cv-scrollbar-thin !min-h-[80px]"
  ...
  <div
    ...
    class="overflow-auto flex flex-col cv-suggestion-list cv-scrollbar-thin border-1 border-gray-200 rounded-lg mt-4"
  >
    ...
    <div v-if="variableList" :style="{ order: priority === 1 ? 2 : 1 }">
      ...
      >
        <template #renderItem="{ item, index }">
          ...
          >
            <a-list-item-meta class="cv-variable-list-item">
              <template #title>
                <div class="flex items-center gap-x-1 justify-between">
                  ...
                  <NcButton size="small" type="text" class="cv-variable-list-item-use-field-btn !h-7 px-3 !text-small invisible">
...
:deep(.ant-list-item) {
  ...
  &:not(:has(.cv-variable-list-item)) {
    @apply !py-[7px] !px-2;
  }
  .cv-variable-list-item {
  ...
  &:hover .cv-variable-list-item-use-field-btn {
    @apply visible;
  }
}

.formula-monaco {
  @apply rounded-md cv-scrollbar-md border-gray-200 border-1 overflow-y-auto overflow-x-hidden resize-y;
  max-height: 250px;
  min-height: 50px;
...
.prompt-wrapper {
  ...
  .cv-triangle-bottom-bar {
    @apply absolute -top-[8px] left-[50%] transform -translate-x-1/2 w-3.5 h-2 bg-transparent border-2 border-transparent !border-b-cv-bg-gray-extralight;
  }

  .prompt-input-wrapper {
    @apply relative inline-block transition-all duration-300 shadow-default border-1 rounded-lg bg-cv-bg-gray-extralight border-purple-100 z-10;

    .cv-ai-formula-helper-input {
      @apply rounded-b-lg !border-purple-100 !-m-[1px] !max-w-[calc(100%_+_2px)] !w-[calc(100%_+_2px)] !shadow-none;

      &:focus {
        @apply rounded-lg !border-cv-border-purple !shadow-selected-ai;
      }
    }
  }
}
