...
import { type SelectOptionsType, UITypes } from 'clairview-sdk'
...
const addNewOption = () => {
  ...
  nextTick(() => {
    ...
    setTimeout(() => {
      const doms = document.querySelectorAll(`.cv-col-option-select-option .cv-select-col-option-select-option`)
  ...
  onClickOutside(optionsWrapperDomRef, (e) => {
    ...
    if (
      (e.target as HTMLElement)?.closest(
        `.cv-select-option-color-picker, .cv-add-select-option-auto-suggest, .cv-kanban-stack-header-${
          option?.id || 'new-stack'
        }`,
      )
    ) {
...
<template>
  <div class="w-full">
    <div
      ...
      class="cv-col-option-select-option"
      :class="{
    ...
    >
      <template v-if="isKanbanStack">
        <div v-if="kanbanStackOption" class="flex items-center cv-select-option">
          <div class="flex items-center w-full">
            <NcDropdown
              v-model:visible="colorMenus[kanbanStackOption.index!]"
              :auto-close="false"
              overlay-class-name="cv-select-option-color-picker"
            ...
            <a-input
              ...
              class="caption !rounded-lg cv-select-col-option-select-option cv-kanban-stack-input !bg-transparent"
              data-testid="cv-kanban-stack-title-input"
              :disabled="isLoadingPredictOptions"
      ...
      <template v-else>
        ...
        <Draggable :list="renderedOptions" item-key="id" handle=".cv-child-draggable-icon" @change="syncOptions">
          <template #item="{ element, index }">
            <div class="flex py-1 items-center cv-select-option hover:bg-gray-100 group">
              <div
              ...
              >
                <div
                  v-if="!isKanban"
                  class="cv-child-draggable-icon p-2 flex cursor-pointer"
                ...
                <a-input
                  ...
                  class="caption !rounded-lg cv-select-col-option-select-option !bg-transparent"
    ...
    <div
      v-if="!isKanbanStack"
      class="cv-add-select-option-btn-wrapper flex shadow-sm"
      :class="{
    ...
    >
      <NcButton
        type="text"
        class="cv-add-select-option-btn flex-1 caption"
        size="small"
        data-testid="cv-add-select-option-btn"
      ...
      <NcTooltip v-if="isFeatureEnabled(FEATURE_FLAG.AI_FEATURES)" class="w-1/2">
        ...
        <NcButton
          type="secondary"
          theme="ai"
          class="cv-add-select-option-auto-suggest w-full caption"
          size="small"
          :bordered="false"
    ...
    <div v-else-if="!kanbanStackOption?.id" class="mt-2 pl-1">
      <NcTooltip v-if="isFeatureEnabled(FEATURE_FLAG.AI_FEATURES)" class="w-full" placement="bottom">
        ...
        <NcButton
          type="secondary"
          theme="ai"
          class="cv-add-select-option-auto-suggest caption w-full"
          size="small"
...
:deep(.cv-select-col-option-select-option) {
  @apply !truncate;

  &:not(.cv-kanban-stack-input):not(:focus):hover {
    @apply !border-transparent;
  }

  &:not(.cv-kanban-stack-input):not(:focus) {
    @apply !border-transparent;
  }
...
.cv-add-select-option-btn-wrapper {
  @apply border-1 border-cv-border-gray-medium rounded-lg overflow-hidden;

  .cv-add-select-option-btn {
    @apply rounded-none;
  }
  .cv-add-select-option-auto-suggest {
    @apply -my-[1px] h-[34px] rounded-none !border-l-1 !border-l-cv-border-gray-medium;
  }
}
</style>
