...
import { UITypes, UITypesName, isAIPromptCol } from 'clairview-sdk'
  ...
  <div class="flex flex-col gap-4">
      ...
      <a-form-item class="flex items-center">
        <NcTooltip :disabled="!(richMode || (isPvColumn && !isEnabledGenerateText))" class="flex items-center">
          ...
          <NcSwitch
            ...
            class="cv-ai-field-generate-text cv-ai-input"
            @change="handleDisableSubmitBtn"
          >
            <span
              class="text-sm font-semibold pl-1"
              :class="{
                'text-cv-content-purple-dark': isEnabledGenerateText,
                'text-cv-content-gray': !isEnabledGenerateText,
              }"
            >
        ...
        <NcTooltip class="ml-2 mr-[40px] flex cursor-pointer">
          ...
          <GeneralIcon icon="info" class="text-cv-content-gray-muted hover:text-cv-content-gray-subtle opacity-70 w-3.5 h-3.5" />
        </NcTooltip>
    ...
    <template v-if="isPromptEnabled && (!isEdit ? aiIntegrationAvailable && isEnabledGenerateText : isEnabledGenerateText)">
      <a-form-item class="flex">
        <div class="cv-prompt-input-wrapper bg-cv-bg-gray-light rounded-lg w-full">
          <AiPromptWithFields
            v-model="vModel.prompt_raw"
            :options="availableFields"
            :read-only="!aiIntegrationAvailable"
            placeholder="Write custom AI Prompt instruction here"
            prompt-field-tag-class-name="!text-cv-content-purple-dark font-weight-500"
            suggestion-icon-class-name="!text-cv-content-purple-medium"
          />
          <div class="rounded-b-lg flex items-center gap-1.5 p-1">
            <GeneralIcon icon="info" class="!text-cv-content-purple-medium w-3.5 h-3.5" />
            <span class="text-xs text-cv-content-gray-subtle2"
              >Mention fields using curly braces, e.g. <span class="text-cv-content-purple-dark">{Field name}</span>.</span
            >
          </div>
        </div>
      </a-form-item>
      <div v-if="aiIntegrationAvailable && isEnabledGenerateText" class="cv-ai-options-preview overflow-hidden">
        <div>
          <div
            class="flex items-center gap-2 transition-all duration-300"
            :class="{
              'pl-3 py-2 pr-2': !isAlreadyGenerated,
              'pl-3 py-1 pr-1 border-b-1 border-cv-border-gray-medium': isAlreadyGenerated,
            }"
          >
            <div class="flex flex-col flex-1 gap-1">
              <div class="flex items-center gap-2">
                <span class="text-sm font-bold text-cv-content-gray-subtle">Preview</span>
                <NcTooltip class="flex cursor-pointer">
                  <template #title> Preview is generated using the first record in this table</template>
                  <GeneralIcon
                    icon="info"
                    class="text-cv-content-gray-muted hover:text-cv-content-gray-subtle opacity-70 w-3.5 h-3.5"
                  />
                </NcTooltip>
              </div>
              <span v-if="!isAlreadyGenerated" class="text-[11px] leading-[18px] text-cv-content-gray-muted">
            ...
            <NcTooltip :disabled="isPreviewEnabled">
              ...
              <NcButton
                class="cv-aioptions-preview-generate-btn"
                :class="{
                  'cv-is-already-generated': isAlreadyGenerated,
                  'cv-preview-enabled': isPreviewEnabled,
                }"
                size="xs"
              ...
              >
                <div
                  :class="{
                    'cv-animate-dots min-w-[91px] text-left': generatingPreview,
...
.cv-ai-options-preview {
  @apply rounded-lg border-1 border-cv-border-gray-medium;
  box-shadow: 0px 0px 4px 0px rgba(0, 0, 0, 0.08);

  :deep(.cv-text-area-expand-btn) {
    @apply right-1;
  }
}

.cv-aioptions-preview-generate-btn {
  &:not(.cv-is-already-generated) {
    &.cv-preview-enabled {
