    ...
    <template v-if="props.advancedDbOptions">
      <div class="flex justify-between w-full gap-1">
        ...
        <a-form-item :disabled="sqlUi.colPropUNDisabled(vModel) || !sqlUi.columnEditable(vModel)" @change="onAlter">
          ...
          <a-checkbox v-model:checked="vModel.un" class="cv-column-checkbox-UN" />
        </a-form-item>
        ...
        <a-form-item :disabled="sqlUi.colPropAuDisabled(vModel) || !sqlUi.columnEditable(vModel)" @change="onAlter">
          ...
          <a-checkbox v-model:checked="vModel.au" class="cv-column-checkbox-AU" />
        </a-form-item>
      </div>

      <a-form-item :label="$t('labels.databaseType')" v-bind="validateInfos.dt">
        <a-select v-model:value="vModel.dt" dropdown-class-name="cv-dropdown-db-type" @change="onDataTypeChange">
          <template #suffixIcon>
          ...
          <a-select-option v-for="type in dataTypes" :key="type" :value="type">
            <div class="flex gap-2 items-center justify-between">
              {{ type }}
              <component :is="iconMap.check" v-if="vModel.dt === type" id="cv-selected-item-icon" class="text-primary w-4 h-4" />
            </div>
          </a-select-option>
        </a-select>
      </a-form-item>
