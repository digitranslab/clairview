...
import { UITypes, UITypesName, readonlyMetaAllowedTypes } from 'clairview-sdk'
...
const handleAutoScrollOption = () => {
  const option = document.querySelector('.cv-column-list-option-active')
...
<template>
  <div
    ...
    data-testid="cv-column-uitypes-options-list-wrapper"
  ...
  >
    <div class="w-full pb-2 px-2" @click.stop>
      <a-input
        ...
        class="cv-column-type-search-input cv-toolbar-dropdown-search-field-input"
        @keydown.enter.stop="handleKeydownEnter"
        @change="activeFieldIndex = 0"
      >
        <template #prefix> <GeneralIcon icon="search" class="cv-search-icon h-4 w-4 mr-1" /> </template>
      </a-input>
    </div>
    <div class="cv-column-list-wrapper flex-col w-full max-h-[290px] cv-scrollbar-thin !overflow-y-auto px-2">
      ...
      >
        <div
          ...
          :class="[
            `cv-column-list-option-${index}`,
            {
              'hover:bg-gray-100 cursor-pointer': !isDisabledUIType(option.name),
              'bg-gray-100 cv-column-list-option-active': activeFieldIndex === index && !isDisabledUIType(option.name),
              '!text-gray-400 cursor-not-allowed': isDisabledUIType(option.name),
              '!text-cv-content-purple-dark': option.name === 'AIButton' || option.name === 'AIPrompt',
            },
          ]"
        ...
        >
          <div class="flex flex-1 gap-2 items-center">
            ...
            <span v-if="option.isNew" class="text-sm text-cv-content-purple-dark bg-purple-50 px-2 rounded-md">{{
              $t('general.new')
            }}</span>
          </div>
