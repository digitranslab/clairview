...
import { ButtonActionsType, type ButtonType, type ColumnType, type HookType } from 'clairview-sdk'
  ...
  <div class="relative flex flex-col gap-4">
    <a-row :gutter="8">
      <a-col :span="12">
        <a-form-item v-bind="validateInfos.label" class="mt-4" :label="$t('general.label')">
          <a-input
            ...
            class="cv-column-label-input cv-input-shadow !rounded-lg"
            :class="{
      ...
      <a-col :span="6">
        <a-form-item :label="$t('general.style')" v-bind="validateInfos.theme">
          <NcDropdown v-model:visible="isDropdownOpen" class="cv-color-picker-dropdown-trigger">
            <div
              :class="{
                'cv-button-style-dropdown': isDropdownOpen,
                '!border-cv-border-purple !shadow-selected-ai': isDropdownOpen && isAiMode,
            ...
            >
              <div
                ...
                class="flex items-center justify-center cv-cell-button rounded-md h-6 w-6 gap-2"
              >
      ...
      <a-col :span="6">
        <a-form-item :label="$t('labels.icon')" v-bind="validateInfos.icon">
          <NcDropdown v-model:visible="isButtonIconDropdownOpen" class="cv-color-picker-dropdown-trigger">
            <div
              :class="{
                'cv-button-style-dropdown ': isButtonIconDropdownOpen,
                '!border-cv-border-purple !shadow-selected-ai': isButtonIconDropdownOpen && isAiMode,
              ...
              <div class="bg-white w-80 space-y-3 h-70 overflow-y-auto rounded-lg">
                <div class="!sticky top-0 flex gap-2 bg-white px-2 py-2">
                  <a-input
                    ...
                    class="cv-dropdown-search-unified-input z-10 cv-input-shadow"
                    :class="{
                      'cv-ai-input': isAiMode,
                    }"
                  >
                    <template #prefix> <GeneralIcon icon="search" class="cv-search-icon h-3.5 w-3.5 mr-1" /> </template
                  ></a-input>
                ...
                <div class="grid px-3 auto-rows-max pb-2 cv-scrollbar-md gap-3 grid-cols-10">
                  <component
                    :is="icon"
      ...
      <a-col :span="24">
        <a-form-item :label="$t('labels.onClick')" v-bind="validateInfos.type">
          <a-select
            ...
            class="w-52 cv-button-type-select cv-select-shadow"
            :class="{
              'cv-ai-input': isAiMode,
            }"
            dropdown-class-name="cv-dropdown-button-cell-type"
    ...
    <a-form-item v-if="vModel?.type === buttonActionsType.Webhook">
      <div class="mb-2 text-gray-800 text-[13px] flex justify-between">
        ...
        <a
          class="font-medium"
          href="https://docs.clairview.com/fields/field-types/custom-types/button#create-a-button-field"
          target="_blank"
        >
          Docs
        </a>
      ...
      <div class="flex rounded-lg">
        <NcDropdown v-model:visible="isWebHookSelectionDropdownOpen" :trigger="['click']">
          ...
          <div
            :class="{
              'cv-button-style-dropdown shadow-dropdown-open remove-right-shadow': isWebHookSelectionDropdownOpen,
            }"
            class="cv-button-webhook-select border-r-0 flex items-center justify-center border-1 h-8 px-[8px] border-gray-300 !w-full transition-all cursor-pointer !rounded-l-lg"
          >
        ...
        <NcButton
          size="small"
          type="secondary"
          class="!rounded-l-none border-l-[#d9d9d9] !hover:bg-white cv-button-style-dropdown"
          :class="{
            'cv-button-style-dropdown shadow-dropdown-open remove-left-shadow': isWebHookSelectionDropdownOpen,
          }"
          @click="editWebhook"
        >
          <GeneralIcon
