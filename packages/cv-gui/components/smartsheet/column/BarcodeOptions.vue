...
import { AllowedColumnTypesForQrAndBarcodes, isVirtualCol } from 'clairview-sdk'
...
<template>
  <div class="flex flex-col gap-4">
    <a-form-item
      class="flex pb-2 cv-barcode-value-column-select flex-row"
      ...
      <div class="flex flex-row items-center">
        ...
        >
          ...
          <a-select-option v-for="(option, index) of columnsAllowedAsBarcodeValue" :key="index" :value="option.id">
            <div class="w-full flex gap-2 truncate items-center justify-between" :data-testid="`cv-barcode-${option.title}`">
    ...
    <a-form-item class="flexp cv-barcode-format-select" :label="$t('general.format')" v-bind="validateInfos.barcode_format">
      <a-select
