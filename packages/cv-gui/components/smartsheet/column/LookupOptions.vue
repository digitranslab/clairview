...
import type { ColumnType, LinkToAnotherRecordType, LookupType, TableType } from 'clairview-sdk'
import { UITypes, isLinksOrLTAR, isSystemColumn, isVirtualCol } from 'clairview-sdk'
  ...
  <div v-if="refTables.length" class="w-full flex flex-row space-x-2">
    ...
    >
      <a-select
        ...
        placeholder="-select-"
        dropdown-class-name="!w-64 !rounded-md cv-dropdown-relation-table"
        ...
        <a-select-option v-for="(table, i) of refTables" :key="i" :value="table.col.fk_column_id">
          <div class="flex gap-2 w-full justify-between truncate items-center">
            ...
            <div class="inline-flex items-center truncate gap-2">
              <div class="text-[0.65rem] leading-4 flex-1 truncate text-gray-600 cv-relation-details">
    ...
    >
      <a-select
        ...
        dropdown-class-name="cv-dropdown-relation-column !rounded-md"
...
<style scoped>
:deep(.ant-select-selector .ant-select-selection-item .cv-relation-details) {
  @apply hidden;
}
</style>
