...
import { type ColumnReqType, type ColumnType, isAIPromptCol, isSupportedDisplayValueColumn } from 'clairview-sdk'
import {
  ButtonActionsType,
  UITypes,
  UITypesName,
...
<template>
  <div
    v-if="!warningVisible"
    ref="editOrAddRef"
    class="overflow-auto cv-scrollbar-md"
    :class="{
      ...
      '!bg-cv-bg-gray-extralight': aiAutoSuggestMode && formState.uidt && !props.fromTableExplorer,
    ...
    >
      <template v-if="!isEdit && !props.fromTableExplorer && (aiAutoSuggestMode || !formState.uidt)">
        ...
        >
          <div class="flex items-center gap-3">
            <div class="flex-1 text-base font-bold text-cv-content-gray">New Field</div>
            <div
              :class="{
            ...
            >
              <NcButton
                ...
                type="text"
                size="small"
                class="-my-1.5 !text-cv-content-purple-dark hover:text-cv-content-purple-dark"
                :class="{
                  '!pointer-events-none !cursor-not-allowed': aiLoading,
                  '!bg-cv-bg-purple-dark hover:!bg-gray-100': aiAutoSuggestMode,
                }"
          ...
          <template v-if="aiAutoSuggestMode">
                ...
                <div class="px-5 pt-4 pb-2">
                  ...
                  <div v-else-if="aiAutoSuggestModeStep === 'init'">
                    <div class="text-cv-content-purple-light text-sm h-7 flex items-center gap-2">
                      ...
                      <!-- Todo: add table name  -->
                      <div class="cv-animate-dots">Auto suggesting fields for {{ meta?.title }}</div>
                    </div>
                  </div>
                        ...
                        <template v-for="f of activeTabPredictedFields" :key="f.title">
                          <NcTooltip :disabled="selected.length < maxSelectionCount || f.selected">
                            ...
                            <a-tag
                              ...
                              :class="{
                                'cv-disabled': saving || (!f.selected && selected.length >= maxSelectionCount),
                                'cv-selected': f.selected,
                                'cv-bg-selected': activeSelectedField === f.ai_temp_id,
                              }"
            ...
            <div
              ...
              class="w-full p-4 flex items-start gap-4 border-1 border-cv-border-gray-medium rounded-lg"
            >
              <GeneralIcon icon="ncInfoSolid" class="flex-none text-cv-content-red-dark" />
              <div class="flex flex-col gap-1">
                <div class="text-cv-content-gray text-base font-bold">Failed to add fields</div>
                <div class="text-cv-content-gray-muted text-sm">
                  ClairView was unable to add {{ predicted.length }} fields to the table. Please retry adding the fields.
                </div>
              </div>
        ...
        <div
          ...
          :class="{
            'pb-5 border-b-1 border-b-cv-border-gray-medium': formState.uidt,
          }"
        >
          <a-form-item>
            <div class="flex gap-x-2 justify-end">
              ...
              <NcButton
                ...
                data-testid="cv-field-modal-submit-btn"
      ...
      <a-form-item v-if="isFieldsTab" v-bind="validateInfos.title" class="flex">
        ...
        >
          <input
            ...
            class="flex flex-grow cv-fields-input cv-input-shadow text-sm font-semibold outline-none bg-inherit min-h-6"
            :class="{
              'cv-ai-input': isAiMode,
            }"
      ...
      >
        <a-input
          ...
          class="cv-column-name-input cv-input-shadow !rounded-lg"
          :class="{
            'cv-ai-input': isAiMode,
          }"
          :placeholder="`${$t('objects.field')} ${$t('general.name').toLowerCase()} ${isEdit ? '' : $t('labels.optional')}`"
        ...
        >
          <NcTooltip :disabled="!(!isEdit && formState.uidt && !!formState?.ai_temp_id)">
            ...
            <a-select
              ...
              show-search
              class="cv-column-type-input cv-select-shadow !rounded-lg"
              :class="{
                'cv-ai-input': isAiMode,
              ...
              "
              dropdown-class-name="cv-dropdown-column-type border-1 !rounded-lg border-gray-200"
            ...
            >
              ...
              <a-select-option
                ...
                :class="{
                  ...
                  '!text-cv-content-purple-dark': [AIPrompt, AIButton].includes(opt.name),
                }"
                ...
                >
                  ...
                  <div class="flex-1 flex gap-2 items-center max-w-[calc(100%_-_24px)]">
                    <component
                      :is="
                        isAiButtonSelectOption(opt.name) && !isColumnTypeOpen
                          ...
                          : opt.icon
                      "
                      class="cv-field-type-icon w-4 h-4 !opacity-90 text-current"
                    />
                    <div class="flex-1">
                    ...
                    <span
                      ...
                      class="cv-new-field-badge text-sm text-cv-content-purple-dark bg-purple-50 px-2 rounded-md font-normal"
                      >{{ $t('general.new') }}</span
                    >
                  </div>
                  <component
                    ...
                    :class="{
                      ...
                      'text-cv-content-purple-medium': isAiMode,
                    }"
                  />
                </NcTooltip>
              </a-select-option>
      ...
      <a-form-item v-if="enableDescription && aiAutoSuggestMode">
        ...
        <a-textarea
          ...
          class="cv-input-sm cv-input-text-area cv-input-shadow !text-gray-800 px-3 !max-h-[300px]"
          hide-details
      ...
      <template v-if="formState.uidt">
        <div v-if="formState.meta && columnToValidate.includes(formState.uidt)" class="flex items-center gap-1">
          <NcSwitch v-model:checked="formState.meta.validate" size="small" class="cv-switch">
        ...
        <template v-if="!readOnly && isFullUpdateAllowed">
          <div class="cv-column-options-wrapper flex flex-col gap-4">
            <!--
          ...
          "
            ...
            <div
              ...
              class="flex gap-1"
            >
              <NcSwitch v-model:checked="formState.unique" size="small" class="cv-switch">
          ...
          >
            <div
              class="cv-more-options flex items-center gap-1 cursor-pointer select-none"
        ...
        <template v-else>
          <div
            ...
            :class="{
              'border-t-1 border-cv-border-gray-medium pt-3': isScrollEnabled,
            }"
          >
...
<style lang="scss">
.cv-dropdown-column-type {
  .ant-select-item-option-active-selected {
    @apply !bg-gray-100;
  }
}

.cv-edit-or-add-provider-wrapper .cv-ai-mode {
  .cv-fields-input,
  .cv-column-name-input {
  }
}
</style>
...
:deep(.ant-select.cv-column-type-input) {
  .cv-new-field-badge {
    @apply hidden;
  }
}
...
:deep(.cv-color-picker-dropdown-trigger),
:deep(.cv-default-value-wrapper) {
...
.cv-column-options-wrapper {
  &:empty {
    @apply hidden;
  }
}

.cv-field-modal-ai-toggle-btn {
