...
<template>
  <div class="flex flex-col mt-2 pt-1 border-t-1 border-gray-50 px-2 cv-view-sidebar-webhook">
    <div
      v-e="['c:actions:webhook']"
      class="py-2 flex gap-2 items-center justify-between button text-gray-700"
      @click="openEditor()"
    >
      <div class="flex flex-row items-center gap-x-2 cv-view-sidebar-webhook-label">
        <component :is="iconMap.hook" class="text-gray-600" />
        {{ $t('objects.webhooks') }}
      </div>
      <div class="flex !rounded-md hover:(bg-gray-50 text-black) p-1 cursor-pointer">
        <component :is="iconMap.plus" class="text-primary-600 cv-view-sidebar-webhook-plus-icon" />
      </div>
    </div>
    ...
    <div v-else class="flex flex-col ml-6">
      <div
        v-for="hook in hooks"
        :key="hook.id"
        class="flex flex-row items-center hover:bg-gray-50 rounded-md px-1.5 py-2.5 cursor-pointer group cv-view-sidebar-webhook-item"
      >
        ...
        <div class="flex">
          <a-dropdown placement="bottom" :trigger="['click']">
            <div
              class="cv-docs-sidebar-page-options px-0.5 hover:(!bg-gray-300 !bg-opacity-30 rounded-md) cursor-pointer select-none"
  ...
  <GeneralModal v-model:visible="showEditModal" width="48rem" destroy-on-close>
    <div class="py-6">
      <div class="webhook-scroll px-5 cv-drawer-webhook-body">
        <!--
