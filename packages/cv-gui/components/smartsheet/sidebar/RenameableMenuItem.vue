...
import type { KanbanType, ViewType, ViewTypes } from 'clairview-sdk'
  ...
  >
    <div v-e="['a:view:open', { view: vModel.type }]" class="text-xs flex items-center w-full gap-1" data-testid="view-item">
      <div class="flex min-w-6" :data-testid="`view-sidebar-drag-handle-${vModel.alias || vModel.title}`">
        ...
        >
          <template #default>
            <GeneralViewIcon :meta="props.view" class="cv-view-icon" />
          </template>
        </LazyGeneralEmojiPicker>
      </div>
        ...
        <NcDropdown v-model:visible="isDropdownOpen" overlay-class-name="!rounded-lg">
          ...
          >
            <NcButton
              size="xxsmall"
              type="text"
              class="cv-view-sidebar-node-context-btn !px-1 !border-none !bg-inherit !tr !hover:text-gray-900"
          ...
          <template #overlay>
            <NcMenu :data-testid="`view-sidebar-view-actions-${vModel.alias || vModel.title}`">
              ...
              <NcMenuItem size="small" :centered="false" class="cv-view-copy-icon" @click.stop="onDuplicate">
              ...
              <template v-if="!vModel.is_default">
                <NcDivider />
                <NcMenuItem size="small" class="cv-view-delete-icon !text-red-600 !hover:bg-red-50" @click.stop="onDelete">
