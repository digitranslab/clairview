...
import type { ColumnReqType, ColumnType } from 'clairview-sdk'
...
<template>
  <a-dropdown
    ...
    overlay-class-name="cv-dropdown-form-column-operations !border-1 rounded-lg !shadow-xl"
    @click.stop="isOpen = !isOpen"
  >
    <NcButton
      type="secondary"
      size="small"
      class="cv-form-add-field"
      data-testid="cv-form-add-field"
      @click.stop="showAddColumnDropdown = true"
    >
      <component :is="iconMap.threeDotVertical" class="flex-none w-4 h-4" />
    </NcButton>
    <template #overlay>
      <NcMenu class="flex flex-col gap-1 border-gray-200 cv-column-options">
        ...
        <NcMenuItem v-if="false" :disabled="!isDuplicateAllowed" @click="openDuplicateDlg">
          <div class="cv-column-duplicate cv-form-header-menu-item">
        ...
        <NcMenuItem :disabled="isRequired" @click="hideField">
          <div class="cv-column-insert-before cv-form-header-menu-item">
        ...
        <template v-if="!column?.pv">
          ...
          <NcMenuItem :disabled="!isDeleteAllowed" class="!hover:bg-red-50" @click="handleDelete">
            <div class="cv-column-delete cv-form-header-menu-item text-red-600">
  ...
  <SmartsheetHeaderDeleteColumnModal
    ...
    class="cv-form-column-delete-dropdown"
