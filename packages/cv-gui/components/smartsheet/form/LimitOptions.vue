...
import type { ColumnType, SelectOptionType, SelectOptionsType, UserFieldRecordType } from 'clairview-sdk'
...
<template>
  <div class="w-full h-full cv-col-select-option cv-form-scrollbar">
    <div v-if="vModel.length > 12">
      <a-input
        v-model:value="searchQuery"
        class="!h-9 !px-3 !py-1 !rounded-lg mb-2"
        :placeholder="`${$t('placeholder.searchOptions')}...`"
        name="cv-form-field-limit-option-search-input"
        data-testid="cv-form-field-limit-option-search-input"
      >
        <template #prefix>
    ...
    <Draggable
      ...
      :model-value="vModel"
      item-key="id"
      handle=".cv-child-draggable-icon"
      ghost-class="cv-form-field-limit-option-ghost"
      class="rounded-b-lg border-1 border-gray-200 !max-h-[224px] overflow-y-auto cv-form-scrollbar"
      @change="onMove($event)"
      @start="drag = true"
      @end="drag = false"
    >
      <template #item="{ element }">
        <div
          ...
          :class="[
            `cv-form-field-${column.title?.replaceAll(' ', '')}-limit-option-${element.title?.replaceAll(' ', '')}`,
            `${element.show ? 'hover:bg-gray-50' : 'bg-gray-100'}`,
          ]"
          :data-testid="`cv-form-field-${column.title?.replaceAll(' ', '')}-limit-option-${element.title?.replaceAll(' ', '')}`"
        >
          <component :is="iconMap.drag" class="cv-child-draggable-icon flex-none cursor-move !h-4 !w-4 text-gray-600" />
...
.cv-form-field-limit-option-ghost {
  @apply bg-gray-50;
}
</style>
