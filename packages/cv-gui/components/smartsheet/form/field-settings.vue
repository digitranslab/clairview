...
import { UITypes, isSelectTypeCol } from 'clairview-sdk'
...
<template>
  ...
  <template v-if="activeField">
    <div class="cv-form-field-settings p-4 flex flex-col gap-4 border-b border-gray-200">
      <div class="text-sm font-bold text-gray-800">{{ $t('objects.field') }} {{ $t('activity.validations').toLowerCase() }}</div>
      <div class="flex flex-col gap-6">
        <div class="flex items-center justify-between gap-3">
          <div
            class="cv-form-input-required text-gray-800 font-medium"
            @click="
              () => {
          ...
          <a-switch
            ...
            size="small"
            data-testid="cv-form-input-required"
            @change="updateColMeta(activeField)"
          />
        </div>

        <div v-if="columnSupportsScanning(activeField.uidt)" class="!my-0 cv-form-input-enable-scanner-form-item">
          <div class="flex items-center justify-between gap-3">
            <div class="cv-form-input-enable-scanner text-gray-800 font-medium">
              {{ $t('general.enableScanner') }}
            </div>
            <a-switch
              v-model:checked="activeField.enable_scanner"
              v-e="['a:form-view:field:mark-enable-scanner']"
              data-testid="cv-form-input-enable-scanner"
              size="small"
              @change="updateColMeta(activeField)"
            />
          </div>
        ...
        <div v-if="isSelectTypeCol(activeField.uidt)" class="w-full flex items-start justify-between gap-3">
          ...
          <a-switch
            ...
            size="small"
            class="flex-none cv-form-switch-focus"
            @change="updateColMeta(activeField)"
          />
        </div>
      </div>
    ...
    <div
      ...
      class="cv-form-field-appearance-settings p-4 flex flex-col gap-4 border-b border-gray-200"
    >
      ...
      <div class="flex flex-col gap-6">
        ...
        <div>
          ...
          <a-radio-group
            ...
            class="cv-form-field-layout !mt-3 max-w-[calc(100%_-_40px)]"
