...
<template>
  <div
    :class="[
      `cv-cell-${(column?.uidt || 'default').toLowerCase()}`,
      {
        'cv-display-value-cell': isPrimary(column) && !props.virtual && !isForm && !isCalendar,
        'cv-grid-numeric-cell-right':
          isGrid &&
          isNumericField &&
          !isEditColumnMenu &&
          !isForm &&
        ...
        'cv-grid-numeric-cell-left': (isForm && isNumericField && isExpandedFormOpen) || isEditColumnMenu,
        '!min-h-30': isTextArea(column) && (isForm || isSurveyForm),
        'cv-cell-longtext-ai': cellType === 'ai',
      },
    ]"
    class="cv-cell w-full h-full relative"
