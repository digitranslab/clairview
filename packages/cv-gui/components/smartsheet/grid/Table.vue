...
import type { ButtonType, ColumnReqType, ColumnType, PaginatedType, TableType, ViewType } from 'clairview-sdk'
import {
  UITypes,
  ViewTypes,
  isAIPromptCol,
...
const gridWrapperClass = computed<string>(() => {
  ...
  if (headerOnly.value !== true) {
    if (!scrollParent.value) {
      classes.push('cv-scrollbar-x-lg !overflow-auto')
    }
  } else {
...
const {
  ...
  async (e: KeyboardEvent) => {
    ...
    const activeDropdownEl = document.querySelector(
      '.cv-dropdown-single-select-cell.active,.cv-dropdown-multi-select-cell.active',
    )
    if (activeDropdownEl) {
      e.preventDefault()
      return true
...
onClickOutside(tableBodyEl, (e) => {
  ...
  const activePickerOrDropdownEl = document.querySelector(
    '.cv-picker-datetime.active,.cv-dropdown-single-select-cell.active,.cv-dropdown-multi-select-cell.active,.cv-dropdown-user-select-cell.active,.cv-picker-date.active,.cv-picker-year.active,.cv-picker-time.active,.cv-link-dropdown-root',
  )
  if (
    e.target &&
...
useEventListener(document, 'mousedown', (e) => {
  ...
  if ((e.target as HTMLElement).closest('.cv-grid-cell:not(.caption)')) {
...
useEventListener(document, 'keyup', async (e: KeyboardEvent) => {
  ...
  const activeDropdownEl = document.querySelector('.cv-dropdown-single-select-cell.active,.cv-dropdown-multi-select-cell.active')
...
<template>
  <div class="flex flex-col" :class="`${headerOnly !== true ? 'h-full w-full' : ''}`">
    ...
    <div ref="gridWrapper" class="cv-grid-wrapper min-h-0 flex-1 relative" :class="gridWrapperClass">
      <div
      ...
      <NcDropdown
        ...
        overlay-class-name="cv-dropdown-grid-context-menu"
      >
        <div>
          <table
            class="xc-row-table cv-grid backgroundColorDefault !h-auto bg-white sticky top-0 z-5 bg-white"
            :class="{
              mobile: isMobileMode,
          ...
          >
            <thead v-show="hideHeader !== true" ref="tableHeadEl">
              ...
              <tr v-show="!isViewColumnsLoading" class="cv-grid-header">
                <th
                ...
                >
                  <div class="w-full h-full flex pl-2 pr-1 items-center" data-testid="cv-check-all">
                    <template v-if="!readOnly && !hideCheckbox">
                      <div class="cv-no-label text-gray-500" :class="{ hidden: vSelectedAllRecords }">#</div>
                      <div
                        :class="{
                          hidden: !vSelectedAllRecords,
                          flex: vSelectedAllRecords,
                        }"
                        class="cv-check-all w-full items-center"
                      >
                ...
                >
                  <div class="absolute top-0 left-0 h-8 border-b-1 border-r-1 border-gray-200 cv-grid-add-edit-column group">
                    <a-dropdown
                      v-model:visible="addColumnDropdown"
                      :trigger="['click']"
                      overlay-class-name="cv-dropdown-grid-add-column"
                    ...
                    >
                      <div class="h-full w-[60px] flex items-center justify-center">
                        ...
                        <component :is="iconMap.plus" class="text-base cv-column-add text-gray-500 !group-hover:text-black" />
                      </div>
          ...
          <div
            v-if="!showSkeleton"
            class="table-overlay"
            :class="{ 'cv-grid-skeleton-loader': showSkeleton }"
            :style="{
          ...
          >
            <table
              ref="smartTable"
              class="xc-row-table cv-grid backgroundColorDefault !h-auto bg-white relative"
              :class="{
                'mobile': isMobileMode,
                'desktop': !isMobileMode,
            ...
            >
              <tbody v-if="headerOnly !== true" ref="tableBodyEl">
                ...
                <LazySmartsheetRow v-for="{ row, index: rowIndex } in visibleData" :key="rowIndex" :row="row">
                  <template #default="{ state }">
                    ...
                    >
                      <td
                        key="row-index"
                        class="caption cv-grid-cell w-[64px] min-w-[64px]"
                        :style="{
                          left: `-${leftOffset}px`,
                        }"
                      ...
                      >
                        <div class="w-[60px] pl-2 pr-1 items-center flex gap-1">
                          <div
                            class="cv-row-no sm:min-w-4 text-xs text-gray-500"
                          ...
                          <div
                            ...
                            class="cv-row-expand-and-checkbox"
                          >
                          ...
                          <div
                            class="cv-expand"
                            :data-testid="`cv-expand-${rowIndex}`"
                            :class="{ 'cv-comment': row.rowMeta?.commentCount }"
                          >
                            <a-spin
                            ...
                            <div
                              ...
                              class="cursor-pointer flex items-center border-1 border-gray-100 active:ring rounded-md p-1 hover:(bg-white border-cv-border-gray-medium)"
                            >
                              <component
                                :is="iconMap.maximize"
                                v-if="expandForm"
                                v-e="['c:row-expand:open']"
                                class="select-none cv-row-expand opacity-90 w-4 h-4"
                                @click="expandAndLooseFocus(row, state)"
                              />
                            </div>
                          </div>
                      ...
                      <SmartsheetTableDataCell
                        v-if="fields[0]"
                        :key="fields[0].id"
                        class="cell relative cv-grid-cell cursor-pointer"
                        :class="{
                          ...
                          'cv-required-cell': cellMeta[rowIndex][0].isColumnRequiredAndNull && !isPublicView,
                      ...
                      <SmartsheetTableDataCell
                        ...
                        :class="{
                          ...
                          'cv-required-cell': cellMeta[rowIndex][colIndex].isColumnRequiredAndNull && !isPublicView,
                          'align-middle': !rowHeight || rowHeight === 1,
                          'align-top': rowHeight && rowHeight !== 1,
                ...
                <tr
                  ...
                  class="text-left cv-grid-add-new-cell cursor-pointer group relative z-3 xs:hidden"
                  :class="{
                ...
                >
                  <td
                    class="cv-grid-add-new-cell-item h-8 border-b-1 border-gray-100 bg-white group-hover:bg-gray-50 absolute left-0 bottom-0 px-2 sticky z-40 w-full flex items-center text-gray-500"
                    :style="{
                      left: `-${leftOffset}px`,
                    }"
                  >
        ...
        <template #overlay>
          <NcMenu class="!rounded !py-0" @click="contextMenu = false">
            ...
            <NcMenuItem
              ...
              class="cv-base-menu-item !text-red-600 !hover:bg-red-50"
              data-testid="cv-delete-row"
            ...
            <template v-if="contextMenuTarget && selectedRange.isSingleCell() && isUIAllowed('commentEdit') && !isMobileMode">
              <NcDivider />
              <NcMenuItem class="cv-base-menu-item" @click="commentRow(contextMenuTarget.row)">
    ...
    <div class="relative">
      <LazySmartsheetPagination
        ...
        :key="`cv-pagination-${isMobileMode}`"
      ...
      >
        <template v-if="isAddingEmptyRowAllowed && !showSkeleton" #add-record>
          <div class="flex ml-1">
            ...
            >
              <div data-testid="cv-pagination-add-record" class="flex items-center px-2 text-gray-600 hover:text-black">
                <span>
              ...
              <template #overlay>
                <div class="relative overflow-visible min-h-17 w-10">
                  ...
                  >
                    <div
                      v-e="['c:row:add:grid']"
                      class="px-4 py-3 flex flex-col select-none gap-y-2 cursor-pointer rounded-md hover:bg-gray-100 text-gray-600 cv-new-record-with-grid group"
                      @click="onNewRecordToGridClick"
                    >
                      <div class="flex flex-row items-center justify-between w-full">
                        <div class="flex flex-row items-center justify-start gap-x-3">
                          <component :is="viewIcons[ViewTypes.GRID]?.icon" class="cv-view-icon text-inherit" />
                    ...
                    <div
                      v-e="['c:row:add:form']"
                      class="px-4 py-3 flex flex-col select-none gap-y-2 cursor-pointer rounded-md hover:bg-gray-100 text-gray-600 cv-new-record-with-form group"
    ...
    <div v-if="headerOnly !== true && paginationDataRef && !isGroupBy" class="absolute bottom-12 left-2" @click.stop>
      <NcDropdown v-if="isAddingEmptyRowAllowed && !showSkeleton">
        <div class="flex shadow-cv-sm rounded-lg">
          <NcButton
            v-if="isMobileMode"
            v-e="[isAddNewRecordGridMode ? 'c:row:add:grid' : 'c:row:add:form']"
            :disabled="isPaginationLoading"
          ...
          <NcButton
            ...
            v-e="[isAddNewRecordGridMode ? 'c:row:add:grid' : 'c:row:add:form']"
            :disabled="isPaginationLoading"
            class="!rounded-r-none !border-r-0 cv-grid-add-new-row"
            size="small"
            type="secondary"
            :shadow="false"
            @click="isAddNewRecordGridMode ? addEmptyRow() : onNewRecordToFormClick()"
          >
            <div data-testid="cv-pagination-add-record" class="flex items-center gap-2">
              <GeneralIcon icon="plus" />
              <template v-if="isAddNewRecordGridMode">
                {{ $t('activity.newRecord') }}
              </template>
          ...
          <NcButton
            v-if="!isMobileMode"
            size="small"
            class="!rounded-l-none cv-add-record-more-info"
            type="secondary"
            :shadow="false"
          >
        ...
        <template #overlay>
          <NcMenu>
            <NcMenuItem v-e="['c:row:add:grid']" class="cv-new-record-with-grid group" @click="onNewRecordToGridClick">
              <div class="flex flex-row items-center justify-start gap-x-3">
                <component :is="viewIcons[ViewTypes.GRID]?.icon" class="cv-view-icon text-inherit" />
                {{ $t('activity.newRecord') }} - {{ $t('objects.viewType.grid') }}
              </div>
            ...
            <NcMenuItem v-e="['c:row:add:form']" class="cv-new-record-with-form group" @click="onNewRecordToFormClick">
              <div class="flex flex-row items-center justify-start gap-x-3">
                <GeneralIcon class="h-4.5 w-4.5" icon="article" />
                {{ $t('activity.newRecord') }} - {{ $t('objects.viewType.form') }}
              </div>
...
<style lang="scss">
.cv-grid-pagination-wrapper .ant-dropdown-button {
  > .ant-btn {
...
.cv-grid-wrapper {
  ...
  .cv-grid-add-edit-column {
    @apply bg-gray-50;
  }

  .cv-grid-add-new-cell:hover td {
    @apply text-black !bg-gray-50;
  }

  td:not(.cv-grid-add-new-cell-item),
  th {
  ...
  th {
    ...
    :deep(.cv-cell-icon),
    :deep(.cv-virtual-cell-icon) {
      @apply !w-3.5 !h-3.5 !text-small;
    }
  }

  .cv-grid-header th:last-child {
    @apply !border-b-1;
  }
  ...
  td:not(:first-child):not(.cv-grid-add-new-cell-item) {
    @apply px-3;

    &.align-top {
      @apply py-2;

      &:has(.cv-cell.cv-cell-longtext textarea) {
        @apply py-0 pr-0;
      }
    }
    ...
    :deep(.cv-virtual-cell) {
      ...
      &.cv-cell-longtext .long-text-wrapper .cv-rich-text-grid {
        @apply pl-0 -ml-1;
      }
  ...
  tbody td:not(.cv-grid-add-new-cell-item):nth-child(1) {
    position: sticky !important;
    left: 0;
    z-index: 4;
    background: white;
  ...
  .desktop {
    ...
    tbody tr:not(.cv-grid-add-new-cell) td:nth-child(2) {
      position: sticky !important;
      z-index: 4;
      left: 64px;
      background: white;
  ...
  .cv-grid-skeleton-loader {
    thead th:nth-child(2) {
      @apply border-r-1 !border-r-gray-50;
    }

    tbody td:not(.cv-grid-add-new-cell-item):nth-child(2) {
      @apply border-r-1 !border-r-gray-50;
    }
  }
}
...
.cv-grid-row {
  ...
  &:not(.selected-row):has(+ .selected-row) {
    td.cv-grid-cell:not(.active),
    td:nth-child(2):not(.active):not(.cv-grid-add-new-cell-item) {
...
.cv-fill-handle:hover,
.cv-fill-handle:active,
.cv-fill-handle:focus {
