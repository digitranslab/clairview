...
import type { ColumnType, TableType } from 'clairview-sdk'
import { UITypes, isSystemColumn } from 'clairview-sdk'
...
onClickOutside(globalSearchWrapperRef, (e) => {
  ...
  if (search.value.query || targetEl.closest('.cv-dropdown-toolbar-search-field-option')) {
    return
  }
...
<template>
  <div ref="globalSearchWrapperRef" class="cv-global-search-wrapper">
    <a-button
      v-if="!search.query && !showSearchBox"
      class="cv-toolbar-btn !rounded-lg !h-7 !px-1.5"
      data-testid="cv-global-search-show-input"
    ...
    >
      <NcDropdown
        ...
        :trigger="['click']"
        overlay-class-name="cv-dropdown-toolbar-search-field-option"
      >
        <div
