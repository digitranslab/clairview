...
import { ProjectRoles, type TableType, type ViewType, WorkspaceUserRoles } from 'clairview-sdk'
import { ViewTypes, viewTypeAlias } from 'clairview-sdk'
  ...
  >
      ...
      <NcSubMenu key="download">
        <template #title>
          <div
            ...
            class="cv-base-menu-item group cv-view-context-download-option"
          >
      ...
      >
        <template #title>
          ...
          >
            ...
            <div class="cv-base-menu-item flex !flex-shrink group !py-1 !px-1 rounded-md bg-brand-50">
              <LazySmartsheetToolbarLockType
                :type="lockType"
                class="flex cv-view-actions-lock-type !text-brand-500 !flex-shrink !cursor-auto"
                hide-tick
              />
            </div>
        ...
        <a-menu-item
          class="!mx-1 !py-2 !rounded-md cv-view-action-lock-subaction max-w-[100px]"
        ...
        <a-menu-item
          class="!mx-1 !py-2 !rounded-md cv-view-action-lock-subaction"
          :disabled="!isUIAllowed('fieldAdd')"
    ...
    <template v-if="!view.is_default && isUIAllowed('viewCreateOrEdit')">
      ...
      <NcTooltip v-if="lockType === LockType.Locked">
        <template #title> {{ $t('msg.info.disabledAsViewLocked') }} </template>
        <NcMenuItem class="!cursor-not-allowed !text-gray-400">
          <GeneralIcon class="cv-view-delete-icon" icon="delete" />
          {{
...
.cv-view-actions-lock-type {
  @apply !min-w-0;
}
</style>

<style lang="scss">
.cv-view-actions-lock-type > div {
  @apply !py-0;
}

.cv-view-action-lock-subaction {
  @apply !min-w-82;
}
</style>
