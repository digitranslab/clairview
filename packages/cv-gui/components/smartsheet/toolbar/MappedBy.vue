...
import type { MapType } from 'clairview-sdk'
...
<template>
  <a-dropdown v-if="!IsPublic" v-model:visible="mappedByDropdown" :trigger="['click']" class="!xs:hidden">
    <div class="cv-map-btn">
      <a-button v-e="['c:map:change-grouping-field']" class="cv-map-stacked-by-menu-btn cv-toolbar-btn" :disabled="isLocked">
    ...
    <template #overlay>
      <div
        ...
        class="p-3 min-w-[280px] bg-gray-50 shadow-lg cv-table-toolbar-menu max-h-[max(80vh,500px)] overflow-auto !border"
        @click.stop
      >
        <div>
        ...
        <div class="cv-fields-list py-1">
          <div class="grouping-field">
            <a-select
              v-model:value="geoDataMappingFieldColumnId"
              class="w-full cv-msp-grouping-field-select"
