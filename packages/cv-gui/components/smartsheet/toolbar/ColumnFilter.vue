...
import { type ColumnType, type FilterType, isCreatedOrLastModifiedTimeCol, isSystemColumn, isVirtualCol } from 'clairview-sdk'
import { PlanLimitTypes, UITypes } from 'clairview-sdk'
...
watch(
  ...
  async () => {
    ...
    const filterWrapper = document.querySelectorAll(`.cv-filter-wrapper-${draftFilter.value.fk_column_id}`)

    draftFilter.value = {}
    if (!filterWrapper.length) return

    const filterInputElement =
      filterWrapper[filterWrapper.length - 1]?.querySelector<HTMLInputElement>('.cv-filter-value-select input')
...
<template>
  ...
  >
    <div v-if="nested" class="flex min-w-full w-min items-center gap-1 mb-2">
      <div :class="[`cv-filter-logical-op-level-${nestedLevel}`]">
        <slot name="start"></slot>
      </div>
      <div class="flex-grow"></div>
      <NcDropdown
        :trigger="['hover']"
        overlay-class-name="cv-dropdown-filter-group-sub-menu"
    ...
    <div
      ...
      ref="wrapperDomRef"
      class="flex flex-col gap-y-1.5 cv-filter-grid min-w-full w-min"
      :class="{ 'max-h-420px cv-scrollbar-thin cv-filter-top-wrapper pr-4 my-2 py-1': !nested, '!pr-0': webHook && !nested }"
      @click.stop
    >
        ...
        <template v-if="filter.status !== 'delete'">
          <template v-if="filter.is_group">
            <div class="flex flex-col min-w-full w-min gap-y-2">
              <div class="flex rounded-lg p-2 min-w-full w-min border-1" :class="[`cv-filter-nested-level-${nestedLevel}`]">
                ...
                >
                  <template #start>
                    <span v-if="!visibleFilters.indexOf(filter)" class="flex items-center cv-filter-where-label ml-1">{{
                      $t('labels.where')
                    }}</span>
                    <div v-else :key="`${i}nested`" class="flex cv-filter-logical-op">
                      <NcSelect
                        ...
                        dropdown-class-name="cv-dropdown-filter-logical-op-group"
                        :disabled="(i > 1 && !isLogicalOpChangeAllowed) || isLockedView"
                        :class="{
                          'cv-disabled-logical-op': filter.readOnly || (i > 1 && !isLogicalOpChangeAllowed),
                          '!max-w-18': !webHook,
                          '!w-full': webHook,
                        }"
                        @click.stop
                  ...
                  <template #end>
                    <NcButton
                      ...
                      class="cv-filter-item-remove-btn cursor-pointer"
          ...
          <div v-else class="flex flex-row gap-x-0 w-full cv-filter-wrapper" :class="`cv-filter-wrapper-${filter.fk_column_id}`">
            <div v-if="!visibleFilters.indexOf(filter)" class="flex items-center !min-w-18 !max-w-18 pl-3 cv-filter-where-label">
              {{ $t('labels.where') }}
            </div>
            ...
            <NcSelect
              ...
              dropdown-class-name="cv-dropdown-filter-logical-op"
              :class="{
                'cv-disabled-logical-op': filter.readOnly || (visibleFilters.indexOf(filter) > 1 && !isLogicalOpChangeAllowed),
              }"
              @change="onLogicalOpUpdate(filter, i)"
              @click.stop
            >
            ...
            <template v-else>
              <SmartsheetToolbarFieldListAutoCompleteDropdown
                ...
                class="cv-filter-field-select min-w-32 max-h-8"
              ...
              <NcSelect
                ...
                :dropdown-match-select-width="false"
                class="caption cv-filter-operation-select !min-w-26.75 max-h-8"
                ...
                :disabled="filter.readOnly || isLockedView"
                hide-details
                dropdown-class-name="cv-dropdown-filter-comp-op !max-w-80"
              ...
              <NcSelect
                ...
                :dropdown-match-select-width="false"
                class="caption cv-filter-sub_operation-select min-w-28"
                :class="{
                ...
                :disabled="filter.readOnly || isLockedView"
                hide-details
                dropdown-class-name="cv-dropdown-filter-comp-sub-op"
                @change="filterUpdateCondition(filter, i)"
              >
                <template
              ...
              <div class="flex items-center flex-grow">
                <div v-if="link && (filter.dynamic || filter.fk_value_col_id)" class="flex-grow">
                  <SmartsheetToolbarFieldListAutoCompleteDropdown
                    ...
                    class="cv-filter-field-select min-w-32 w-full max-h-8"
                ...
                <template v-else>
                  ...
                  <SmartsheetToolbarFilterInput
                    v-if="showFilterInput(filter)"
                    class="cv-filter-value-select rounded-md min-w-34"
                    :class="{
                      '!w-full': webHook,
                    }"
                ...
                <template v-if="link">
                  <NcDropdown
                    class="cv-settings-dropdown h-full flex items-center min-w-0 rounded-lg"
                    :trigger="['click']"
                    placement="bottom"
                    :disabled="isLockedView"
                  >
            ...
            <NcButton
              ...
              size="small"
              :disabled="isLockedView"
              class="cv-filter-item-remove-btn self-center"
              @click.stop="deleteFilter(filter, i)"
            >
              <component :is="iconMap.deleteListItem" />
            </NcButton>
...
.cv-disabled-logical-op :deep(.ant-select-arrow) {
  @apply hidden;
}
...
.cv-filter-wrapper {
  ...
  :deep(.cv-filter-field-select) > div {
  ...
  :deep(.cv-select:not(.cv-disabled-logical-op):not(.ant-select-disabled):hover) {
    &,
    .ant-select-selector {
      @apply bg-gray-50;
    }
...
.cv-filter-nested-level-0 {
  @apply bg-[#f9f9fa];
}

.cv-filter-nested-level-1,
.cv-filter-nested-level-3 {
  @apply bg-gray-[#f4f4f5];
}

.cv-filter-nested-level-2,
.cv-filter-nested-level-4 {
  @apply bg-gray-[#e7e7e9];
}

.cv-filter-logical-op-level-3,
.cv-filter-logical-op-level-5 {
  :deep(.cv-select.ant-select .ant-select-selector) {
    @apply border-[#d9d9d9];
  }
}

.cv-filter-where-label {
  @apply text-gray-400;
}
...
:deep(.cv-filter-logical-op .cv-select.ant-select .ant-select-selector) {
  @apply shadow-none;
}

:deep(.cv-select-expand-btn) {
  @apply text-gray-500;
}
...
.cv-filter-input-wrapper :deep(input) {
