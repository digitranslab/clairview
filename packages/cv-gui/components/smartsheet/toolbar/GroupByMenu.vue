...
import type { ColumnType, LinkToAnotherRecordType } from 'clairview-sdk'
import { RelationTypes, UITypes, isLinksOrLTAR, isSystemColumn } from 'clairview-sdk'
...
<template>
  <NcDropdown
    ...
    :trigger="['click']"
    class="!xs:hidden"
    overlay-class-name="cv-dropdown-group-by-menu cv-toolbar-dropdown overflow-hidden"
  >
    <NcTooltip :disabled="!isMobileMode && !isToolbarIconMode" :class="{ 'cv-active-btn': groupedByColumnIds?.length }">
      <template #title>
        {{ $t('activity.group') }}
      </template>
      <NcButton
        v-e="['c:group-by']"
        class="cv-group-by-menu-btn cv-toolbar-btn !border-0 !h-7"
        size="small"
        type="secondary"
    ...
    <template #overlay>
      ...
      >
        ...
        <div
          v-else
          class="flex flex-col bg-white overflow-auto cv-group-by-list menu-filter-dropdown w-100 p-4"
          data-testid="cv-group-by-menu"
        >
              ...
              <template #item="{ element: group }">
                <div :key="group.fk_column_id" class="flex first:mb-0 !mb-1.5 !last:mb-0 items-center">
                  ...
                  <LazySmartsheetToolbarFieldListAutoCompleteDropdown
                    ...
                    class="caption cv-sort-field-select !w-36"
                    :columns="fieldsToGroupBy"
                    :allow-empty="true"
                    :meta="meta"
                    :disabled="isLocked"
                  ...
                  <NcSelect
                    ...
                    class="flex flex-grow-1 w-full cv-sort-dir-select"
                    :label="$t('labels.operation')"
                    dropdown-class-name="sort-dir-dropdown cv-dropdown-sort-dir"
                  ...
                  <NcTooltip placement="top" title="Remove" class="flex-none">
                    <NcButton
                      ...
                      class="cv-group-by-item-remove-btn !border-l-transparent !rounded-l-none min-w-40"
                      size="small"
                      type="secondary"
                      :shadow="false"
                      :disabled="isLocked"
          ...
          <NcDropdown
            ...
            :trigger="['click']"
            overlay-class-name="cv-toolbar-dropdown"
            :disabled="isLocked"
          >
            <NcButton
              v-e="['c:group-by:add']"
              ...
              class="cv-add-group-by-btn mt-2"
              :class="{
...
:deep(.cv-select:not(.ant-select-disabled):hover) {
  &,
  .ant-select-selector {
    @apply bg-gray-50;
  }
