...
import type { CalendarType, ColumnType, GalleryType, KanbanType, LookupType } from 'clairview-sdk'
import { UITypes, ViewTypes, isVirtualCol } from 'clairview-sdk'
...
<template>
  <NcDropdown
    ...
    :trigger="['click']"
    class="!xs:hidden"
    overlay-class-name="cv-dropdown-fields-menu cv-toolbar-dropdown overflow-hidden"
  >
    <NcTooltip :disabled="!isMobileMode && !isToolbarIconMode" :class="{ 'cv-active-btn': numberOfHiddenFields }">
      <template #title>
        {{
      ...
      <NcButton
        v-e="['c:fields']"
        class="cv-fields-menu-btn cv-toolbar-btn !h-7 !border-0"
        size="small"
        type="secondary"
    ...
    <template #overlay>
      <div
        class="pt-2 bg-white w-full min-w-72 max-w-80 rounded-lg cv-table-toolbar-menu"
        data-testid="cv-fields-menu"
        @click.stop
      >
        <div
        ...
        >
          ...
          <div
            class="flex-1 cv-dropdown-cover-image-wrapper flex items-stretch border-1 border-gray-200 rounded-lg transition-all duration-0.3s max-w-[206px]"
            :class="{
              'cv-disabled': isLocked,
            }"
          >
            <a-select
              v-model:value="coverImageColumnId"
              class="flex-1 max-w-[calc(100%_-_33px)]"
              dropdown-class-name="cv-dropdown-cover-image !rounded-lg"
              :bordered="false"
              :disabled="isLocked"
              @click.stop
            >
            ...
            >
              ...
              <template #overlay>
                <NcMenu class="cv-cover-image-object-fit-dropdown-menu min-w-[168px]">
                  <NcMenuItem
        ...
        <div class="px-2" @click.stop>
          <a-input
            ...
            class="cv-toolbar-dropdown-search-field-input"
          >
            <template #prefix> <GeneralIcon icon="search" class="cv-search-icon h-3.5 w-3.5 mr-1" /> </template
          ></a-input>
        </div>

        <div class="flex flex-col mt-2 pb-2 cv-scrollbar-thin max-h-[47vh] px-2">
          <div class="cv-fields-list">
            <div
            ...
            <Draggable
              v-model="fields"
              item-key="id"
              ghost-class="cv-fields-menu-items-ghost"
              :disabled="isLocked"
              @change="onMove($event)"
            ...
            >
              <template #item="{ element: field }">
                <div
                  ...
                  "
                  :key="field.id"
                  :data-testid="`cv-fields-menu-${field.title}`"
        ...
        <div v-if="!filterQuery" class="flex px-2 gap-2 py-2">
          <NcButton
            class="cv-fields-show-all-fields"
            size="small"
            type="ghost"
            :disabled="isLocked"
          ...
          <NcButton
            v-if="!isLocalMode"
            class="cv-fields-show-system-fields"
            size="small"
            type="ghost"
            :disabled="isLocked"
...
.cv-fields-menu-items-ghost {
  @apply bg-gray-50;
}

.cv-fields-show-all-fields,
.cv-fields-show-system-fields {
...
.cv-cover-image-object-fit-dropdown-menu {
  :deep(.cv-menu-item-inner) {
    @apply !w-full flex items-center justify-between;
  }
}
.cv-dropdown-cover-image-wrapper {
  @apply h-8;
  &:not(.cv-disabled):not(:focus-within) {
    @apply shadow-default hover:shadow-hover;
  }
  &:not(.cv-disabled):focus-within {
