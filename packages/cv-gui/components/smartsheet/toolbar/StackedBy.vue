...
import type { ColumnType, KanbanType } from 'clairview-sdk'
...
<template>
  <NcDropdown
    ...
    :trigger="['click']"
    overlay-class-name="cv-dropdown-kanban-stacked-by-menu overflow-hidden"
    class="!xs:hidden"
  >
    <NcTooltip :disabled="!isToolbarIconMode" class="cv-kanban-btn">
      <template #title>
      ...
      <NcButton
        ...
        class="cv-kanban-stacked-by-menu-btn cv-toolbar-btn !border-0 !h-7 group"
        size="small"
        type="secondary"
    ...
    <template #overlay>
      <div v-if="open" class="p-4 w-90 bg-white cv-table-toolbar-menu rounded-lg flex flex-col gap-5" @click.stop>
          ...
          <div class="cv-fields-list">
            <div class="grouping-field">
              <a-select
                ...
                class="cv-select-shadow w-full cv-kanban-grouping-field-select !rounded-lg"
