...
import { PlanLimitTypes, RelationTypes, UITypes, getEquivalentUIType, isLinksOrLTAR, isSystemColumn } from 'clairview-sdk'
...
<template>
  <NcDropdown
    ...
    :trigger="['click']"
    class="!xs:hidden"
    overlay-class-name="cv-dropdown-sort-menu cv-toolbar-dropdown overflow-hidden"
  >
    <NcTooltip :disabled="!isMobileMode && !isToolbarIconMode" :class="{ 'cv-active-btn': sorts?.length }">
      <template #title>
        {{ $t('activity.sort') }}
      </template>
      <NcButton
        ...
        class="cv-sort-menu-btn cv-toolbar-btn !h-7"
        size="small"
        type="secondary"
    ...
    <template #overlay>
      ...
      >
        ...
        <div v-else class="pt-2 pb-2 pl-4 cv-filter-list max-h-[max(80vh,30rem)] min-w-102" data-testid="cv-sorts-menu">
          <div class="sort-grid max-h-120 cv-scrollbar-thin pr-4 my-2 py-1" @click.stop>
            <template v-for="(sort, i) of sorts" :key="i">
              <SmartsheetToolbarFieldListAutoCompleteDropdown
                v-model="sort.fk_column_id"
                class="flex caption cv-sort-field-select w-44 flex-grow"
                :columns="columns"
                is-sort
                :meta="meta"
                :disabled="isLocked"
              ...
              <NcSelect
                ...
                class="shrink grow-0 cv-sort-dir-select"
                :label="$t('labels.operation')"
                dropdown-class-name="sort-dir-dropdown cv-dropdown-sort-dir !rounded-lg"
                :disabled="isLocked"
                @click.stop
              ...
              <NcButton
                ...
                size="small"
                :disabled="isLocked"
                class="cv-sort-item-remove-btn !max-w-8"
