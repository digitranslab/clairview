...
import { type CalendarRangeType, FormulaDataTypes, UITypes } from 'clairview-sdk'
...
<template>
  ...
  >
    <NcTooltip :disabled="!isToolbarIconMode" class="cv-calendar-btn">
      <template #title>
        {{ $t('activity.settings') }}
      </template>
      ...
      <NcButton
        ...
        class="cv-toolbar-btn !border-0 group !h-6"
        size="small"
        type="secondary"
        data-testid="cv-calendar-range-btn"
    ...
    <template #overlay>
      <div v-if="calendarRangeDropdown" class="w-108 space-y-6 rounded-2xl p-6" data-testid="cv-calendar-range-menu" @click.stop>
        <div
          v-for="(range, id) in _calendar_ranges"
          :key="id"
          class="flex flex-col w-full gap-2 mb-2"
          data-testid="cv-calendar-range-option"
        >
          ...
          <a-select
            ...
            class="cv-select-shadow w-full !rounded-lg"
          ...
          <div v-if="isEeUI" class="w-full space-y-2">
            <NcTooltip v-if="range.fk_to_column_id === null && isRangeEnabled" placement="left" :disabled="!isDisabled">
              <NcButton
                size="small"
                data-testid="cv-calendar-range-add-end-date"
                class="w-23"
                type="text"
                :shadow="false"
            ...
            <template v-else-if="isEeUI">
              ...
              <div class="flex">
                <a-select
                  ...
                  class="!rounded-r-none cv-select-shadow w-full flex-1"
                  allow-clear
...
<style lang="scss">
.cv-to-select .ant-select-selector {
