...
const code = computed(() => {
  if (activeLang.value?.name === 'ClairView-SDK') {
    return `${selectedClient.value === 'node' ? 'const { Api } = require("clairview-sdk");' : 'import { Api } from "clairview-sdk";'}
...
const supportedDocs = [
  {
    title: 'Data APIs',
    href: 'https://data-apis-v2.clairview.com/',
  },
  {
    title: 'Meta APIs',
    href: 'https://meta-apis-v2.clairview.com/',
  },
  {
    title: 'Create API Token',
    href: 'https://docs.clairview.com/account-settings/api-tokens/#create-api-token',
  },
  {
    title: 'Swagger',
    href: 'https://docs.clairview.com/bases/actions-on-base/#rest-apis',
  },
] as {
  title: string
  href: string
...
<template>
  ...
  >
    <div class="flex gap-4 max-w-[1000px] mx-auto h-full">
      <NcMenu class="cv-api-snippets-menu !h-full w-[252px] min-w-[252px] cv-scrollbar-thin !pr-3">
        <div
      ...
      <div class="w-[calc(100%_-_264px)] flex flex-col gap-6 h-full max-h-full">
        <div class="cv-api-clents-tab-wrapper h-[calc(100%_-_56px)] flex flex-col mt-2">
          <NcTabs v-model:activeKey="selectedClient" class="cv-api-clents-tab">
            <template #rightExtra>
              <NcButton
                v-e="[
                  'c:snippet:copy',
...
:deep(.cv-api-clents-tab.ant-tabs) {
  .ant-tabs-nav {
    @apply px-3;
