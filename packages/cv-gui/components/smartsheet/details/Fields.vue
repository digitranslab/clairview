...
import type { ButtonType, ColumnType, FilterType, SelectOptionsType, TableType } from 'clairview-sdk'
...
onKeyDown('ArrowRight', () => {
  ...
  if (activeField.value) {
    const input = document.querySelector('.cv-fields-input') as HTMLInputElement
    if (input) {
      input.focus()
    }
  }
...
<template>
  <div class="cv-fields-wrapper w-full p-4">
      ...
      <template v-else>
        <div class="flex w-full justify-between pt-2">
          <a-input
            ...
            data-testid="cv-field-search-input"
          ...
          >
            ...
            <template #suffix>
              <GeneralIcon
                ...
                data-testid="cv-field-clear-search"
                @click="searchQuery = ''"
              />
            </template>
          </a-input>
          <div class="flex gap-2">
            <template v-if="isFeatureEnabled(FEATURE_FLAG.AI_FEATURES)">
              <div class="cv-fields-add-new-field-btn-wrapper rounded-lg shadow-cv-sm">
                <NcTooltip :disabled="isLocked">
                  <template #title> {{ `${renderAltOrOptlKey()} + C` }} </template>
                  <NcButton
                    data-testid="cv-field-add-new"
                    type="secondary"
                    size="small"
                    class="cv-field-add-new !rounded-r-none !border-r-transparent"
                ...
                <NcTooltip :title="aiMode ? 'Disable AI suggestions' : 'Suggest fields using AI'">
                  <NcDropdown :trigger="['hover']" placement="bottomRight" overlay-class-name="!border-purple-200">
                    ...
                    <template #overlay>
                      <NcMenu>
                        <NcMenuItem class="!children:w-full !text-cv-content-purple-dark" @click="toggleAiMode()">
                          <component :is="getUIDTIcon(UITypes.SingleLineText)" class="flex-none w-3.5 h-3.5" />
                          {{ $t('labels.autoSuggestFields') }}
                        </NcMenuItem>
                        <NcMenuItem
                          v-show="!isForm"
                          class="!children:w-full !text-cv-content-purple-dark"
            ...
            <template v-else>
              <div class="cv-fields-add-new-field-btn-wrapper shadow-sm">
                <NcTooltip :disabled="isLocked">
                  <template #title> {{ `${renderAltOrOptlKey()} + C` }} </template>
                  <NcButton
                    data-testid="cv-field-add-new"
            ...
            <NcTooltip :disabled="isLocked">
              ...
              <NcButton
                data-testid="cv-field-save-changes"
                type="primary"
                size="small"
                :loading="loading"
                :disabled="
        ...
        >
          <div class="flex-1 h-full flex flex-col">
            <div v-if="aiMode" class="pt-3 bg-cv-bg-gray-extralight border-b-1 border-b-cv-border-gray-medium">
              <!-- Ai field wizard  -->
                ...
                <template #AutoSuggestedContent>
                  <div class="px-5 pt-4 pb-5">
                    ...
                    <div v-else-if="aiError" class="w-full flex items-center gap-3">
                      ...
                      <NcTooltip class="truncate flex-1 text-sm text-cv-content-gray-subtle" show-on-truncate-only>
                        <template #title>
                          {{ aiError }}
                        </template>
                        {{ aiError }}
                      ...
                      <NcButton size="small" type="text" class="!text-cv-content-brand" @click.stop="handleRefreshOnError">
                        {{ $t('general.refresh') }}
                      </NcButton>
                    </div>

                    <div v-else-if="aiModeStep === 'init'">
                      <div class="text-cv-content-purple-light text-sm h-7 flex items-center gap-2">
                        <GeneralLoader size="regular" class="!text-cv-content-purple-dark" />

                        <div class="cv-animate-dots">
                    ...
                    <div v-else-if="aiModeStep === 'pick'" class="flex gap-3 items-start">
                      <div class="flex-1 flex gap-2 flex-wrap">
                        <template v-if="activeTabNonSelectedFields.length">
                          <template v-for="f of activeTabNonSelectedFields" :key="f.title">
                            <NcTooltip :disabled="activeTabSelectedFields.length < maxSelectionCount || f.selected">
                              ...
                              <a-tag
                                class="cv-ai-suggested-tag"
                                :class="{
                                  'cv-disabled': loading || (!f.selected && activeTabSelectedFields.length >= maxSelectionCount),
                                  'cv-selected': f.selected,
                                }"
                        ...
                        <div v-else-if="activeTabSelectedFields.length" class="text-cv-content-purple-light">
                          To generate more {{ isFormulaPredictionMode ? 'formula' : '' }} field suggestions, click the + or ⟳ icon
                          on the right
                        </div>
                        <div v-else class="text-cv-content-gray-subtle2">{{ $t('labels.noData') }}</div>
                      </div>
                    ...
                    <template v-else>
                      <div class="relative">
                        <a-textarea
                          ...
                          class="cv-ai-input cv-input-shadow !px-3 !pt-2 !pb-3 !text-sm !min-h-[68px] !rounded-lg"
                          @keydown.enter.stop
                        >
                        </a-textarea>
                      ...
                      <div v-else-if="isPromtAlreadyGenerated" class="flex flex-col gap-3">
                        <div class="text-cv-content-purple-dark font-semibold text-xs">Generated Field(s)</div>
            ...
            <div ref="fieldsListWrapperDomRef" class="flex-1 flex-grow-1 cv-scrollbar-md !overflow-auto">
              <Draggable
                :model-value="fields"
                :disabled="isLocked"
                item-key="id"
                data-testid="cv-field-list-wrapper"
                @change="onMove($event)"
              >
                <template #item="{ element: field }">
                  <div
                    ...
                    :data-testid="`cv-field-item-${fieldState(field)?.title || field.title}`"
                  ...
                  >
                    <div class="flex items-center flex-1 py-2.5 gap-1 w-2/6">
                      ...
                      <NcCheckbox
                        ...
                        "
                        data-testid="cv-field-visibility-checkbox"
                        @change="
                        (event: any) => {
                    ...
                    <div class="flex items-center justify-end gap-1">
                      <div class="cv-field-status-wrapper flex items-center">
                        ...
                        <NcBadge
                          ...
                          data-testid="cv-field-status-new-field"
                        >
                        ...
                        <NcBadge
                          ...
                          data-testid="cv-field-status-updated-field"
                        >
                        ...
                        <NcBadge
                          ...
                          data-testid="cv-field-status-incomplete-configuration"
                        >
                        ...
                        <NcTooltip v-if="!!fieldError(field)" class="cursor-pointer">
                          ...
                          <NcBadge
                            ...
                            data-testid="cv-field-status-error-configuration"
                          >
                      ...
                      <NcButton
                        ...
                        :disabled="loading"
                        data-testid="cv-field-restore-changes"
                      ...
                      <NcDropdown
                        v-else
                        :trigger="['click']"
                        overlay-class-name="cv-field-item-action-dropdown cv-dropdown-table-explorer"
                      ...
                      >
                        <NcButton
                          ...
                          data-testid="cv-field-item-action-button"
                        >
                          ...
                          <NcMenu style="padding-top: 0.45rem !important">
                            ...
                            <template v-if="!isLocked">
                              <NcMenuItem
                                ...
                                data-testid="cv-field-item-action-duplicate"
                              ...
                              <NcMenuItem
                                ...
                                data-testid="cv-field-item-action-insert-above"
                              ...
                              <NcMenuItem
                                ...
                                data-testid="cv-field-item-action-insert-below"
                              ...
                              <NcMenuItem
                                ...
                                data-testid="cv-field-item-action-delete"
                ...
                >
                  <div
                    ...
                    :data-testid="`cv-field-item-${fieldState(displayColumn)?.title || displayColumn.title}`"
                  ...
                  >
                    <div class="flex items-center flex-1 py-2.5 gap-1 w-2/6">
                      ...
                      />
                      <NcCheckbox :disabled="true" :checked="true" data-testid="cv-field-visibility-checkbox" />
                      <SmartsheetHeaderCellIcon
                        v-if="displayColumn"
                    ...
                    <div class="flex items-center justify-end gap-1">
                      ...
                      <NcDropdown
                        v-else
                        :trigger="['click']"
                        overlay-class-name="cv-field-item-action-dropdown-display-column cv-dropdown-table-explorer-display-column"
                        @update:visible="onFieldOptionUpdate"
                        @click.stop
                      >
                        <NcButton
          ...
          <Transition name="slide-fade">
            <div
              v-if="!changingField"
              class="border-gray-200 border-l-1 cv-scrollbar-md h-full !overflow-y-auto"
              @keydown.up.stop
              @keydown.down.stop
            >
...
<style lang="scss">
.cv-dropdown-table-explorer {
  @apply !overflow-hidden;
}

.cv-dropdown-table-explorer > div > ul.ant-dropdown-menu.cv-menu {
  @apply !pt-0;
}

.cv-dropdown-table-explorer-display-column {
  @apply !overflow-hidden;
}

.cv-dropdown-table-explorer-display-column > div > ul.ant-dropdown-menu.cv-menu {
  @apply !py-1.5;
}
</style>

<style lang="scss" scoped>
:deep(ul.ant-dropdown-menu.cv-menu) {
  @apply !pt-0;
}
...
.cv-fields-add-new-field-btn-wrapper {
