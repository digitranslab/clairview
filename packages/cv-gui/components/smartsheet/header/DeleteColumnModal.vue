...
import { RelationTypes, isLinksOrLTAR, isVirtualCol } from 'clairview-sdk'
  ...
  <GeneralDeleteModal v-model:visible="visible" :entity-name="$t('objects.column')" :on-delete="onDelete">
    <template #entity-preview>
      <div v-if="column" class="flex flex-row items-center py-2 px-3 bg-gray-50 rounded-lg text-gray-700 mb-4">
        <SmartsheetHeaderVirtualCellIcon v-if="isVirtualCol(column)" class="cv-view-icon"></SmartsheetHeaderVirtualCellIcon>
        <SmartsheetHeaderCellIcon v-else class="cv-view-icon"></SmartsheetHeaderCellIcon>
        <div
