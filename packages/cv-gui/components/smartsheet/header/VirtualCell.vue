...
import { RelationTypes, UITypes, UITypesName, substituteColumnIdWithAliasInFormula } from 'clairview-sdk'
...
const tooltipMsg = computed(() => {
  ...
  if (isLinksOrLTAR(column.value) && relatedTableTitle.value && isExternalSource.value) {
    if (isMm(column.value)) {
      ...
      suffix = fkColumn?.title?.startsWith('cv_') ? '' : `\nForeign Key Column: ${fkColumn.title}`
...
<template>
  ...
  >
    <div
      class="cv-virtual-cell-name-wrapper flex-1 flex items-center"
      :class="{
