...
import NcLayout from '~icons/cv-icons/layout'
...
watch(
  ...
  () => {
    ...
    useTitle(`${currentBase.value?.title ?? activeWorkspace.value?.title ?? 'ClairView'}`)
  },
  {
    immediate: true,
  },
...
<template>
  <div class="h-full cv-base-view">
    <div
      v-if="!isAdminPanel"
      class="flex flex-row px-2 py-2 gap-3 justify-between w-full border-b-1 border-gray-200"
      :class="{ 'cv-table-toolbar-mobile': isMobileMode, 'h-[var(--topbar-height)]': !isMobileMode }"
    >
