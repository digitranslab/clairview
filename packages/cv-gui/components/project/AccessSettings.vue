...
import type { Roles, WorkspaceUserRoles } from 'clairview-sdk'
import { OrderedProjectRoles, OrgUserRoles, ProjectRoles, WorkspaceRolesToProjectRoles } from 'clairview-sdk'
...
<template>
  <div
    class="cv-collaborator-table-container cv-access-settings-view flex flex-col"
    :class="{
      'h-[calc(100vh_-_100px)]': !isAdminPanel,
    }"
  >
    <div v-if="isAdminPanel">
      <div class="cv-breadcrumb px-2">
        <div class="cv-breadcrumb-item">
          {{ org.title }}
        </div>
        <GeneralIcon icon="ncSlash1" class="cv-breadcrumb-divider" />
        <NuxtLink
        ...
        <GeneralIcon icon="ncSlash1" class="cv-breadcrumb-divider" />
        <div class="cv-breadcrumb-item active truncate capitalize">
          {{ currentBase?.title }}
        </div>
      </div>
      <NcPageHeader>
        <template #icon>
          <div class="cv-page-header-icon flex justify-center items-center h-5 w-5">
    ...
    <div class="cv-content-max-w h-full flex flex-col items-center gap-6 px-6 pt-6">
      <div v-if="!isAdminPanel" class="w-full flex justify-between items-center max-w-350 gap-3">
        <a-input
          ...
          :disabled="isLoading"
          allow-clear
          class="cv-input-border-on-value !max-w-90 !h-8 !px-3 !py-1 !rounded-lg"
        >
          <template #prefix>
      ...
      <NcTable
        ...
        class="flex-1 cv-collaborators-list max-w-350"
      >
        <template #emptyText>
...
:deep(.cv-collaborator-role-select .ant-select-selector) {
  @apply !rounded;
}
