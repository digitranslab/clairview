...
<template>
  <GeneralModal v-model:visible="visible" width="35rem">
    <div class="flex flex-col px-8 pt-6 pb-9">
      ...
      <div class="row mb-10">
        <div class="cv-base-view-import-sub-btn" @click="onClick('airtable')">
          <GeneralIcon icon="airtable" />
          <div class="label">{{ $t('labels.airtable') }}</div>
        </div>
        <div class="cv-base-view-import-sub-btn" @click="onClick('csv')">
          <GeneralIcon icon="csv" />
          <div class="label">{{ $t('labels.csv') }}</div>
        </div>
      </div>
      <div class="row">
        <div class="cv-base-view-import-sub-btn" @click="onClick('excel')">
          <GeneralIcon icon="excelColored" />
          <div class="label">{{ $t('labels.excel') }}</div>
        </div>
        <div class="cv-base-view-import-sub-btn" @click="onClick('json')">
