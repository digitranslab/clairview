...
import type { ColumnType, LinkToAnotherRecordType, TableType } from 'clairview-sdk'
import { UITypes, isLinksOrLTAR } from 'clairview-sdk'
...
<template>
  <div class="cv-container flex flex-col h-full" @drop="onDrop" @dragover.prevent>
    <LazySmartsheetTopbar />
    <div style="height: calc(100% - var(--topbar-height))">
      <Splitpanes
        v-if="openedViewsTab === 'view'"
        class="cv-extensions-content-resizable-wrapper"
        @ready="() => onReady()"
        @resized="onResize"
      >
...
<style lang="scss">
:deep(.cv-right-sidebar.ant-layout-sider-collapsed) {
  @apply !w-0 !max-w-0 !min-w-0 overflow-x-hidden;
}

.cv-extensions-content-resizable-wrapper > {
