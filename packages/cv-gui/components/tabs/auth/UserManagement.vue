...
import { OrgUserRoles, RoleColors } from 'clairview-sdk'
import type { ProjectUserReqType, RequestParams } from 'clairview-sdk'
...
<template>
  ...
  <div v-else class="flex flex-col w-full px-6">
    ...
    <a-modal
      ...
      class="!rounded-md"
      :footer="null"
      wrap-class-name="cv-modal-delete-user"
    >
    ...
    <div class="flex flex-row mb-4 mx-4 justify-between pb-2">
      ...
      <div class="flex flex-row space-x-1">
        ...
        <a-button
          ...
          size="middle"
          type="primary"
          class="cv-invite-team !rounded-md"
          @click="onInvite"
        >
          <div class="flex flex-row justify-center items-center caption capitalize space-x-1">
    ...
    <div class="px-5">
      ...
      <div v-for="(user, index) of users" :key="index" class="flex flex-row items-center border-b-1 py-2 px-2 cv-user-row">
        <div class="flex w-3/6 flex-wrap cv-user-email">
          {{ user.email }}
        </div>
        ...
        <div class="flex w-2/6 justify-start gap-2 flex-wrap ml-4">
          <div
            ...
            class="rounded-full px-3 py-1 cv-user-role"
            :style="{ backgroundColor: RoleColors[OrgUserRoles.SUPER_ADMIN] }"
          >
            Super Admin
          </div>
          <div v-if="user.roles" class="rounded-full px-3 py-1 cv-user-role" :style="{ backgroundColor: RoleColors[user.roles] }">
        ...
        <div class="flex w-1/6 flex-wrap justify-end">
          <template v-if="!isSuperAdmin(user)">
            <a-tooltip v-if="user.base_id" placement="bottom">
              ...
              <a-button type="text" class="!rounded-md cv-user-edit" @click="onEdit(user)">
                <template #icon>
            ...
            <a-tooltip v-if="!user.base_id" placement="bottom">
              ...
              <a-button type="text" class="!rounded-md cv-user-invite" @click="inviteUser(user)">
                <template #icon>
            ...
            <a-tooltip v-else placement="bottom">
              ...
              <a-button v-e="['c:user:delete']" type="text" class="!rounded-md cv-user-delete" @click="onDelete(user)">
                <template #icon>
            ...
            <a-dropdown
              ...
              overlay-class-name="cv-dropdown-user-mgmt"
            >
