...
import type { HookLogType, HookType } from 'clairview-sdk'
import { timeAgo } from 'clairview-sdk'
  ...
  <div v-else>
    <a-card class="!mb-[20px]" :body-style="{ padding: '10px' }">
      ...
      <span>
        ...
        <a href="https://docs.clairview.com/developer-resources/webhooks#call-log" target="_blank" rel="noopener">here</a>.
      </span>
    </a-card>
    ...
    <div v-if="showLogs">
      ...
      <a-layout v-else>
        <a-layout-content>
          <a-collapse v-model:activeKey="activeKey" class="cv-hook-log-collapse">
            <a-collapse-panel v-for="(hookLog, idx) of hookLogs" :key="idx">
              ...
              <div v-if="hookLog.type !== 'Email'">
                <div v-if="hookLog?.response?.config?.headers" class="cv-hook-log-request">
                  <div class="cv-hook-pre-title">Request</div>
                  <pre class="cv-hook-pre">{{ hookLog.response.config.headers }}</pre>
                </div>

                <div v-if="hookLog?.response?.headers" class="cv-hook-log-response">
                  <div class="cv-hook-pre-title">Response</div>
                  <pre class="cv-hook-pre">{{ hookLog.response.headers }}</pre>
                </div>

                <div v-if="hookLog?.response?.config?.data" class="cv-hook-log-payload">
                  <div class="cv-hook-pre-title">Payload</div>
                  <pre class="cv-hook-pre">{{ hookLog.response.config.data }}</pre>
                </div>
              </div>
              <div v-else>
                <div v-if="hookLog?.payload" class="cv-hook-log-payload">
                  <div class="cv-hook-pre-title">Payload</div>
                  <pre class="cv-hook-pre">{{ hookLog.payload }}</pre>
                </div>
              </div>
            </a-collapse-panel>
          </a-collapse>
