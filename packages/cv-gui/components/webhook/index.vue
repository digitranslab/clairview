...
import type { HookReqType, HookTestReqType, HookType } from 'clairview-sdk'
...
const supportedDocs = [
  {
    ...
    href: 'https://docs.clairview.com/automation/webhook/create-webhook/',
  },
  {
    title: 'Create webhook',
    href: 'https://docs.clairview.com/automation/webhook/webhook-overview',
  },
  {
    title: 'Custom payload',
    href: 'https://docs.clairview.com/automation/webhook/create-webhook#webhook-with-custom-payload-',
  },
  {
    title: 'Trigger on condition',
    href: 'https://docs.clairview.com/automation/webhook/create-webhook#webhook-with-conditions',
  },
]
...
<template>
  <NcModal v-model:visible="modalVisible" :show-separator="true" size="large" wrap-class-name="cv-modal-webhook-create-edit">
    <template #header>
      <div class="flex w-full items-center p-4 justify-between">
        ...
        <div class="flex justify-end items-center gap-3">
          ...
          <NcButton :loading="loading" type="primary" size="small" data-testid="cv-save-webhook" @click.stop="saveHooks">
            {{ hook ? $t('labels.multiField.saveChanges') : $t('activity.createWebhook') }}
          </NcButton>

          <NcButton type="text" size="small" data-testid="cv-close-webhook-modal" @click.stop="closeModal">
    ...
    <div class="flex bg-white rounded-b-2xl h-[calc(100%_-_66px)]">
      <div
        ref="containerElem"
        class="h-full flex-1 flex flex-col overflow-y-auto scroll-smooth cv-scrollbar-thin px-12 py-6 mx-auto"
      >
          ...
          <a-form-item v-bind="validateInfos.title">
            ...
            >
              <input
                ...
                class="flex flex-grow text-lg px-2 font-medium capitalize outline-none bg-inherit cv-text-field-hook-title"
          ...
          <a-form class="flex flex-col gap-8" :model="hookRef" name="create-or-edit-webhook">
              ...
              <div class="flex w-full gap-3 custom-select">
                <a-form-item class="w-1/3" v-bind="validateInfos.eventOperation">
                  <a-select
                    ...
                    class="cv-text-field-hook-event !h-9 capitalize"
                    dropdown-class-name="cv-dropdown-webhook-event"
                  >
                    <template #suffixIcon>
                ...
                <a-form-item class="w-2/3" v-bind="validateInfos['notification.type']">
                  <a-select
                    ...
                    size="medium"
                    :disabled="isEeUI"
                    class="cv-select-hook-notification-type !h-9"
                    :placeholder="$t('general.notification')"
                    dropdown-class-name="cv-dropdown-webhook-notification"
              ...
              <div v-if="hookRef.notification.type === 'URL'" class="flex flex-col gap-8">
                  ...
                  <div class="flex w-full gap-3">
                    <a-form-item class="w-1/3">
                      <a-select
                        ...
                        size="medium"
                        class="cv-select-hook-url-method"
                        dropdown-class-name="cv-dropdown-hook-notification-url-method"
                      >
                        <template #suffixIcon>
                          <GeneralIcon icon="arrowDown" class="text-gray-700" />
                        </template>
                    ...
                    <a-form-item class="w-2/3" v-bind="validateInfos['notification.payload.path']">
                      <a-input
                        ...
                        class="cv-text-field-hook-url-path cv-input-shadow h-9 !rounded-lg"
                      />
                    </a-form-item>
                  </div>
                </div>
                <div>
                  <NcTabs v-model:activeKey="urlTabKey">
                    ...
                    <a-tab-pane key="headers" :tab="$t('title.headers')" class="cv-tab-headers">
            ...
            <div v-if="isConditionSupport">
              <div class="w-full cursor-pointer flex items-center" @click.prevent="hookRef.condition = !hookRef.condition">
                <NcSwitch :checked="Boolean(hookRef.condition)" class="cv-check-box-hook-condition">
                ...
                <div v-for="(input, i) in formInput[hookRef.notification.type]" :key="i">
                  <a-form-item v-if="input.type === 'LongText'" v-bind="validateInfos[`notification.payload.${input.key}`]">
                    <a-textarea
                      ...
                      class="!rounded-lg !min-h-[120px] cv-scrollbar-thin cv-input-shadow"
                  ...
                  <a-form-item v-else v-bind="validateInfos[`notification.payload.${input.key}`]">
                    <a-input
                      v-model:value="hookRef.notification.payload[input.key]"
                      class="!rounded-lg cv-input-shadow !h-9"
                      :placeholder="input.label"
                    />
                  </a-form-item>
                </div>
...
.cv-button :not(.cv-icon):not(.material-symbols) {
  @apply !w-full;
}
