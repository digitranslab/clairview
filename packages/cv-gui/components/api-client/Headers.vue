  ...
  <div class="flex flex-col py-3 gap-1.5 w-full">
    <div v-for="(headerRow, idx) in vModel" :key="idx" class="flex relative items-center w-full">
      <a-form-item class="form-item w-8">
        <NcCheckbox v-model:checked="headerRow.enabled" size="large" class="cv-hook-header-checkbox" />
      </a-form-item>
      <a-form-item class="form-item w-3/6">
        <a-auto-complete
          v-model:value="headerRow.name"
          class="!rounded-l-lg !rounded-r-0 cv-input-hook-header-key hover:!border-x-0 !border-gray-200"
          :options="headerList"
      ...
      <a-form-item class="form-item w-3/6">
        <a-input
          ...
          class="cv-webhook-header-value-input !border-x-0 hover:!border-x-0 !border-gray-200 !rounded-none"
        />
      </a-form-item>
    ...
    <div class="mt-1.5">
      <NcButton size="small" type="secondary" class="cv-btn-focus" @click="addHeaderRow">
...
:deep(.ant-input.cv-webhook-header-value-input) {
  @apply !border-x-0;
}
...
:deep(.cv-input-hook-header-key.ant-select.ant-select-auto-complete) {
  @apply !text-sm;
