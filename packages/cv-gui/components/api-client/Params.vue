...
<template>
  <div class="flex flex-col py-3 gap-1.5 w-full">
    <div v-for="(paramRow, idx) in vModel" :key="idx" class="flex relative items-center w-full">
      ...
      <a-form-item class="form-item w-3/6">
        <a-input
          ...
          class="cv-webhook-parameters-value-input !border-x-0 !border-gray-200 !rounded-none"
        />
      </a-form-item>
    ...
    <div class="mt-1.5">
      <NcButton size="small" type="secondary" class="cv-btn-focus" @click="addParamRow">
...
:deep(.ant-input.cv-webhook-parameters-value-input) {
  @apply !border-x-0;
}
