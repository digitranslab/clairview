...
<template>
  ...
  >
    ...
    <div data-testid="cv-expand-table-modal" class="flex flex-col">
      <div class="mb-2 cv-content-gray">
      ...
      <a-radio-group v-if="(newRows ?? 0) > 0" v-model:value="expand">
        <a-radio
          data-testid="cv-table-expand-yes"
          :style="{
            display: 'flex',
            height: '30px',
            lineHeight: '30px',
        ...
        <a-radio
          ...
          :value="false"
        >
          <div class="cv-content-gray leading-5">
    ...
    <div class="flex flex-row mt-5 justify-end gap-x-2">
      <div class="flex gap-2 items-center">
        <NcButton data-testid="cv-table-expand-cancel" type="secondary" size="small" @click="close">
          {{ $t('labels.cancel') }}
        </NcButton>
      </div>
      <div class="flex gap-2 items-center">
        <NcButton data-testid="cv-table-expand" type="primary" size="small" @click="updateExpand">
          {{ $t('labels.continue') }}
        </NcButton>
      </div>
    </div>
