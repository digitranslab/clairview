...
import { viewTypeAlias } from 'clairview-sdk'
...
<template>
  <div v-if="isForm" class="cv-unlock-view-wrapper rounded-2xl bg-white p-6 w-full max-w-[384px] flex flex-col gap-5">
    <div class="flex flex-col gap-2">
      <div class="text-base font-bold text-cv-content-gray-emphasis">
        {{ $t('title.thisFormIsLocked') }}
      </div>
      <div v-if="view?.meta?.lockedViewDescription" class="text-sm bg-cv-bg-gray-light rounded-lg px-2 py-2">
        {{ view?.meta?.lockedViewDescription }}
      </div>
      <div class="text-sm text-cv-content-gray">{{ $t('title.unlockThisVieToMakeChanges') }}</div>
      <div class="text-sm text-cv-content-gray">
  ...
  <NcModal
    ...
    size="small"
    wrap-class-name="cv-lock-view-modal-wrapper"
  ...
  >
    <template v-if="changeType === LockType.Locked" #header>
      <div class="flex flex-col gap-2 w-full">
        ...
        <div class="text-sm font-normal text-cv-content-gray-subtle">
    ...
    >
      <div v-if="changeType === LockType.Locked" class="flex flex-col gap-5">
        <a-form-item name="description" :rules="formRules.description">
          <a-textarea
            ...
            class="!rounded-lg !text-sm cv-input-shadow !min-h-[120px] max-h-[500px] cv-scrollbar-thin"
            size="large"
            hide-details
            data-testid="cv-lock-view-description-input"
      ...
      <div class="flex gap-2 items-center justify-end">
        <NcButton type="secondary" size="small" :disabled="isLoading" data-testid="cv-cancel-btn" @click="dialogShow = false">{{
          $t('general.cancel')
        }}</NcButton>
        ...
        <NcButton
          ...
          data-testid="cv-lock-or-unlock-btn"
        >
          <template #icon>
