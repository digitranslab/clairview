...
import { UITypes, ViewTypes } from 'clairview-sdk'
...
const fullAuto = async (e) => {
  ...
  if (
    ...
    aiLoading.value ||
    aiError.value ||
    target.closest('button, input, .cv-button, textarea')
  ) {
    return
  }
...
<template>
  <NcModal
    ...
    class="cv-view-create-modal !top-[25vh]"
    :show-separator="false"
    size="xs"
    height="auto"
    :centered="false"
    cv-modal-class-name="!p-0"
    wrap-class-name="cv-modal-view-create-wrapper"
  >
    <div class="py-5 flex flex-col gap-5" @dblclick.stop="fullAuto">
      <div class="px-5 flex w-full flex-row justify-between items-center">
        <div class="flex font-bold text-base gap-x-3 items-center">
          <GeneralIcon v-if="isAIViewCreateMode" icon="ncAutoAwesome" class="text-cv-content-purple-dark h-6 w-6" />
          <GeneralViewIcon v-else :meta="{ type: form.type }" class="cv-view-icon !text-[24px] !leading-6 max-h-6 max-w-6" />
        ...
        <!-- <a
          ...
          :href="`https://docs.clairview.com/views/view-types/${typeAlias}`"
          target="_blank"
        >
          Docs
        </a> -->
        ...
        >
          <NcButton
            type="text"
            size="small"
            class="-my-1 !text-cv-content-purple-dark hover:text-cv-content-purple-dark"
            :class="{
              '!pointer-events-none !cursor-not-allowed': aiLoading,
              '!bg-cv-bg-purple-dark hover:!bg-gray-100': aiMode,
            }"
        ...
        <template v-if="!aiMode">
          <a-form-item :rules="viewNameRules" name="title" class="relative">
            <a-input
              ...
              autofocus
              class="cv-view-input cv-input-sm cv-input-shadow"
          ...
          >
            <NcSelect
              ...
              class="cv-select-shadow w-full cv-gallery-cover-image-field-select"
            >
          ...
          >
            <NcSelect
              ...
              class="cv-select-shadow w-full cv-kanban-grouping-field-select"
            >
              <a-select-option v-for="option of viewSelectFieldOptions" :key="option.value" :value="option.value">
                <div class="w-full flex gap-2 items-center justify-between" :title="option.label">
                  <div class="flex-1 flex items-center gap-1 max-w-[calc(100%_-_24px)]">
            ...
            >
              <div class="w-full space-y-2">
                ...
                <a-select
                  ...
                  class="cv-select-shadow w-full cv-from-select !rounded-lg"
                  dropdown-class-name="!rounded-lg"
                  :placeholder="$t('placeholder.notSelected')"
                  data-testid="cv-calendar-range-from-field-select"
                  @click.stop
                ...
                <template v-else-if="isEeUI">
                  ...
                  <div class="flex">
                    <a-select
                      ...
                      class="cv-select-shadow w-full flex-1"
                      allow-clear
                      :disabled="isMetaLoading || isDisabled"
                      :loading="isMetaLoading"
                      :placeholder="$t('placeholder.notSelected')"
                      data-testid="cv-calendar-range-to-field-select"
                      dropdown-class-name="!rounded-lg"
                      @click.stop
                    >
                      <template #suffixIcon><GeneralIcon icon="arrowDown" class="text-gray-700" /></template>
              ...
              <div class="px-5 pt-5 pb-2">
                ...
                <div v-else-if="aiModeStep === 'init'">
                  <div class="text-cv-content-purple-light text-sm h-7 flex items-center gap-2">
                    ...
                    <div class="cv-animate-dots">Auto suggesting views for {{ meta?.title }}</div>
                  </div>
                </div>
                      ...
                      <template v-for="v of activeTabPredictedViews" :key="v.title">
                        <NcTooltip :disabled="!(activeTabSelectedViews.length >= maxSelectionCount || !!v?.description)">
                          ...
                          <a-tag
                            ...
                            :class="{
                              'cv-disabled': isAiSaving || (!v.selected && activeTabSelectedViews.length >= maxSelectionCount),
                              'cv-selected': v.selected,
                            }"
              ...
              <div class="px-5 pt-5 pb-2 flex flex-col gap-5">
                <div class="relative">
                  <a-textarea
                    ...
                    class="cv-ai-input cv-input-shadow !px-3 !pt-2 !pb-3 !text-sm !min-h-[120px] !rounded-lg"
                    @keydown.enter.stop
                  >
                  </a-textarea>
                ...
                <div v-else-if="isPromtAlreadyGenerated" class="flex flex-col gap-3">
                  <div class="text-cv-content-purple-dark font-semibold text-xs">Generated Views(s)</div>
      ...
      <a-form-item v-if="enableDescription && !aiMode" class="!px-5">
        ...
        <a-textarea
          ...
          class="cv-input-sm cv-input-text-area cv-input-shadow px-3 !text-gray-800 max-h-[150px] min-h-[100px]"
          hide-details
...
.cv-nocoai-footer {
  @apply px-6 py-1 flex items-center gap-2 text-cv-content-purple-dark border-t-1 border-purple-100;

  .cv-nocoai-settings {
    &:not(:disabled) {
      @apply hover:!bg-cv-bg-purple-light;
    }
    &.cv-ai-loading {
      @apply !cursor-wait;
    }
...
<style lang="scss">
.cv-modal-wrapper.cv-modal-view-create-wrapper {
  .ant-modal-content {
    @apply !rounded-5;
  }
}
