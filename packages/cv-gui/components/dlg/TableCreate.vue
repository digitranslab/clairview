...
import type { TableType } from 'clairview-sdk'
...
const fullAuto = async (e) => {
  ...
  if (
    ...
    aiLoading.value ||
    aiError.value ||
    target.closest('button, input, .cv-button, textarea, .ant-tag')
  ) {
    return
  }
...
<template>
  <NcModal
    ...
    wrap-class-name="cv-modal-table-create-wrapper"
  ...
  >
    <div class="py-5 flex flex-col gap-5" @dblclick.stop="fullAuto">
      <div class="px-5 flex justify-between w-full items-center">
        ...
        <!-- <a href="https://docs.clairview.com/tables/create-table" target="_blank" class="text-[13px]">
          {{ $t('title.docs') }}
        </a> -->
        <div
      ...
      >
        <div class="flex flex-col gap-5">
          <a-form-item v-if="!aiMode" v-bind="validateInfos.title" class="relative cv-table-input-wrapper relative">
            <a-input
              ref="inputEl"
              v-model:value="table.title"
              class="cv-table-input cv-input-sm cv-input-shadow"
              hide-details
                ...
                <div class="px-5 pt-5 pb-2">
                  ...
                  <div v-else-if="aiModeStep === 'init'">
                    <div class="text-cv-content-purple-light text-sm h-7 flex items-center gap-2">
                      ...
                      <div class="cv-animate-dots">Auto suggesting tables for {{ base?.title }}</div>
                    </div>
                  </div>
                        ...
                        <template v-for="t of activeTabPredictedTables" :key="t.title">
                          <NcTooltip :disabled="activeTabSelectedTables.length < maxSelectionCount || t.selected">
                            ...
                            <a-tag
                              ...
                              :class="{
                                'cv-disabled': isAiSaving || (!t.selected && activeTabSelectedTables.length >= maxSelectionCount),
                                'cv-selected': t.selected,
                              }"
              ...
              <template #PromptContent>
                <div class="px-5 pt-5 pb-2 flex flex-col gap-5">
                  ...
                  <div v-else-if="isPromtAlreadyGenerated" class="flex flex-col gap-3">
                    <div class="text-cv-content-purple-dark font-semibold text-xs">Generated Table(s)</div>
        ...
        <div v-if="isAdvanceOptVisible && !aiMode" class="cv-table-advanced-options" :class="{ active: isAdvanceOptVisible }">
          <div>
            <div class="mb-1">
              <!-- Add Default Columns -->
...
:deep(.ant-form-item.cv-table-input-wrapper) {
  &.cv-ai-mode {
    .cv-ai-mode-table-input-wrapper {
...
<style lang="scss">
.cv-modal-wrapper.cv-modal-table-create-wrapper {
  .ant-modal-content {
    @apply !rounded-5;
  }
}
