...
import { ProjectRoles, ViewLockType } from 'clairview-sdk'
...
<template>
  <NcModal v-model:visible="vModel" wrap-class-name="cv-modal-re-assign" width="448px">
    <div class="mb-5">
      <div class="flex text-base font-bold mb-2">Re-assign this view</div>
      <div class="flex text-cv-content-gray-subtle">
    ...
    <div class="mb-5">
      ...
      <UserItem :user="currentOwner" class="bg-cv-bg-gray-light rounded-lg" />
    </div>
    <div class="mb-5">
      ...
      >
        ...
        <div v-else class="flex flex-row items-center gap-x-2 h-12.5 p-2 cv-list-user-item">
          <GeneralIcon icon="search" class="text-gray-500 ml-2" />
          <input
            :ref="inputEl"
            v-model="searchQuery"
            placeholder="Search User to assign..."
            class="border-0 px-2.5 outline-none cv-search-input"
          />
        </div>

        <div v-if="!selectedUser || userSelectMenu" class="max-h-65 overflow-auto cv-scrollbar-thin">
          <UserItem
            v-for="user of filterdBaseUsers"
            :key="user.id"
            class="cursor-pointer hover:(bg-gray-100) cv-list-user-item"
...
.cv-modal-re-assign {
  ...
  .cv-list-user-item:not(:last-of-type) {
