...
const configs = computed(() => [
  {
    ...
    docsLink: 'https://docs.clairview.com/account-settings/oss-specific-details#configure-email',
    buttonClick: () => {
  ...
  },
  {
    ...
    docsLink: 'https://docs.clairview.com/account-settings/oss-specific-details#configure-storage',
    buttonClick: () => {
  ...
  //   docsLink: 'https://docs.clairview.com',
  //   buttonClick: () => {
...
<template>
  <div class="flex flex-col" data-test-id="cv-setup-main">
    <NcPageHeader>
      <template #icon>
    ...
    <div
      class="cv-content-max-w flex-1 max-h-[calc(100vh_-_100px)] overflow-y-auto cv-scrollbar-thin flex flex-col items-center gap-6 p-6"
    >
      <div class="flex flex-col gap-6 w-150">
        <div
          ...
          :data-testid="`cv-setup-${config.key}`"
        ...
        >
          <div class="flex gap-3 items-center" data-rec="true">
            <NcTooltip v-if="!config.configured || config.isPending">
              ...
              <GeneralIcon icon="ncAlertCircle" class="text-orange-500 -mt-1 w-6 h-6 cv-pending" />
            </NcTooltip>
            <GeneralIcon v-else icon="circleCheckSolid" class="text-success w-6 h-6 bg-white-500 cv-configured" />
