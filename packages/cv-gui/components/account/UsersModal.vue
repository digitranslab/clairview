...
import type { OrgUserReqType } from 'clairview-sdk'
...
<template>
  <a-modal
    ...
    wrap-class-name="cv-modal-invite-user"
    @cancel="emit('closed')"
  >
    <div class="flex flex-col">
      <div class="flex flex-row justify-between items-center pb-1.5 mb-2 border-b-1 w-full">
        ...
        <a-button type="text" class="!rounded-md mr-1 -mt-1.5" @click="emit('closed')">
          <template #icon>
            <MaterialSymbolsCloseRounded data-testid="cv-root-user-invite-modal-close" class="flex mx-auto" />
          </template>
        </a-button>
      </div>
                ...
                <div class="flex flex-col w-2/4">
                  <a-form-item name="role" :rules="[{ required: true, message: $t('msg.roleRequired') }]">
                    ...
                    <NcSelect
                      ...
                      dropdown-class-name="cv-dropdown-user-role max-w-64"
                    >
                      <a-select-option
                        v-for="(option, idx) of userRoleOptions"
                        :key="idx"
                        class="cv-role-option"
                        :value="option.value"
                      >
