  ...
  return [
    {
      ...
      url: `https://docs.clairview.com/account-settings/oss-specific-details#configure-${plugin.value?.category?.toLowerCase()}`,
    },
...
<template>
  <div class="flex flex-col h-full h-[calc(100vh_-_40px)]" data-testid="cv-setup-config">
    <NcPageHeader>
      <template #title>
    ...
    <div class="h-full flex h-[calc(100%_-_48px)]">
      <div class="cv-config-left-panel cv-scrollbar-thin relative h-full flex flex-col">
        <div class="w-full flex items-center gap-3 border-gray-200 py-6 px-6">
          ...
          <div class="flex gap-2">
            <NcButton
              ...
              :data-testid="`cv-setup-config-action-${action.key?.toLowerCase()}`"
      ...
      <div class="cv-config-right-panel">
...
.cv-config-left-panel {
  @apply w-full flex-1 flex justify-stretch;
}

.cv-config-right-panel {
