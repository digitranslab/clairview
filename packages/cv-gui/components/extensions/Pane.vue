...
const handleAutoScroll = async (id: string) => {
  ...
  const extension = document.querySelector(`.cv-extension-list-wrapper .cv-extension-item-${id}`)
    ...
    <Transition name="layout" :duration="150">
      <div v-if="isPanelExpanded" class="flex flex-col h-full">
          ...
          >
            ...
            <div v-else class="flex flex-grow items-center justify-end !max-w-[300px]">
              <a-input
                ...
                type="text"
                class="cv-input-border-on-value !h-7 !px-3 !py-1 !rounded-lg"
        ...
        <template v-if="extensionList.length === 0">
          <div class="flex-1 flex items-center justify-center flex-col gap-4 w-full cv-scrollbar-md text-center p-4">
            <div class="text-base font-bold text-cv-content-gray">Supercharge Your Workflow with Extensions</div>
            <div class="text-sm text-cv-content-gray-subtle2">
        ...
        <template v-else>
          <Draggable
            ...
            draggable=".cv-extension-item"
            item-key="id"
            handle=".cv-extension-drag-handler"
            ghost-class="ghost"
            class="cv-extension-list-wrapper flex items-center flex-col gap-3 w-full cv-scrollbar-md py-4"
            :class="{
          ...
          >
            <template #item="{ element: ext }">
              <div class="cv-extension-item w-full" :class="`cv-extension-item-${ext.id}`">
