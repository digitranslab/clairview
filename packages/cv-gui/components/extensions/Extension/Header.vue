...
<template>
  <div
    ...
    :class="{
      'border-b-1 border-cv-border-gray-medium h-[49px]': !collapsed && !isFullscreen,
      'collapsed border-transparent h-[48px]': collapsed && !isFullscreen,
      'px-3 py-2 gap-1': !isFullscreen,
      'gap-3 px-4 pt-3 pb-[11px] border-b-1 border-cv-border-gray-medium': isFullscreen,
    }"
    @click="expandExtension"
  >
    <slot v-if="isFullscreen" name="prefix"></slot>
    <NcButton v-if="!isFullscreen" size="xs" type="text" class="cv-extension-drag-handler !px-1" @click.stop>
    ...
    <template v-if="!isFullscreen">
      <NcButton
        ...
        size="xs"
        type="text"
        class="cv-extension-expand-btn !px-1"
