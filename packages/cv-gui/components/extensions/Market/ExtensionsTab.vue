...
<template>
  <div class="h-full py-4 overflow-auto cv-scrollbar-thin">
    <div class="h-full flex flex-col gap-5 flex-1 pt-2 px-6 w-full mx-auto">
      <div class="text-base font-bold text-cv-content-gray">Popular Extensions</div>
      <div v-if="searchQuery" class="text-base text-cv-content-gray-subtle">Search result for ‘{{ searchQuery }}’</div>
      ...
      >
        <template v-for="ext of filteredAvailableExtensions" :key="ext.id">
          <div
            class="cv-market-extension-item flex items-center gap-3 border-1 rounded-xl p-3 cursor-pointer hover:bg-gray-50 transition-all"
          ...
          >
            ...
            <div class="flex-1 flex flex-grow flex-col gap-2">
              <div>
                <div class="text-sm font-bold text-cv-content-gray line-clamp-1">
                  {{ ext.title }}
                </div>
                <div v-if="ext.publisher?.name" class="mt-0.5 text-xs leading-[18px] text-cv-content-gray-muted line-clamp-1">
                  Built by {{ ext.publisher.name }}
                </div>
              </div>

              <div class="max-h-[36px] text-small leading-[18px] text-cv-content-gray-subtle line-clamp-2">
                {{ ext.subTitle }}
              </div>
            </div>
