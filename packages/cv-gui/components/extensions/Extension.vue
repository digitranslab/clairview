...
<template>
  <div ref="extensionRef" class="w-full px-4" :class="`cv-${extensionManifest?.id}`" :data-testid="extension.id">
    <div
    ...
    >
      ...
      <template v-if="activeError">
        <div
          v-show="!collapsed"
          class="extension-content cv-scrollbar-thin h-[calc(100%_-_50px)] flex items-center justify-center"
          :class="{
            fullscreen,
          }"
        >
          <a-result status="error" title="Extension Error" class="cv-extension-error">
      ...
      <template v-else>
        <Teleport to="body" :disabled="!fullscreen">
          <div
            ...
            :class="[
              fullscreen ? `cv-${extensionManifest?.id}` : '',
          ...
          >
            ...
            >
              <div
                ...
                :class="{ 'fullscreen': fullscreen, 'h-full cv-scrollbar-thin': !fullscreen }"
              >
...
:deep(.cv-extension-error.ant-result) {
  @apply p-0;
  .ant-result-icon {
    @apply mb-3;
    & > span {
