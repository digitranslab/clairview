...
import { dateFormats, isSystemColumn, timeFormats } from 'clairview-sdk'
...
onClickOutside(datePickerRef, (e) => {
  if ((e.target as HTMLElement)?.closest(`.${randomClass}, .cv-${randomClass}`)) return
  ...
  (next) => {
    if (next) {
      ...
      onClickOutside(document.querySelector(`.${randomClass}`)! as HTMLDivElement, (e) => {
        if ((e?.target as HTMLElement)?.closest(`.cv-${randomClass}`)) {
          return
        }
        open.value = false
      })
...
<template>
  <div v-bind="$attrs" class="cv-cell-field relative">
    <NcDropdown
      :visible="isOpen"
      :placement="isDatePicker ? 'bottomLeft' : 'bottomRight'"
      :auto-close="false"
      :trigger="['click']"
      class="cv-cell-picker-datetime"
      :class="[`cv-${randomClass}`, { 'cv-null': modelValue === null && showNull }]"
      :overlay-class-name="`${randomClass} cv-picker-datetime ${open ? 'active' : ''} !min-w-[0] overflow-hidden`"
    >
      <div
        :title="localState?.format(dateTimeFormat)"
        class="cv-date-picker ant-picker-input flex relative !w-auto gap-2"
        :class="{
      ...
      >
        ...
        >
          <input
            ...
            class="cv-date-input w-full !truncate border-transparent outline-none !text-current !bg-transparent !focus:(border-none outline-none ring-transparent)"
        ...
        >
          <input
            ...
            class="cv-time-input w-full !truncate border-transparent outline-none !text-current !bg-transparent !focus:(border-none outline-none ring-transparent)"
            :readonly="!!isMobileMode || isColDisabled"
            @focus="onFocus(false)"
    ...
    <GeneralIcon
      ...
      icon="closeCircle"
      class="cv-clear-date-time-icon cv-action-icon h-4 w-4 absolute right-0 top-[50%] transform -translate-y-1/2 invisible cursor-pointer"
