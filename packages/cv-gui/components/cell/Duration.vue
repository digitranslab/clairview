...
<template>
  <div class="duration-cell-wrapper">
    ...
    <input
      ...
      :ref="focus"
      v-model="localState"
      class="cv-cell-field w-full !border-none !outline-none py-1"
    ...
    <span v-else-if="modelValue === null && showNull" class="cv-cell-field cv-null uppercase">{{ $t('general.null') }}</span>

    <span v-else class="cv-cell-field"> {{ localState }}</span>

    <div v-if="showWarningMessage && showValidationError" class="cv-cell-field duration-warning">
