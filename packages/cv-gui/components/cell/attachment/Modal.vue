...
<template>
  <NcModal
    ...
    wrap-class-name="cv-modal-attachment-expand-cell"
    class="cv-attachment-modal"
  ...
  >
    ...
    <div ref="dropZoneRef" tabindex="0" class="relative">
      ...
      <div
        ref="sortableRef"
        :class="{ dragging }"
        class="grid max-h-140 overflow-auto cv-scrollbar-md md:grid-cols-3 xl:grid-cols-5 gap-y-8 gap-x-4 relative"
      >
        <div
          v-for="(item, i) in visibleItems"
          :key="`${item?.title}-${i}`"
          class="cv-attachment-item group gap-1 flex border-1 rounded-md border-gray-200 flex-col relative"
        >
          <NcCheckbox
            v-model:checked="selectedVisibleItems[i]"
            class="cv-attachment-checkbox absolute top-2 left-2 group-hover:(opacity-100)"
          ...
          <div class="relative px-1 pb-1 items-center flex" :title="item.title">
            ...
            <div class="flex-none hide-ui transition-all transition-ease-in-out !h-5 gap-0.5 flex items-center bg-white">
              ...
              <NcTooltip v-if="!isSharedForm || (!readOnly && isUIAllowed('dataEdit') && !isPublic)" placement="bottom">
                ...
                <NcButton
                  size="xsmall"
                  class="!p-0 cv-attachment-rename !h-5 !w-5 !text-gray-500 !min-w-[fit-content] gap-2"
                  type="text"
              ...
              <NcTooltip v-if="isSharedForm || (!readOnly && isUIAllowed('dataEdit') && !isPublic)" placement="bottom">
                ...
                <NcButton
                  class="!p-0 !h-4 !w-4 !text-red-500 cv-attachment-remove !min-w-[fit-content]"
                  size="xsmall"
                  type="text"
        ...
        <div v-if="isLoading" class="flex flex-col gap-1">
          <a-card class="cv-attachment-item group">
            <div class="cv-attachment h-full w-full flex items-center justify-center">
