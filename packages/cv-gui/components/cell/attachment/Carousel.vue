...
useEventListener(container, 'click', (e) => {
  ...
  if (!target.closest('.keep-open') && !target.closest('.cv-button') && !target.closest('img') && !target.closest('video')) {
      ...
      >
            ...
            <NcCarouselItem v-for="(item, index) in visibleItems" :key="index">
              <div v-if="selectedIndex === index" :key="isUpdated" class="justify-center w-full h-full flex items-center">
                <LazyCellAttachmentPreviewImage
                  ...
                  class="cv-attachment-img-wrapper"
                  object-fit="contain"
                  controls
                  :alt="item.title"
            ...
            <NcCarouselContent class="!flex !gap-2">
              ...
              >
                <div class="flex items-center justify-center">
                  <LazyCellAttachmentPreviewImage
                    v-if="isImage(item.title, item.mimeType)"
                    class="cv-attachment-img-wrapper h-12"
                    object-fit="contain"
                    :alt="item.title"
        ...
        <div class="absolute keep-open right-2 z-30 bottom-3 transition-all gap-3 transition-ease-in-out !h-6 flex items-center">
          ...
          >
            ...
            <NcButton
              size="xsmall"
              class="cv-attachment-rename !hover:text-gray-400 !hover:bg-transparent !text-white"
              type="text"
