...
useSelectedCellKeyupListener(inject(ActiveCellInj, ref(false)), (e) => {
  ...
  if (e.key === 'Enter' && !isReadonly.value && !selectedFile.value) {
    ...
    if (!modalVisible.value && !isMobileMode.value) {
      ...
      ;(document.querySelector('.cv-attachment-modal.active .cv-attach-file') as HTMLDivElement)?.click()
    }
  }
})
...
<template>
  <div v-if="isExpandedForm || isForm" class="form-attachment-cell">
    ...
    <div v-if="visibleItems.length > 0" class="grid mt-2 gap-2 grid-cols-2">
      <div
        ...
        :key="`${item?.title}-${i}`"
        class="cv-attachment-item group gap-2 flex border-1 rounded-md border-gray-200 flex-col relative"
      >
        <div
          :class="[dragging ? 'cursor-move' : 'cursor-pointer']"
          class="cv-attachment h-full flex justify-center items-center overflow-hidden"
        >
        ...
        <div class="relative px-1 flex" :title="item.title">
          ...
          <div class="flex-none hide-ui transition-all transition-ease-in-out !h-5 gap-0.5 pb-2 flex items-center bg-white">
            ...
            <NcTooltip v-if="!isSharedForm || (!isReadonly && isUIAllowed('dataEdit') && !isPublic)" placement="bottom">
              ...
              <NcButton
                size="xsmall"
                class="!p-0 cv-attachment-rename !h-5 !w-5 !text-gray-500 !min-w-[fit-content]"
                type="text"
            ...
            <NcTooltip v-if="!isReadonly" placement="bottom">
              ...
              <NcButton
                class="!p-0 !h-5 !w-5 !text-red-500 cv-attachment-remove !min-w-[fit-content]"
                size="xsmall"
                type="text"
  ...
  <div v-else ref="attachmentCellRef" class="cv-attachment-cell relative group color-transition">
    <LazyCellAttachmentCarousel v-if="selectedFile" />
    ...
    <template v-if="!isReadonly && !dragging && !!currentCellRef">
      <general-overlay
        ...
        data-rec="true"
        class="cv-attachment-cell-dropzone text-white text-lg bg-gray-600/75 flex text-sm items-center justify-center gap-2"
      >
    ...
    <template v-if="visibleItems.length > 0">
      <div
        ...
        class="cv-attachment-wrapper flex cursor-pointer w-full items-center flex-wrap gap-2 mt-0 items-start overflow-y-auto cv-scrollbar-thin"
        :style="{
          height: `max(${
      ...
      >
        <NcTooltip v-for="(item, i) of visibleItems" :key="item.url || item.title" placement="bottom" class="cv-attachment-item">
          <template #title>
          ...
          >
            <LazyCellAttachmentPreviewImage
              ...
              :alt="item.title || `#${i}`"
              class="cv-attachment rounded-lg w-full h-full object-cover overflow-hidden"
              :srcs="getPossibleAttachmentSrc(item, attachmentSize)"
              @click="() => onFileClick(item)"
            />
            <div v-else class="cv-attachment h-full w-full flex items-center justify-center" @click="onFileClick(item)">
      ...
      <NcTooltip
        placement="bottom"
        class="cv-action-icon !absolute hidden right-0 cv-text-area-expand-btn !group-hover:block z-3"
        :class="{
      ...
      <NcTooltip
        placement="bottom"
        class="cv-action-icon !absolute hidden left-0 cv-text-area-expand-btn !group-hover:block z-3"
        :class="{
          'top-0': isGrid && !(!rowHeight || rowHeight === 1),
          'top-1': !isGrid,
        }"
    ...
    <template #entity-preview>
      <span>
        <div class="flex flex-row items-center py-2.25 px-2.5 bg-gray-50 rounded-lg text-gray-700 mb-4">
          <GeneralIcon icon="file" class="cv-view-icon"></GeneralIcon>
          <div
