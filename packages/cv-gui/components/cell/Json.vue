...
onClickOutside(inputWrapperRef, (e) => {
  if ((e.target as HTMLElement)?.closest('.cv-json-action')) return
  editEnabled.value = false
})
...
watch(inputWrapperRef, () => {
  ...
  const modal = document.querySelector('.cv-json-expanded-modal') as HTMLElement
...
<template>
  <component
    ...
    centered
    :footer="null"
    :wrap-class-name="isExpanded ? '!z-1051 cv-json-expanded-modal' : null"
  >
    <div v-if="editEnabled && !readOnly" class="flex flex-col w-full" @mousedown.stop @mouseup.stop @click.stop>
      <div class="flex flex-row justify-between pt-1 pb-2 cv-json-action" @mousedown.stop>
        ...
        <div v-if="!isForm || isExpanded" class="flex flex-row my-1 space-x-1">
          ...
          <a-button
            ...
            size="small"
            class="cv-save-json-value-btn !rounded-lg"
            :class="{
              'cv-edit-modal': !isExpanded,
            }"
      ...
      <span v-if="error" class="cv-cell-field text-xs w-full py-1 text-red-500">
        {{ error.toString() }}
      </span>
    </div>

    <span v-else-if="vModel === null && showNull" class="cv-cell-field cv-null uppercase">{{ $t('general.null') }}</span>

    <LazyCellClampedText v-else :value="vModel ? stringifyProp(vModel) : ''" :lines="rowHeight" class="cv-cell-field" />
  </component>
</template>
...
.cv-save-json-value-btn {
  &.cv-edit-modal:not(:disabled) {
