...
<template>
  <NcDropdown
    ...
    :overlay-class-name="`${randomClass} cv-picker-year ${open ? 'active' : ''} !min-w-[260px]`"
  >
    <div
      v-bind="$attrs"
      :title="localState?.format('YYYY')"
      class="cv-year-picker flex items-center justify-between ant-picker-input relative"
    >
      <input
        ref="datePickerRef"
        type="text"
        ...
        class="cv-year-input border-none outline-none !text-current bg-transparent !focus:(border-none outline-none ring-transparent)"
      ...
      <GeneralIcon
        ...
        icon="closeCircle"
        class="cv-clear-year-icon cv-action-icon absolute right-0 top-[50%] transform -translate-y-1/2 invisible cursor-pointer"
