...
import { isDateMonthFormat, isSystemColumn } from 'clairview-sdk'
...
const onBlur = (e) => {
  ...
  if (
    (e?.relatedTarget as HTMLElement)?.closest(`.${randomClass}, .cv-${randomClass}`) ||
    (e?.target as HTMLElement)?.closest(`.${randomClass}, .cv-${randomClass}`)
  ) {
    return
  }
...
<template>
  <NcDropdown
    ...
    :overlay-class-name="`${randomClass} cv-picker-date ${open ? 'active' : ''} !min-w-[260px]`"
  >
    <div
      v-bind="$attrs"
      :title="localState?.format(dateFormat)"
      class="cv-date-picker h-full flex items-center justify-between ant-picker-input relative"
    >
      <input
        ref="datePickerRef"
        type="text"
        ...
        class="cv-date-input border-none outline-none !text-current bg-transparent !focus:(border-none outline-none ring-transparent)"
      ...
      <GeneralIcon
        ...
        icon="closeCircle"
        class="cv-clear-date-icon cv-action-icon absolute right-0 top-[50%] transform -translate-y-1/2 invisible cursor-pointer"
