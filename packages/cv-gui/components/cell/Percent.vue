...
const onTabPress = (e: KeyboardEvent) => {
  if (e.shiftKey && (isExpandedFormOpen.value || isForm.value)) {
    ...
    const focusesNcCellIndex = Array.from(
      document.querySelectorAll(`${isExpandedFormOpen.value ? '.cv-expanded-form-row' : '.cv-form-wrapper'} .cv-data-cell`),
    ).findIndex((el) => {
      return el.querySelector('.cv-filter-value-select') === wrapperRef.value
    })

    if (focusesNcCellIndex >= 0) {
      const nodes = document.querySelectorAll(
        `${isExpandedFormOpen.value ? '.cv-expanded-form-row' : '.cv-form-wrapper'} .cv-data-cell`,
      )
...
<template>
  <div
    ...
    class="cv-filter-value-select w-full focus:outline-transparent"
  ...
  >
    ...
    <input
      ...
      :ref="focus"
      v-model="vModel"
      class="cv-cell-field w-full !border-none !outline-none focus:ring-0 py-1"
      :type="inputType"
    ...
    <span v-else class="cv-cell-field">{{ vModel }} {{ !vModel ? '&nbsp;' : '' }}</span>
  </div>
</template>
