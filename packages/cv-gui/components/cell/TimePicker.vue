...
import { isSystemColumn } from 'clairview-sdk'
...
<template>
  <NcDropdown
    ...
    :overlay-class-name="`${randomClass} cv-picker-time ${isOpen ? 'active' : ''} !min-w-[0]`"
  >
    <div
      v-bind="$attrs"
      :title="localState?.format('HH:mm')"
      class="cv-time-picker h-full flex items-center justify-between ant-picker-input relative"
    >
      <input
        ref="datePickerRef"
        type="text"
        ...
        class="cv-time-input border-none outline-none !text-current bg-transparent !focus:(border-none outline-none ring-transparent)"
      ...
      <GeneralIcon
        ...
        icon="closeCircle"
        class="cv-clear-time-icon cv-action-icon absolute right-0 top-[50%] transform -translate-y-1/2 invisible cursor-pointer"
