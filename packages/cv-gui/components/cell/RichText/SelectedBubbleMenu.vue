...
const onToggleLink = () => {
  ...
  if (isActiveNodeMarkActive) {
    ...
    setTimeout(() => {
      const linkInput = document.querySelector('.cv-text-area-rich-link-option-input')
      if (linkInput) {
...
<template>
  <div
    ...
    :class="{
      'cv-form-field-bubble-menu inline-flex py-0': isFormField,
...
.bubble-menu.embed-mode:not(.cv-form-field-bubble-menu) {
...
.embed-mode.bubble-menu:not(.cv-form-field-bubble-menu) {
