...
import type { AIRecordType } from 'clairview-sdk'
...
onClickOutside(inputWrapperRef, (e) => {
  if ((e.target as HTMLElement)?.className.includes('cv-long-text-toggle-expand')) return

  const targetEl = e?.target as HTMLElement

  if (
    targetEl?.closest(
      '.bubble-menu, .tippy-content, .cv-textarea-rich-editor, .tippy-box, .mention, .cv-mention-list, .tippy-content',
    )
  ) {
    return
  }
...
watch(
  position,
  () => {
    const dom = document.querySelector('.cv-long-text-expanded-modal .ant-modal-content') as HTMLElement
...
watch(inputWrapperRef, () => {
  ...
  const modal = document.querySelector('.cv-long-text-expanded-modal') as HTMLElement
...
const STORAGE_KEY = 'cv-long-text-expanded-modal-size'
...
const updateSize = () => {
  try {
    ...
    let elem = document.querySelector('.cv-text-area-expanded') as HTMLElement

    if (isRichMode.value) {
      elem = document.querySelector('.cv-long-text-expanded-modal .cv-textarea-rich-editor .tiptap') as HTMLElement
    }
  ...
  (value) => {
    ...
    if (value) {
      if (isRichMode.value && isVisible.value) {
        setTimeout(() => {
          const el = document.querySelector('.cv-long-text-expanded-modal .cv-textarea-rich-editor .tiptap') as HTMLElement
...
<template>
  <div
    :class="{
      'cv-expanded-form-open': isExpandedFormOpen,
    }"
  >
    <div
    ...
    >
      ...
      <div
        ...
        class="w-full cursor-pointer cv-readonly-rich-text-wrapper"
        :class="{
          'cv-readonly-rich-text-grid ': !isExpandedFormOpen && !isForm,
          'cv-readonly-rich-text-sort-height': localRowHeight === 1 && !isExpandedFormOpen && !isForm,
        }"
        :style="{
          maxHeight: isForm
            ? undefined
      ...
      <div
        ...
        class="h-full w-full"
        :class="{
          'my-1 bg-cv-bg-purple-light rounded-lg': props.isAi && isExpandedFormOpen && !readOnly,
        }"
      >
        <textarea
          ref="textAreaRef"
          v-model="vModel"
          :rows="isForm ? 5 : 4"
          class="h-full w-full !outline-none cv-scrollbar-thin"
          :class="{
            'p-2': editEnabled,
            'py-1 h-full': isForm,
            'px-2': isExpandedFormOpen,
            'border-none': !(props.isAi && isExpandedFormOpen),
            'border-1 border-cv-border-gray-medium rounded-lg !focus:(shadow-selected border-primary ring-0) transition-shadow duration-300':
        ...
        <div v-if="!readOnly && props.isAi && isExpandedFormOpen" class="-mt-1">
          <div v-if="props.aiMeta?.isStale" ref="aiWarningRef">
            <div class="flex items-start p-3 bg-cv-bg-purple-light gap-4">
              <GeneralIcon icon="alertTriangleSolid" class="text-cv-content-purple-medium h-4 w-4 flex-none" />
              <div class="flex flex-col">
                <div class="font-bold text-small leading-[18px] text-cv-content-gray">Record Data Updated</div>
                <div class="text-small leading-[18px] text-cv-content-gray-muted">
          ...
          <div v-if="!isEditColumn" class="flex items-center gap-2 px-3 py-0.5 !text-small leading-[18px]">
            <span class="text-cv-content-purple-light truncate">Generated by AI</span>
            <NcTooltip v-if="isAiEdited" class="text-cv-content-green-dark flex-1 truncate" show-on-truncate-only>
              <template #title> Edited by you </template>
              Edited by you
            </NcTooltip>
            <NcTooltip
              v-else-if="props.aiMeta?.lastModifiedBy && idUserMap[props.aiMeta?.lastModifiedBy]"
              class="text-cv-content-green-dark flex-1 truncate"
      ...
      <span v-else-if="vModel === null && showNull" class="cv-null uppercase">{{ $t('general.null') }}</span>

      <LazyCellClampedText
        v-else-if="rowHeight"
        :value="vModel"
        :lines="rowHeightTruncateLines(localRowHeight)"
        class="cv-text-area-clamped-text"
        :style="{
      ...
      <div
        class="!absolute !hidden cv-text-area-expand-btn group-hover:block z-3 flex items-center gap-1"
        :class="{
          'right-1': isForm,
          'right-0': !isForm,
      ...
      >
        ...
        <NcTooltip v-if="!isVisible && !isForm" placement="bottom" class="cv-action-icon">
    ...
    <a-modal
      ...
      :closable="false"
      :footer="null"
      wrap-class-name="cv-long-text-expanded-modal"
      :mask="true"
      :mask-closable="false"
    ...
    >
      ...
      >
        ...
        >
          ...
          <template v-if="props.isAi && !isEditColumn">
            <div class="flex items-center text-small leading-[18px] gap-3 ml-2">
              <span class="text-cv-content-purple-dark truncate">Generated by AI</span>
              <template v-if="!readOnly">
                <span v-if="isAiEdited" class="text-cv-content-green-dark truncate"> Edited by you </span>
        ...
        <div v-if="props.isAi && props.aiMeta?.isStale && !readOnly" ref="aiWarningRef" class="border-b-1 border-gray-100">
          <div class="flex items-center p-4 bg-cv-bg-purple-light gap-4">
            <GeneralIcon icon="alertTriangleSolid" class="text-cv-content-purple-medium h-6 w-6 flex-none" />
            <div class="flex flex-col">
              <div class="font-bold text-base text-cv-content-gray">Record Data Updated</div>
              <div class="text-cv-content-gray-muted text-sm">
        ...
        <div v-if="!isRichMode" class="p-3 pb-0 h-full">
          <a-textarea
            ref="inputRef"
            v-model:value="vModel"
            class="cv-text-area-expanded !py-1 !px-3 !text-black !transition-none !cursor-text !min-h-[210px] !rounded-lg focus:border-brand-500 disabled:!bg-gray-50 cv-longtext-scrollbar"
...
.cv-longtext-scrollbar {
  @apply scrollbar-thin scrollbar-thumb-gray-200 hover:scrollbar-thumb-gray-300 scrollbar-track-transparent;
}

.cv-readonly-rich-text-wrapper {
  &.cv-readonly-rich-text-grid {
    :deep(.ProseMirror) {
      @apply !pt-0;
    }
    // &.cv-readonly-rich-text-sort-height {
    //   @apply mt-1;
    // }
  }
}
...
<style lang="scss">
.cell:hover .cv-text-area-expand-btn,
.long-text-wrapper:hover .cv-text-area-expand-btn {
...
.cv-data-cell {
  &:has(.cv-cell-longtext-ai .cv-expanded-form-open) {
    ...
    .cv-text-area-expand-btn {
      @apply top-2 right-1;
    }
  }
}

.cv-long-text-expanded-modal {
  .ant-modal {
