...
import type { UserFieldRecordType } from 'clairview-sdk'
...
const handleClose = (e: MouseEvent) => {
  ...
  if (
    ...
    !document.querySelector('.cv-dropdown-user-select-cell.active')?.contains(e.target as Node)
  ) {
...
<template>
  <div
    class="cv-cell-field cv-user-select h-full w-full flex items-center"
  ...
  >
      ...
      >
        <template v-for="op of options" :key="op.id || op.email">
          <component
            ...
            :class="`cv-select-option-${column.title}-${op.email}`"
          >
    ...
    <template v-else>
      ...
      <a-select
        ...
        :dropdown-class-name="`cv-dropdown-user-select-cell !min-w-156px ${isOpen ? 'active' : ''}`"
      ...
      >
        ...
        <template #tagRender="{ label, value: val, onClose }">
          <a-tag
            ...
            class="rounded-tag cv-selected-option !pl-0"
            :class="{
              '!my-0': !rowHeight || rowHeight === 1,
            }"
