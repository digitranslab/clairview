...
import type { SelectOptionType, SelectOptionsType } from 'clairview-sdk'
...
const handleClose = (e: MouseEvent) => {
  ...
  if (
    ...
    !document.querySelector('.cv-dropdown-multi-select-cell.active')?.contains(e.target as Node)
  ) {
...
<template>
  <div
    class="cv-cell-field cv-multi-select h-full w-full flex items-center"
  ...
  >
    <div v-if="!isEditColumn && isForm && parseProp(column.meta)?.isList" class="w-full max-w-full">
      <a-checkbox-group v-model:value="vModel" :disabled="readOnly || !editAllowed" class="cv-field-layout-list" @click.stop>
        <a-checkbox
          v-for="op of options"
          :key="op.title"
          :value="op.title"
          ...
          :class="`cv-select-option-${column.title}-${op.title}`"
        >
    ...
    <template v-else>
      ...
      <a-select
        ...
        :dropdown-class-name="`cv-dropdown-multi-select-cell !min-w-156px ${isOpen ? 'active' : ''}`"
        @search="search"
        @keydown="onKeyDown"
        @focus="onFocus"
        @blur="isOpen = false"
      >
        <template #suffixIcon>
          <GeneralIcon icon="arrowDown" class="text-gray-700 cv-select-expand-btn" />
        </template>
        <a-select-option
          v-for="op of options"
          :key="op.id || op.title"
        ...
        <template #tagRender="{ value: val, onClose }">
          <a-tag
            ...
            class="rounded-tag cv-selected-option"
            :class="{
              '!my-0': !rowHeight || rowHeight === 1,
            }"
