  ...
  onBlur: (e) => {
    if (
      !(e?.event?.relatedTarget as HTMLElement)?.closest(
        '.bubble-menu, .cv-textarea-rich-editor, .cv-rich-text, .tippy-box, .mention, .cv-mention-list, .tippy-content',
      )
    ) {
...
<template>
  <div
    class="cv-rich-text h-full focus:outline-none"
    :class="{
      'flex flex-col flex-grow cv-rich-text-full': fullMode,
      'cv-rich-text-embed flex flex-col pl-1 w-full': !fullMode,
      'readonly': readOnly,
      'cv-form-rich-text-field !p-0 relative': isFormField,
  ...
  >
    ...
    <template v-else>
      ...
      <EditorContent
        ref="editorDom"
        :editor="editor"
        class="flex flex-col cv-textarea-rich-editor w-full"
        :class="{
          ...
          [`!overflow-hidden cv-truncate cv-line-clamp-${rowHeightTruncateLines(localRowHeight)}`]:
      ...
      />
      <div v-if="isFormField && !readOnly" class="cv-form-field-bubble-menu-wrapper overflow-hidden">
        <div
...
.cv-rich-text-embed {
  ...
  &:not(.cv-form-rich-text-field):not(.cv-rich-text-grid) {
    .ProseMirror {
      min-height: 8rem;
    }
  }

  &.cv-form-rich-text-field {
    .ProseMirror {
      padding: 0;
    }
    &.readonly {
