...
const { isOpen } = useSidebar('cv-mini-sidebar', { isOpen: true })
...
<template>
  <a-layout-sider
    ...
    width="50"
    collapsed-width="0"
    class="cv-mini-sidebar !bg-primary h-full"
    :trigger="null"
    collapsible
    theme="light"
  >
    <a-dropdown placement="bottom" :trigger="['click']" overlay-class-name="cv-dropdown">
      <div class="transition-all duration-200 p-2 cursor-pointer transform hover:scale-105 cv-noco-brand-icon">
        <a-tooltip placement="bottom">
          <template #title>
            {{ currentVersion }}
          </template>
          <img width="35" alt="ClairView" src="~/assets/img/icons/256x256-trans.png" />
        </a-tooltip>
      </div>

      <template v-if="signedIn" #overlay>
        <a-menu class="ml-2 !py-0 min-w-32 leading-8 !rounded cv-menu-account">
          <a-menu-item-group title="User Settings">
            <a-menu-item key="email" class="!rounded-t">
              <nuxt-link v-e="['c:navbar:user:email']" class="group flex items-center no-underline py-2" to="/user">
                ...
                &nbsp;
                <span class="prose group-hover:text-black cv-user-menu-email">{{ email }}</span>
              </nuxt-link>
            </a-menu-item>
            ...
            <a-menu-item key="signout" class="!rounded-b">
              <div v-e="['a:navbar:user:sign-out']" class="group flex items-center py-2" @click="logout">
                ...
                <span class="prose font-semibold text-gray-500 group-hover:text-black cv-user-menu-signout">
                  {{ $t('general.signOut') }}
                </span>
              </div>
            </a-menu-item>
    ...
    <div id="sidebar" ref="sidebar" class="text-white flex-auto flex flex-col items-center w-full">
      <a-dropdown :trigger="['contextmenu']" placement="right" overlay-class-name="cv-dropdown">
        <div :class="[route.name === 'index' ? 'active' : '']" class="cv-mini-sidebar-item" @click="navigateTo('/')">
      ...
      <a-tooltip placement="right">
        ...
        <div
          :class="[route.name.includes('cv-baseId') ? 'active' : 'pointer-events-none !text-gray-400']"
          class="cv-mini-sidebar-item"
