...
<template>
  <a-dropdown v-model:visible="isOpen" :trigger="['click']" :disabled="readonly">
    <div
      class="flex-none flex flex-row justify-center items-center select-none rounded-md cv-emoji"
      :class="{
    ...
    >
      ...
      <template v-else>
        <Icon :data-testid="`cv-icon-${emojiRef}`" class="text-lg" :icon="emojiRef"></Icon>
      </template>
    </div>
    <template #overlay>
      <div
