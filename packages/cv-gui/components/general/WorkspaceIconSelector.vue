    ...
    <NcDropdown v-model:visible="isOpen" overlay-class-name="w-[432px]" @visible-change="onVisibilityChange">
      <div
        ...
        :class="{
          ...
          'border-cv-gray-medium': !isOpen && vIconType !== WorkspaceIconType.IMAGE,
      ...
      <template #overlay>
        <div class="pt-2 h-[320px]">
          <NcTabs v-model:activeKey="activeTab" class="cv-workspace-icon-dropdown-tabs h-full">
            <template #leftExtra>
              <div class="w-0"></div>
            </template>
            <template #rightExtra>
            ...
            <a-tab-pane :key="WorkspaceIconType.ICON" class="w-full" :disabled="isLoading">
              ...
              <div class="h-full overflow-auto cv-scrollbar-thin flex flex-col">
                <div class="!sticky top-0 flex gap-2 bg-white px-2 py-2">
                  <a-input
                    ref="inputRef"
                    v-model:value="searchQuery"
                    :placeholder="$t('placeholder.searchIcons')"
                    class="cv-dropdown-search-unified-input z-10"
                  >
                  </a-input>
                </div>
              ...
              <div class="p-2 flex flex-col gap-2.5 h-full">
                ...
                <div class="flex-1">
                  <a-upload-dragger
                    ...
                    class="cv-workspace-image-uploader"
                    :custom-request="customReqCbk"
                    :before-upload="beforeUpload"
                    @change="handleChange"
                    @reject="rejectDrop"
                  >
                    <div class="ant-upload-drag-icon !text-cv-content-gray-muted !mb-2 text-center">
                    ...
                    <div class="ant-upload-text !text-cv-content-gray-muted !text-sm">
                      Drop your icon here or <span class="text-cv-content-brand hover:underline">browse file</span>
            ...
            <a-tab-pane :key="WorkspaceIconType.EMOJI" class="w-full" :disabled="isLoading">
              ...
              <div class="h-full">
                <Picker
                  ...
                  class="cv-workspace-emoji-picker"
                  @select="selectEmoji"
                  @click.stop="() => {}"
                ></Picker>
              </div>
...
.cv-workspace-icon-dropdown-tabs {
  :deep(.ant-tabs-nav) {
    @apply px-3;
  ...
  :deep(.ant-tabs-tab-disabled) {
    .tab-title {
      @apply text-cv-content-gray-muted hover:bg-transparent;
    }
  }
}
...
:deep(.cv-workspace-avatar img) {
  @apply !cursor-pointer;
}
</style>

<style>
.cv-workspace-image-uploader {
  &.ant-upload.ant-upload-drag {
    @apply !rounded-lg !bg-white !hover:bg-cv-bg-gray-light !transition-colors duration-300;
  }
  .ant-upload-btn {
...
.cv-workspace-emoji-picker.emoji-mart {
