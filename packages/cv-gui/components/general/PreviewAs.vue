...
import { ProjectRoles } from 'clairview-sdk'
...
<template>
  <div
    v-if="float"
    v-show="previewAs"
    class="floating-reset-btn cv-floating-preview-btn p-4"
  ...
  >
    ...
    <div class="pointer flex items-center gap-4">
      ...
      <!-- Close -->
      <div class="flex items-center gap-2 cursor-pointer cv-preview-btn-exit-to-app" @click="previewAs = null">
  ...
  <template v-else>
    <template v-for="role of roleList" :key="role.value">
      <a-menu-item class="cv-role-preview-menu" @click="previewAs = role.value">
        <div class="cv-workspace-menu-item group">
