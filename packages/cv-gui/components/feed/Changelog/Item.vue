  ...
  return await renderMarkdown(
    truncate.value
      ? Description.replace(/[*_~]|\[.*?\]|<\/?[^>]+(>|$)/g, '')
          ...
          .replace(/^(\*\*)?#?\s*(\p{Emoji})\s*ClairView\s*v[\d.]+(\s*-\s*|\*\*$)/u, '# ')
          .replace(/(!?\(https?:\/\/[^\s)]+\)(?:\]\(https?:\/\/[^\s)]+(?:\s+"[^"]*")?\))?)/g, '')
          .replace('-', '')
          .substring(0, 100)
          .concat('...')
      : Description.replace(/^\[!\[.*?\]\(https?:\/\/.*?\)\]\(https?:\/\/.*?\)/m, '').replace(
          /^(\*\*)?#?\s*(\p{Emoji})\s*ClairView\s*v[\d.]+(\s*-\s*|\*\*$)/u,
          '# ',
        ),
  )
})
...
const expand = (e) => {
  ...
  $e('c:clairview:feed:changelog:expand', {
    title: Title,
  })
}
  ...
  <div class="relative rounded-xl flex flex-col mt-6.25 bg-white changelog-card">
    ...
    <div class="flex my-4 px-4 items-center justify-between">
      <div class="flex items-center flex-wrap gap-3">
        <NcBadge :border="false" color="brand" class="font-semibold text-[13px] cv-title-badge cursor-pointer whitespace-nowrap">
          {{ Title }}
        </NcBadge>
        <span
          v-for="tag in tags"
...
:deep(.prose) {
  ...
  h1 {
    @apply text-3xl text-cv-content-gray-emphasis  leading-9 mb-0;
    font-weight: 700;
  }

  h2 {
    @apply text-cv-content-gray-emphasis text-xl leading-6 !my-4;
  }
  p {
