...
onMounted(() => {
  ...
  // This is useful for integrating ClairView into other products,
...
<template>
  <a-layout id="cv-app">
    <a-layout class="!flex-col bg-white">
      <a-layout-header
        v-if="!disableTopbar"
        class="cv-table-topbar flex items-center justify-between !bg-transparent !px-3 !py-2 border-b-1 border-gray-200 !h-[46px]"
      >
        <div class="flex items-center gap-6 h-7 max-w-[calc(100%_-_280px)] xs:max-w-[calc(100%_-_90px)]">
          ...
          >
            <img width="96" alt="ClairView" src="~/assets/img/brand/clairview.png" class="flex-none min-w-[96px]" />
          </a>

          <div class="flex items-center gap-2 text-gray-900 text-sm truncate">
            <template v-if="isLoading">
              <span data-testid="cv-loading">{{ $t('general.loading') }}</span>

              <component :is="iconMap.reload" :class="{ 'animate-infinite animate-spin ': isLoading }" />
            </template>

            <div v-else class="text-sm font-semibold truncate cv-shared-view-title flex gap-2 items-center">
              ...
              <NcTooltip v-if="sharedView?.description?.length" placement="bottom">
                ...
                <NcButton type="text" class="!hover:bg-transparent" size="xsmall">
                  <GeneralIcon icon="info" class="!w-3.5 !h-3.5 cv-info-icon text-gray-600" />
                </NcButton>
              </NcTooltip>
            </div>
          </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <a href="https://app.clairview.com/#/signin" target="_blank" class="!no-underline xs:hidden" rel="noopener">
            <NcButton size="xs"> {{ $t('labels.signUpForFree') }} </NcButton>
          </a>
        </div>
      </a-layout-header>
      <div
        class="cv-shared-view-container w-full overflow-hidden"
        :class="{
          'cv-shared-mobile-view': isMobileMode,
        }"
      >
        <slot />
      </div>
...
#cv-app {
  ...
  :deep(.cv-table-toolbar) {
    @apply px-2;
  }

  .cv-shared-view-container {
