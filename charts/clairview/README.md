...
Clairview is an open source low-code platform, helping thousands of teams build apps for their workplace in minutes.
...
```console
$ helm repo add clairview https://clairview.github.io/clairview/
$ helm repo update
$ helm install --create-namespace --namespace clairview clairview clairview/clairview
```

To install the chart from this repo:

```console
$ git clone git@github.com:clairview/clairview.git
$ cd clairview/charts/clairview
$ helm install --create-namespace --namespace clairview clairview .
```

## Example minimal configuration

Here's an example `values.yaml` that would get a Clairview instance running in a home
...
ingress:
  ...
  className: "nginx"
  hosts:
    - host: clairview.local # set this to whatever DNS name you'd use
      paths:
        - backend:
            service:
              name: proxy-service
...
If you wanted to use this when bringing up Clairview in your own cluster, you could save it
to your hard disk and run the following:

```console
$ helm install --create-namespace --namespace clairview clairview . -f values.yaml
```
...
| awsAlbIngress.enabled | bool | `false` | Whether to create an ALB Ingress resource pointing to the Clairview proxy. Requires the AWS ALB Ingress Controller. |
...
| couchdb.couchdbConfig.couchdb.uuid | string | `"clairview-couchdb"` | Unique identifier for this CouchDB server instance. You shouldn't need to change this. |
...
| couchdb.image | object | `{}` | We use a custom CouchDB image for running Clairview and we don't support using any other CouchDB image. You shouldn't change this, and if you do we can't guarantee that Clairview will work. |
...
| globals.appVersion | string | `""` | The version of Clairview to deploy. Defaults to what's specified by {{ .Chart.AppVersion }}. Ends up being used as the image version tag for the apps, proxy, and worker images. |
| globals.automationMaxIterations | string | `"200"` | The maximum number of iterations allows for an automation loop step. You can read more about looping here: <https://docs.clairview.com/docs/looping>. |
| globals.clairviewEnv | string | `"PRODUCTION"` | Sets the environment variable CLAIRVIEW_ENVIRONMENT for the apps and worker pods. Should not ordinarily need to be changed. |
| globals.cookieDomain | string | `""` | Sets the domain attribute of the cookie that Clairview uses to store session information. See <https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies#define_where_cookies_are_sent> for details on why you might want to set this. |
...
| globals.enableAnalytics | string | `"1"` | Whether to enable analytics or not. You can read more about our analytics here: <https://docs.clairview.com/docs/analytics>. |
| globals.google | object | `{"clientId":"","secret":""}` | Google OAuth settings. These can also be set in the Clairview UI, see <https://docs.clairview.com/docs/sso-with-google> for details. |
...
| globals.internalApiKey | string | `""` | API key used for internal Clairview API calls. You don't need to set this if `createSecrets` is true. |
...
| globals.smtp.from | string | `""` | The email address to use in the "From:" field of emails sent by Clairview. |
...
| ingress.enabled | bool | `true` | Whether to create an Ingress resource pointing to the Clairview proxy. |
| ingress.hosts | list | `[]` | Standard hosts block for the Ingress resource. Defaults to pointing to the Clairview proxy. |
...
| service.type | string | `"ClusterIP"` | Service type for the service that points to the main Clairview proxy pod. |
...
| services.redis.password | string | `"clairview"` | The password to use when connecting to Redis. It's recommended that you change this from the default if you're running Redis in-cluster. |
...
To uninstall the chart, assuming you named the release `clairview` (both commands in the installation section do so):

```console
$ helm uninstall --namespace clairview clairview
```
