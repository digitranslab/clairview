...
const getPastClientVersion = async () => {
  const manifestRaw = await fetch(`https://api.github.com/repos/clairview/clairview/contents/packages/client/manifest.json?ref=v${version}`).then(r => r.json());
  ...
  const npmRegistry = await fetch(`https://registry.npmjs.org/@clairview/client/${version}`).then(r => r.json());
  ...
  if (typeof npmRegistry === "string" &&  npmRegistry.startsWith("version not found")) {
    throw `Can't find @clairview/client with version "${version}" in npm registry`
  }

  // Create a temp directory to store the @clairview/client library in
  await fs.mkdir("clientVersionTmp", { recursive: true });

  // Get the tarball of the @clairview/client library and pipe it into tar to extract it
  ...
  await fs.copyFile('./clientVersionTmp/package/dist/clairview-client.js', `./packages/server/build/oldClientVersions/${version}/app.js`);
