...
persistent_store_isdeleted() {
	...
	if [ ! -d "$_persistent_store" ]; then
		...
		for container in $($CONFIG_DOCKER_COMMAND ps | grep -Eo 'clairview-[a-z]+-[0-9]+$'); do
...
check_existing_installation() {
	...
	if [ -d "$NOCO_HOME" ]; then
		NOCO_FOUND=true
	elif $CONFIG_DOCKER_COMMAND ps --format '{{.Names}}' | grep -q "clairview"; then
		NOCO_ID="$($CONFIG_DOCKER_COMMAND ps | grep "digitranslab/clairview" | cut -d ' ' -f 1)"
	...
	# Check if clairview is already installed
	if [ "$NOCO_FOUND" = true ]; then
		echo "ClairView is already installed. And running."
		echo "Do you want to reinstall ClairView? [Y/N] (default: N): "
		read -r REINSTALL
...
check_system_requirements() {
	print_info "Performing ClairView system check and setup"
...
get_user_inputs() {
	CONFIG_DOMAIN_NAME=$(prompt "Enter the IP address or domain name for the ClairView instance" "$(get_public_ip)")
...
get_advanced_options() {
	...
	if [ "$CONFIG_EDITION" = "EE" ]; then
		CONFIG_LICENSE_KEY=$(prompt_required "Enter the ClairView license key")
	else
	...
	NUM_CORES=$(get_nproc)
	CONFIG_NUM_INSTANCES=$(read_number_range "How many instances of ClairView do you want to run?" 1 "$NUM_CORES" 1)
}
...
create_docker_compose_file() {
	...
	if [ "${CONFIG_EDITION}" = "EE" ]; then
		image="digitranslab/clairview-ee:latest"
	else
...
EOF
    ...
    volumes:
      - ./clairview:/usr/app/data
    labels:
      - "com.centurylinklabs.watchtower.enable=true"
      - "traefik.enable=true"
      - "traefik.http.routers.clairview.rule=Host(\`${CONFIG_DOMAIN_NAME}\`)"
EOF
	# IF SSL is Enabled add the following lines
	if [ "$CONFIG_SSL_ENABLED" = "Y" ]; then
		cat >>"$compose_file" <<EOF

      - "traefik.http.routers.clairview.entrypoints=websecure"
      - "traefik.http.routers.clairview.tls=true"
      - "traefik.http.routers.clairview.tls.certresolver=letsencrypt"
EOF
		# If no ssl just configure the web entrypoint
	else
		cat >>"$compose_file" <<EOF
      - "traefik.http.routers.clairview.entrypoints=web"
EOF
	fi
...
EOF
  ...
  minio:
    ...
    volumes:
      - ./minio:/export
    command: -c 'mkdir -p /export/clairview && /usr/bin/minio server /export'
    labels:
      - "traefik.enable=true"
...
EOF
	...
	if [ "${CONFIG_EDITION}" = "EE" ]; then
		echo "DATABASE_URL=postgres://postgres:${encoded_password}@db:5432/clairview" >>"$env_file"
		echo "NC_LICENSE_KEY=${CONFIG_LICENSE_KEY}" >>"$env_file"
	elif [ "${CONFIG_POSTGRES_SQLITE}" = "P" ]; then
		echo "NC_DB=pg://db:5432?d=clairview&user=postgres&password=${encoded_password}" >>"$env_file"
	fi
...
display_completion_message() {
	if [ -n "${CONFIG_DOMAIN_NAME}" ]; then
		if [ "${CONFIG_SSL_ENABLED}" = "Y" ]; then
			message_arr+=("ClairView is now available at https://${CONFIG_DOMAIN_NAME}")
		else
			message_arr+=("ClairView is now available at http://${CONFIG_DOMAIN_NAME}")
		fi
	else
		message_arr+=("ClairView is now available at http://localhost")
	fi
...
management_menu() {
	while true; do
		...
		read -r choice
		case $choice in
		1) start_service && MSG="ClairView Started" ;;
		2) stop_service && MSG="ClairView Stopped" ;;
		3) show_logs ;;
		4) restart_service && MSG="ClairView Restarted" ;;
		5) upgrade_service && MSG="ClairView has been upgraded to latest version" ;;
		6) scale_service && MSG="ClairView has been scaled" ;;
...
scale_service() {
	num_cores=$(get_nproc)
	current_scale=$($CONFIG_DOCKER_COMMAND compose ps -q clairview | wc -l)
	echo -e "\nCurrent number of instances: $current_scale"
	scale_num=$(read_number_range "How many instances of ClairView do you want to run?" 1 "$num_cores" 1)
	...
	$CONFIG_DOCKER_COMMAND compose up -d --scale clairview="$scale_num"
}
